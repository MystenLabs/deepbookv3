...
> Updated after importing the external config worktree `/tmp/predict-wt-config`.
> Suite state at handoff: **predict 317/317 green, predict_math 104/104 green; no compiler warnings; 0 KNOWN-FAILING; BUGS_FOUND.md empty (consistent).**
> Coverage: **145/157 module-qualified error constants tested + 12 documented dispositions; 0 open.**
...
| this commit | Final Phase 2 config-bounds import from `/tmp/predict-wt-config`: protocol-config envelope guards, strike-exposure config/mint-admission guards, matrix regenerated to 145/157 + 12 documented + 0 open, stale `destroy` imports moved to `std::unit_test::destroy`. |

## Key artifacts (all committed)

- `.redesign/COVERAGE_MATRIX.md` — regenerate with `python3 .redesign/gen_coverage_matrix.py`. ✅/📄-documented/❌ per constant; "documented" dispositions live in the `DISPOSITIONS` dict inside the generator (12 entries: defensive / needs-special Lazer / gas-bound / AccumulatorRoot-blocked). Current state: **145/157 covered, 12 documented, 0 open.**
...
## External worktree resolution

- `/tmp/predict-wt-config` was inspected. It contained exactly the two untracked files called out at pause:
  - `packages/predict/tests/config/protocol_config_bounds_tests.move`
  - `packages/predict/tests/config/strike_exposure_config_tests.move`
- Verdict: **merge/import was correct.** The files cover the final P3 config-bounds constants through production setters and real mint admission flows. They were copied into the main worktree, warning-cleaned, formatted, and verified against the full Predict suite.
- The old detached worktree itself did not build because its stale base rejected `edition = "2024"`; that was a worktree/base issue, not a test-logic failure. The main branch suite is the authoritative validation.
- `/tmp/predict-wt-config` is now consumed but was not deleted. It can be removed later after confirming no scratch artifacts are needed.
- **Phase-3 derivation workflow** (`phase3-invariant-derivations.js`) was still running at the original pause and output was likely lost. Re-launch via `Workflow({scriptPath: ".redesign/workflows/phase3-invariant-derivations.js"})`, then author the 8 invariant test files in the main loop from the verified derivations. The 8 scenarios: settled-solvency-boundary (S3/L1), cash-backing-per-flow (S1/S2), multi-expiry-sync-nav (S4/A2), no-double-pay-liquidation (L2), rebate-claim-accounting (A2/A3), supply-withdraw-rounding (A1), liquidation-boundary (P0-7), compaction-parity (P0-8 metamorphic, independence-exempt).
- **Stale Agent-tool worktrees** — `git worktree list` may still show `.claude/worktrees/agent-*` (locked, stale base e276939f) and manual `/tmp/predict-wt-*` worktrees. Clean up consumed ones with `git worktree remove --force <path>` only after confirming their contents are integrated.
...
3. **Wrap-up** — final report format: "N pass, M known-failing (== ledger size)"; manifest check (`grep -rho 'KNOWN-FAILING: BUG-[0-9]\+' packages/predict/tests | sort -u` vs ledger); prettier-format everything touched; both suites run.
