...
///
/// Mirrors the predict indexer's `PredictEventMeta` (`crates/predict-indexer/
/// src/meta.rs`) with the same two deliberate improvements over core:
...
impl OracleEventMeta {
    ...
    /// Always-compiled test constructor so unit tests can build a meta for
    /// `map()` tests.
    pub fn for_test(
        digest: &str,
        sender: &str,
