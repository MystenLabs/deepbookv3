//! Exact-ms Block Scholes surface history (`oracle_lane::ObservationInserted<
//! OracleRead<block_scholes_feed::RawSurface>>`, emitted by
//! `block_scholes_feed::insert_at`). Writes `block_scholes_observation` rows
//! with `is_exact = true`.
...
pub struct BlockScholesObservationInsertedHandler {
    env: OracleEnv,
}

impl BlockScholesObservationInsertedHandler {
...
impl Processor for BlockScholesObservationInsertedHandler {
    const NAME: &'static str = "block_scholes_observation_inserted";
    type Value = Row;
        ...
        for (tx_index, tx) in checkpoint.transactions.iter().enumerate() {
            ...
            for (index, ev) in events.data.iter().enumerate() {
                    ...
                    && payload_is_bs_surface(&ev.type_)
                {
                    ...
                    results.push(map_bs(&decoded, &meta, true));
                    tracing::debug!("Observed block_scholes_observation_inserted event");
                }
            }
        }
        Ok(results)
...
impl sui_indexer_alt_framework::postgres::handler::Handler
    for BlockScholesObservationInsertedHandler
{
    async fn commit<'a>(
        values: &[Self::Value],
