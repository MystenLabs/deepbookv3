//! Live Pyth spot observations (`oracle_lane::ObservationRecorded<OracleRead<
//! pyth_feed::RawSpot>>`, emitted by `pyth_feed::update`). Writes
//! `pyth_observation` rows with `is_exact = false`.
...
pub struct PythObservationRecordedHandler {
    env: OracleEnv,
}

impl PythObservationRecordedHandler {
...
impl Processor for PythObservationRecordedHandler {
    const NAME: &'static str = "pyth_observation_recorded";
    type Value = Row;
        ...
        for (tx_index, tx) in checkpoint.transactions.iter().enumerate() {
            ...
            for (index, ev) in events.data.iter().enumerate() {
                    ...
                    && payload_is_pyth_spot(&ev.type_)
                {
                    ...
                    results.push(map_pyth(&decoded, &meta, false));
                    tracing::debug!("Observed pyth_observation_recorded event");
                }
            }
        }
        Ok(results)
...
#[async_trait::async_trait]
impl sui_indexer_alt_framework::postgres::handler::Handler for PythObservationRecordedHandler {
    async fn commit<'a>(
        values: &[Self::Value],
