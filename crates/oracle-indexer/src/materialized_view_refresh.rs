//! Periodic materialized-view refresh for the oracle analytics views.
//!
//! Mirrors the predict indexer's `materialized_view_refresh.rs`; only the view
//! list differs. Every view here must have a UNIQUE index (required for
//! `REFRESH MATERIALIZED VIEW CONCURRENTLY`) — see the `oracle_lane` migration.
...
const MATERIALIZED_VIEWS_TO_REFRESH: &[&str] = &["oracle_spot_1m"];
