...
use predict_indexer::handlers::{
    AdminVaultBalanceChangedHandler, CollateralizedPositionMintedHandler,
    CollateralizedPositionRedeemedHandler, OracleActivatedHandler, OracleCreatedHandler,
    OraclePricesUpdatedHandler, OracleSettledHandler, OracleSviUpdatedHandler,
    PositionMintedHandler, PositionRedeemedHandler, PredictCreatedHandler,
    PredictManagerCreatedHandler, PricingConfigUpdatedHandler, RiskConfigUpdatedHandler,
    TradingPauseUpdatedHandler,
};
...
async fn main() -> Result<(), anyhow::Error> {
    ...
    indexer
        .concurrent_pipeline(
            AdminVaultBalanceChangedHandler::new(config.clone()),
            Default::default(),
        )
        .await?;
    ...
    indexer
        .concurrent_pipeline(
            CollateralizedPositionMintedHandler::new(config.clone()),
            Default::default(),
        )
        .await?;
    indexer
        .concurrent_pipeline(
            CollateralizedPositionRedeemedHandler::new(config.clone()),
            Default::default(),
        )
        .await?;
