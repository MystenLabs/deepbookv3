...
fun normalize_lazer_price_scales_every_exponent_branch_exactly() {
    ...
    let normalized = lazer_decode::normalize_pyth_price(
...
fun extract_lazer_spot_missing_price_field_aborts() {
    lazer_decode::normalize_pyth_price(
        lazer_decode::extract_lazer_price(option::none()),
        lazer_decode::extract_lazer_exponent(option::some(i16::new(EXPONENT_ZERO, false))),
    );
    abort 999
}
...
fun extract_lazer_spot_missing_exponent_aborts() {
    lazer_decode::normalize_pyth_price(
        lazer_decode::extract_lazer_price(
