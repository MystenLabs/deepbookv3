...
- Block Scholes uses the signed value-batch `source_timestamp_ms`. `apply_spot_batch` independently inserts the canonical spot when that timestamp is a whole-minute boundary, even when the observation does not advance `latest`; `insert_at` performs the same exact-history insertion without changing `latest`. A non-minute timestamp, or a model generated more than `constants::max_block_scholes_settlement_carry_ms` (60s) before the envelope, is a no-op for exact history in both paths and does not abort. A longer model carry still lands on `latest`, where read-time freshness ages it out.
...
The pair is permanent storage for its bound provider base asset rather than a replaceable source wrapper. A bad observation is corrected by a newer signed observation in the same store, and package-version changes use `migrate_value_store` / `migrate_svi_store` in place. If the base-asset binding is wrong or a future store shape cannot be recovered in place, the package upgrade that addresses that concrete failure must also introduce its structural recovery; Propbook does not pre-authorize a generic admin switch to a fresh, empty pair.
