...
/// Risk ratio of the manager against the single debt-side `margin_pool`.
/// Exposed at package scope for the bundled close-and-repay proxy flow, which
/// holds only the `&mut MarginPool<DebtAsset>` it is about to repay into and so
/// cannot use the two-pool `risk_ratio` helper (that would need an immutable
/// borrow of the same pool). Returns `max_risk_ratio` when there is no debt.
public(package) fun risk_ratio_int<BaseAsset, QuoteAsset, DebtAsset>(
    self: &MarginManager<BaseAsset, QuoteAsset>,
    registry: &MarginRegistry,
    base_oracle: &PriceInfoObject,
    quote_oracle: &PriceInfoObject,
