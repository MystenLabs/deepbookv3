...
#[test]
/// Tests that manager_states returns accurate values for multiple managers.
fun manager_states_returns_accurate_values() {
    let (
        mut scenario,
        clock,
        admin_cap,
    ...
    // Create 3 margin managers all owned by user1
    scenario.next_tx(test_constants::user1());
    let mut registry = scenario.take_shared<MarginRegistry>();
    let pool = scenario.take_shared<Pool<BTC, USDC>>();
    let deepbook_registry = scenario.take_shared_by_id<Registry>(registry_id);
    ...
    // Take all 3 managers and deposit different amounts
    scenario.next_tx(test_constants::user1());
    let mut mm1 = scenario.take_shared<MarginManager<BTC, USDC>>();
    let mut mm2 = scenario.take_shared<MarginManager<BTC, USDC>>();
    let mut mm3 = scenario.take_shared<MarginManager<BTC, USDC>>();
    let registry = scenario.take_shared<MarginRegistry>();
    let pool = scenario.take_shared<Pool<BTC, USDC>>();
    let btc_pool = scenario.take_shared_by_id<MarginPool<BTC>>(btc_pool_id);
    let usdc_pool = scenario.take_shared_by_id<MarginPool<USDC>>(usdc_pool_id);

    // Deposit different amounts into each manager
    let usdc_deposit1 = mint_coin<USDC>(100_000_000_000, scenario.ctx());
    ...
    let usdc_deposit2 = mint_coin<USDC>(50_000_000_000, scenario.ctx());
    ...
    let usdc_deposit3 = mint_coin<USDC>(25_000_000_000, scenario.ctx());
    mm3.deposit<BTC, USDC, USDC>(
        &registry,
        &btc_price,
        &usdc_price,
        usdc_deposit3,
    ...
    // Get individual states for comparison
    let (
        id1,
        pool_id1,
        risk1,
        ...
        low1,
        high1,
    ) = mm1.manager_state(&registry, &btc_price, &usdc_price, &pool, &btc_pool, &usdc_pool, &clock);
    ...
    let (
        ...
        low2,
        high2,
    ) = mm2.manager_state(&registry, &btc_price, &usdc_price, &pool, &btc_pool, &usdc_pool, &clock);
    ...
    let (
        ...
        low3,
        high3,
    ) = mm3.manager_state(&registry, &btc_price, &usdc_price, &pool, &btc_pool, &usdc_pool, &clock);

    // Create vector of managers and call manager_states
    let managers = vector[mm1, mm2, mm3];
    let (
        ids,
        pool_ids,
        risks,
    ...
    // Verify all values match individual calls
    assert!(ids.length() == 3);
    assert!(ids[0] == id1);
    assert!(ids[1] == id2);
    assert!(ids[2] == id3);

    assert!(pool_ids[0] == pool_id1);
    assert!(pool_ids[1] == pool_id2);
    assert!(pool_ids[2] == pool_id3);

    assert!(risks[0] == risk1);
    assert!(risks[1] == risk2);
    assert!(risks[2] == risk3);

    assert!(bases[0] == base1);
    assert!(bases[1] == base2);
    assert!(bases[2] == base3);

    assert!(quotes[0] == quote1);
    assert!(quotes[1] == quote2);
    assert!(quotes[2] == quote3);

    assert!(base_debts[0] == base_debt1);
    assert!(base_debts[1] == base_debt2);
    assert!(base_debts[2] == base_debt3);

    assert!(quote_debts[0] == quote_debt1);
    assert!(quote_debts[1] == quote_debt2);
    assert!(quote_debts[2] == quote_debt3);

    assert!(base_pyths[0] == base_pyth1);
    assert!(base_pyths[1] == base_pyth2);
    assert!(base_pyths[2] == base_pyth3);

    assert!(base_decs[0] == base_dec1);
    assert!(base_decs[1] == base_dec2);
    assert!(base_decs[2] == base_dec3);

    assert!(quote_pyths[0] == quote_pyth1);
    assert!(quote_pyths[1] == quote_pyth2);
    assert!(quote_pyths[2] == quote_pyth3);

    assert!(quote_decs[0] == quote_dec1);
    assert!(quote_decs[1] == quote_dec2);
    assert!(quote_decs[2] == quote_dec3);

    assert!(prices[0] == price1);
    assert!(prices[1] == price2);
    assert!(prices[2] == price3);

    assert!(lows[0] == low1);
    assert!(lows[1] == low2);
    assert!(lows[2] == low3);

    assert!(highs[0] == high1);
    assert!(highs[1] == high2);
    assert!(highs[2] == high3);

    // Verify specific values are correct
    assert!(pool_ids[0] == pool_id);
    // Total deposits should sum to 175k USDC (100k + 50k + 25k)
    assert!(quotes[0] + quotes[1] + quotes[2] == 175_000_000_000);

    // Cleanup
    let mut managers = managers;
    let mm3 = managers.pop_back();
    let mm2 = managers.pop_back();
    let mm1 = managers.pop_back();
    managers.destroy_empty();
    destroy_2!(btc_price, usdc_price);
    return_shared_4!(pool, btc_pool, usdc_pool, mm1);
