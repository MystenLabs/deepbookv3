...
/// Unit coverage for expiry-specific fine-grid ticks.
///
/// These tests cover the source-contract behavior only: deriving a market's
/// specific tick from exact Propbook Pyth history, storing it idempotently, and
/// admitting that tick as a mint boundary even when it is off the coarser
/// `admission_tick_size` grid.
#[test_only]
module deepbook_predict::specific_tick_tests;
...
const SPECIFIC_TICK: u64 = 101;
const ADMISSIBLE_OFF_GRID_SPECIFIC_TICK: u64 = 75_788;
const OTHER_OFF_GRID_TICK: u64 = 102;
const SPECIFIC_SPOT_WITH_DUST: u64 = 101_123_456_789;
...
#[test, expected_failure(abort_code = expiry_market::ESpecificTickObservationMissing)]
fun set_specific_tick_missing_exact_history_aborts() {
    ...
    market.set_specific_tick(&config, &oracle_registry, &pyth);
    abort EUnexpectedSuccess
}

#[test, expected_failure(abort_code = expiry_market::EWrongPythFeed)]
fun set_specific_tick_wrong_pyth_feed_aborts() {
    let mut fx = oracle_fixture::setup_oracle_default();
    ...
    market.set_specific_tick(&config, &oracle_registry, &rogue_pyth);
    abort EUnexpectedSuccess
}

#[test]
fun set_specific_tick_floors_spot_and_is_idempotent() {
    let mut fx = oracle_fixture::setup_oracle_default();
    let (
        mut pyth,
        bs_spot,
    ...
    let mut market = take_market(&mut fx);
    let source_timestamp_ms = market.specific_tick_source_timestamp_ms();
    assert_eq!(
        source_timestamp_ms,
        test_constants::default_expiry_ms() - test_constants::default_cadence_period_ms(),
    );

    fx.insert_exact_pyth(&mut pyth, SPECIFIC_SPOT_WITH_DUST, source_timestamp_ms);
    let first_tick = market.set_specific_tick(&config, &oracle_registry, &pyth);
    let second_tick = market.set_specific_tick(&config, &oracle_registry, &pyth);

    assert_eq!(first_tick, SPECIFIC_TICK);
    assert_eq!(second_tick, SPECIFIC_TICK);
    assert_eq!(market.specific_tick().destroy_some(), SPECIFIC_TICK);
...
#[test, expected_failure(abort_code = strike_exposure::EInvalidSpecificTick)]
fun set_specific_tick_floor_to_zero_aborts() {
    let mut fx = oracle_fixture::setup_oracle_default();
    let (
        mut pyth,
        _bs_spot,
...
fun off_grid_tick_before_specific_tick_is_set_aborts() {
    let (
        _fx,
        pricer,
        mut harness,
    ) = setup_priced_harness();

    harness.exposure.allocate_mint_order(
        &pricer,
        SPECIFIC_TICK,
...
#[test]
fun specific_tick_admits_up_and_down_ranges() {
    let (
        fx,
        pricer,
        mut harness,
    ...
    assert_specific_tick_is_off_admission_grid(ADMISSIBLE_OFF_GRID_SPECIFIC_TICK);
    harness.exposure.set_specific_tick(ADMISSIBLE_OFF_GRID_SPECIFIC_TICK);
    let (up_order, _, _) = harness.exposure.allocate_mint_order(
        &pricer,
        ADMISSIBLE_OFF_GRID_SPECIFIC_TICK,
        constants::pos_inf_tick!(),
        test_constants::mint_quantity(),
        test_constants::leverage_one_x(),
    );
    let (down_order, _, _) = harness.exposure.allocate_mint_order(
        &pricer,
        0,
        ADMISSIBLE_OFF_GRID_SPECIFIC_TICK,
        test_constants::mint_quantity(),
    ...
    assert_range(&up_order, ADMISSIBLE_OFF_GRID_SPECIFIC_TICK, constants::pos_inf_tick!());
    assert_range(&down_order, 0, ADMISSIBLE_OFF_GRID_SPECIFIC_TICK);

    cleanup_priced_harness(fx, harness);
}

#[test, expected_failure(abort_code = strike_exposure::EInvalidAdmissionTick)]
fun different_off_grid_tick_after_specific_tick_is_set_aborts() {
    let (
        _fx,
        pricer,
        mut harness,
    ...
    harness.exposure.set_specific_tick(SPECIFIC_TICK);
    harness.exposure.allocate_mint_order(
        &pricer,
        OTHER_OFF_GRID_TICK,
        constants::pos_inf_tick!(),
...
fun assert_specific_tick_is_off_admission_grid(specific_tick: u64) {
    assert!(
        (specific_tick * test_constants::default_tick_size())
            % test_constants::default_admission_tick_size()
            != 0,
    );
}
