...
fun write_feed_then_snapshot_flush_same_tx_aborts() {
    ...
    // The flush's oracle read moved from `value_expiry` to the snapshot stage, so the
    // guard's coverage has to move with it. A keeper that refreshes the surface and
    // snapshots in one PTB is refused: the refresh must be a strictly earlier
    // transaction, and the snapshot must then land inside the feeds' freshness windows.
    // The snapshot stage is atomic, so this abort reverts `start_pool_valuation` with
    // it and the valuation lock is never left engaged by a failed refresh.
    fx.set_clock_for_testing(FRESHER_SOURCE_TS);
