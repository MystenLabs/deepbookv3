...
/// conversion with its open-ended sentinels, the two raw→tick crossings
/// (settlement prefix threshold, reference grid snap), and the settlement win
/// predicate. Every expected value is hand-derived from the documented formula,
...
// === strike_from_tick (tick -> raw Strike, with sentinels) ===

#[test]
fun strike_from_tick_finite_tick_scales_by_tick_size() {
    // 100 ticks at 1e9 raw per tick = 100e9 raw.
    let strike = range_codec::strike_from_tick(100, TICK_SIZE);
    assert_eq!(strike.value(), 100_000_000_000);
    assert!(!strike.is_neg_inf());
    assert!(!strike.is_pos_inf());
}

#[test]
fun strike_from_tick_maps_tick_zero_to_neg_inf() {
    let strike = range_codec::strike_from_tick(0, TICK_SIZE);
    assert_eq!(strike.value(), constants::neg_inf!());
    assert!(strike.is_neg_inf());
}

#[test]
fun strike_from_tick_maps_pos_inf_tick_to_pos_inf() {
    let strike = range_codec::strike_from_tick(POS_INF_TICK, TICK_SIZE);
    assert_eq!(strike.value(), constants::pos_inf!());
    assert!(strike.is_pos_inf());
}

#[test]
fun strike_from_tick_max_finite_tick_stays_finite() {
    // One tick below the sentinel is an ordinary finite conversion.
    let strike = range_codec::strike_from_tick(POS_INF_TICK - 1, TICK_SIZE);
    assert_eq!(strike.value(), (POS_INF_TICK - 1) * TICK_SIZE);
    assert!(!strike.is_pos_inf());
}
...
#[test]
fun grid_tick_floors_a_spot_to_its_containing_tick() {
    // Exact multiple: a spot on a tick boundary is that tick.
    assert_eq!(range_codec::grid_tick(150_000_000_000, TICK_SIZE), 150);
    // One raw unit above a boundary still floors to the same tick — the opposite
    // rounding from prefix_limit_tick, which is a strict-inequality boundary.
    assert_eq!(range_codec::grid_tick(150_000_000_001, TICK_SIZE), 150);
    // One raw unit below a boundary floors to the previous tick.
    assert_eq!(range_codec::grid_tick(149_999_999_999, TICK_SIZE), 149);
    // A sub-tick spot floors to tick 0.
