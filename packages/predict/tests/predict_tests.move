...
#[test]
fun set_trading_paused_and_unpause() {
    let ctx = &mut tx_context::dummy();
    let mut predict = create_predict(ctx);

    predict.set_trading_paused(true);
    predict.set_trading_paused(false);
...
fun set_base_spread() {
    ...
    predict.set_base_spread(100_000_000);
...
fun set_min_spread() {
    ...
    predict.set_min_spread(10_000_000);

    destroy(predict);
}

#[test]
fun set_utilization_multiplier() {
    let ctx = &mut tx_context::dummy();
    let mut predict = create_predict(ctx);

    predict.set_utilization_multiplier(3_000_000_000);

    destroy(predict);
}

#[test]
fun set_max_total_exposure_pct() {
    let ctx = &mut tx_context::dummy();
    let mut predict = create_predict(ctx);

    predict.set_max_total_exposure_pct(500_000_000);
