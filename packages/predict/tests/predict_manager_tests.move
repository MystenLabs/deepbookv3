...
fun position_returns_zero_for_unknown_key() {
    ...
    scenario.next_tx(ALICE);
    {
        ...
        let key = market_key::up(oracle_id, 1000, 50000);
...
fun lock_collateral_moves_free_to_locked() {
    ...
    scenario.next_tx(ALICE);
    {
        let mut manager = scenario.take_shared<PredictManager>();
        let oracle_id = dummy_oracle_id(scenario.ctx());
        let up_key = market_key::up(oracle_id, 1000, 50000);
        let down_key = market_key::down(oracle_id, 1000, 50000);
...
fun different_collateral_pairs_tracked_independently() {
    ...
    scenario.next_tx(ALICE);
    {
        ...
        let key_a = market_key::up(oracle_id_1, 1000, 50000);
        let key_b = market_key::down(oracle_id_1, 1000, 50000);
        let key_c = market_key::up(oracle_id_2, 2000, 60000);
        let key_d = market_key::down(oracle_id_2, 2000, 60000);
