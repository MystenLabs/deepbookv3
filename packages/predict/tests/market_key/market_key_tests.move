...
#[test]
fun oracle_id_returns_correct_id() {
    let ctx = &mut tx_context::dummy();
    let oracle = create_oracle(5000, ctx);
    let oracle_id = oracle::id(&oracle);

    let key = market_key::up(oracle_id, 5000, 100_000_000_000);
    assert_eq!(market_key::oracle_id(&key), oracle_id);
...
#[test]
fun expiry_returns_correct_value() {
    let ctx = &mut tx_context::dummy();
    let oracle = create_oracle(12345, ctx);
    let oracle_id = oracle::id(&oracle);

    let key = market_key::up(oracle_id, 12345, 50_000_000_000);
    assert_eq!(market_key::expiry(&key), 12345);

    destroy(oracle);
}

#[test]
fun strike_returns_correct_value() {
    let ctx = &mut tx_context::dummy();
    let oracle = create_oracle(9999, ctx);
    let oracle_id = oracle::id(&oracle);

    let key = market_key::down(oracle_id, 9999, 42_000_000_000);
    assert_eq!(market_key::strike(&key), 42_000_000_000);

    destroy(oracle);
}

#[test]
fun is_up_true_for_up_key() {
    let ctx = &mut tx_context::dummy();
    let oracle = create_oracle(1000, ctx);
    let key = market_key::up(oracle::id(&oracle), 1000, 50_000_000_000);
...
#[test]
fun is_up_false_for_down_key() {
    let ctx = &mut tx_context::dummy();
    let oracle = create_oracle(1000, ctx);
    let key = market_key::down(oracle::id(&oracle), 1000, 50_000_000_000);
