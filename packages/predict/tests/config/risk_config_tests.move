...
fun new_returns_correct_default() {
    ...
    assert_eq!(config.max_total_exposure_pct(), 800_000_000);
    destroy(config);
}
...
fun set_max_total_exposure_pct_upper_boundary() {
    let mut config = risk_config::new();
    config.set_max_total_exposure_pct(1_000_000_000);
    assert_eq!(config.max_total_exposure_pct(), 1_000_000_000);
    destroy(config);
}
...
fun set_max_total_exposure_pct_exceeds_max_aborts() {
    let mut config = risk_config::new();
    config.set_max_total_exposure_pct(1_000_000_001);
