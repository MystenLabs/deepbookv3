...
A confirmed production finding remains a plain `#[test]` that fails for the independently established reason. Enroll it with one fully qualified row in `known_red_manifest.csv` and one matching `**Known RED test:**` field on its `predeploy/open-items.md` entry. Never use `expected_failure`, a random test, a comment/ignore mechanism, or a fixture value that normalizes the finding. `check_known_red.py` first requires a warning-strict build, then accepts only exact equality between the full Move-test failing set and the manifest; an unlisted failure or a row whose test turns green fails the gate.

Use `**Deferred test:**` only for a named test that an open item still requires but this branch has not implemented. The predeploy checker stops if that function is also a registered response-policy pin, so policy evidence cannot be silently weakened into general backlog.
...
Run `python3 packages/predict/tests/check_architecture.py`, `python3 packages/predict/tests/check_predeploy_debt.py`, `python3 -m unittest discover -s packages/predict/tests -p "*_test.py"`, and `python3 packages/predict/tests/reference/generate_pricing_reference.py --check` before the Move commands. Run `python3 packages/predict/tests/check_known_red.py` for the warning-strict build plus exact failing-set acceptance gate. Linux CI pins Python 3.11 for the stale-output check and runs the raw Predict suite warning-strict; once a known RED exists, that raw CI failure remains intentional evidence. The deterministic structural checks reject executable tests and unapproved new test seams across Predict-cluster production sources and verify the single Scenario owner, exact Clock-only `OwnedResources`, the frozen World transaction-progression surface, ID-based shared retrieval, every post-bootstrap transaction-progression call only in test bodies, at most one World per test, exact executable module taxonomy and category selection, direct positive-test assertions, the known-RED test/manifest/open-item bijection, live generated-data provenance links, and the exact transitional predeploy debt set: missing executable pins, manually reviewed unreachable branches and accumulator-delivery obligations, uncatalogued policies, explicit non-unit exemptions, and stale named paths. The branch remains an intermediate draft until that transitional debt manifest is empty, the wrapper is deleted, and the strict predeploy checker is clean. Structural checks freeze semantic-gap rows but do not prove their meaning.
