...
fun accrue_fee_splits_sixty_twenty_twenty() {
    ...
    assert_eq!(reserve.protocol_asset_balance<SUI>(), 20);
    assert_eq!(reserve.insurance_asset_balance<SUI>(), 20);
    ...
    reserve.drain_protocol_for_testing<SUI>().into_coin(ctx).burn_for_testing();
    reserve.drain_insurance_for_testing<SUI>().into_coin(ctx).burn_for_testing();
    reserve.destroy_empty_for_testing();
}
...
fun set_fee_shares_changes_future_fee_split() {
    ...
    assert_eq!(reserve.protocol_asset_balance<SUI>(), 10);
    assert_eq!(reserve.insurance_asset_balance<SUI>(), 20);
    assert_eq!(reserve.total_fees_accrued(), 100);
...
fun accrue_fee_assigns_rounding_dust_to_lp() {
    let ctx = &mut tx_context::dummy();
    let mut reserve = fee_reserve::new(ctx);
    let fee = coin::mint_for_testing<SUI>(ROUNDED_FEE, ctx).into_balance();
...
fun accrue_zero_fee_does_not_update_counters() {
    let ctx = &mut tx_context::dummy();
    let mut reserve = fee_reserve::new(ctx);
    let fee = balance::zero<SUI>();

    let lp_fee = reserve.accrue_fee(fee, object::id_from_address(PREDICT_ID));

    assert_eq!(lp_fee.value(), 0);
    assert_eq!(reserve.protocol_asset_balance<SUI>(), 0);
    assert_eq!(reserve.insurance_asset_balance<SUI>(), 0);
