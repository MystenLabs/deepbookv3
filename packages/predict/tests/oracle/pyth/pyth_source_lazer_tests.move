...
#[test_only]
module deepbook_predict::pyth_source_lazer_tests;

use deepbook_predict::pyth_source;
...
#[test]
fun normalize_lazer_price_scales_every_exponent_branch_exactly() {
    // BTC-style: 65_000.12345678 USD as (6_500_012_345_678, -8).
    // Target shift = 9 - 8 = 1:
    //   6_500_012_345_678 * 10 = 65_000_123_456_780.
    let normalized = pyth_source::normalize_pyth_price_for_testing(
        i64::new(BTC_PYTH_MAGNITUDE, false),
        i16::new(EXPONENT_NEG_8, true),
    );
    assert_eq!(normalized, BTC_NORMALIZED_SPOT);

    // exponent = -9 exactly: target shift is zero, so magnitude passes through.
    let normalized = pyth_source::normalize_pyth_price_for_testing(
        i64::new(IDENTITY_MAGNITUDE, false),
        i16::new(EXPONENT_NEG_9, true),
    );
    ...
    let normalized = pyth_source::normalize_pyth_price_for_testing(
        i64::new(DIVIDE_MAGNITUDE, false),
        i16::new(EXPONENT_NEG_12, true),
    );
    ...
    //   5 / 1_000 = 0.
    let normalized = pyth_source::normalize_pyth_price_for_testing(
        i64::new(SUB_UNIT_MAGNITUDE, false),
        i16::new(EXPONENT_NEG_12, true),
    );
    ...
    let normalized = pyth_source::normalize_pyth_price_for_testing(
        i64::new(ONE_MAGNITUDE, false),
        i16::new(EXPONENT_ZERO, false),
    );
    ...
    let normalized = pyth_source::normalize_pyth_price_for_testing(
        i64::new(THREE_MAGNITUDE, false),
        i16::new(EXPONENT_POS_2, false),
    );
    assert_eq!(normalized, THREE_E11_SPOT);

    let normalized = pyth_source::normalize_pyth_price_for_testing(
        i64::new(ZERO_MAGNITUDE, false),
        i16::new(EXPONENT_NEG_8, true),
    );
    assert_eq!(normalized, ZERO_MAGNITUDE);
}

#[test, expected_failure(abort_code = pyth_source::ELazerFeedNotFound)]
fun extract_lazer_spot_missing_feed_aborts() {
    pyth_source::extract_spot_fields_for_testing(
        false,
        option::some(option::some(i64::new(ONE_MAGNITUDE, false))),
        option::some(i16::new(EXPONENT_ZERO, false)),
    );
    abort 999
}

#[test, expected_failure(abort_code = pyth_source::ELazerPriceUnavailable)]
...
#[test, expected_failure(abort_code = pyth_source::ELazerNegativePrice)]
fun normalize_lazer_price_negative_price_aborts() {
    pyth_source::normalize_pyth_price_for_testing(
        i64::new(ONE_MAGNITUDE, true),
        i16::new(EXPONENT_ZERO, false),
    );
    abort 999
}
