...
/// Live close timestamp and slippage-policy guards through production entrypoints.
#[test_only]
module deepbook_predict::scope_flow__intent_guard__redeem_tests;
...
fun same_timestamp_mint_then_live_redeem_aborts() {
    ...
    let auth = account::generate_auth(test_world::ctx(&mut world));
    let (_closed, _replacement) = market.redeem_live(
        &mut wrapper,
        auth,
        &config,
        &pricer,
...
fun live_redeem_below_min_probability_aborts() {
...
fun live_redeem_below_min_proceeds_aborts() {
    let (mut world, mut resources) = test_world::new(
        test_values::system(),
        test_values::admin(),
        test_values::now_ms(),
