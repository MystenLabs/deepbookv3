...
/// Exact all-in cost and entry-probability slippage guards for live minting.
#[test_only]
module deepbook_predict::scope_flow__intent_guard__mint_slippage_tests;
...
fun mint_above_zero_max_probability_aborts() {
    let (mut world, resources) = test_world::new(
        test_values::system(),
        test_values::admin(),
        test_values::now_ms(),
    ...
    assert_eq!(quote.entry_probability(), ATM_ENTRY_PROBABILITY);
    let auth = account::generate_auth(test_world::ctx(&mut world));
    let _ = market.mint_exact_quantity(
        &mut wrapper,
        auth,
