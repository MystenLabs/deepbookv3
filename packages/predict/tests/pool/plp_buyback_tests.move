...
#[test_only]
/// Permissionless DEEP buyback funded from the protocol reserve: a configurable slice
/// of each expiry's protocol profit is authorized as a DUSDC `buyback_budget`, and
/// anyone may sell DEEP into the pool for DUSDC out of that budget. The DEEP is priced
/// off the registry-bound DEEP `PythSource` (rounded down, minus the configured
/// discount), paid from `protocol_reserve_balance` (excluded from LP NAV), and folded
/// into the LP-owned `incentive_deep` released balance (so it enters PLP NAV and is
/// paid in-kind on withdrawal). Each swap moves value from protocol revenue into an
/// LP-owned DEEP position (NAV-accretive to LPs) and never touches idle liquidity, so
...
/// Seed the buyback budget and the protocol reserve it draws from.
fun seed_buyback(vault: &mut PoolVault, scenario: &mut test::Scenario, budget: u64, reserve: u64) {
    vault.set_buyback_budget_for_testing(budget);
    vault.set_protocol_reserve_for_testing(reserve, scenario.ctx());
}
...
#[test]
fun swap_pays_dusdc_from_reserve_and_folds_deep() {
    ...
    seed_buyback(&mut vault, &mut scenario, TWO_HUNDRED_DUSDC, TWO_HUNDRED_DUSDC);

    // 50 DEEP @ $1 = $50, paid from the protocol reserve; idle is never touched.
    ...
    assert_eq!(vault.protocol_reserve_balance(), TWO_HUNDRED_DUSDC - FIFTY_DUSDC);
    assert_eq!(vault.idle_balance(), THOUSAND_DUSDC);
...
fun swap_applies_discount() {
    ...
    assert_eq!(vault.protocol_reserve_balance(), TWO_HUNDRED_DUSDC - 45_000_000);
    assert_eq!(vault.incentive_deep_balance(), FIFTY_DEEP);
...
// === swap: NAV effect (protocol subsidizes LPs) ===

#[test]
fun nav_rises_when_protocol_funds_buyback() {
    let (mut scenario, registry, admin_cap, mut vault, mut config, clk) = begin();
    ...
    // The protocol funds 50 DEEP into LP-owned holdings at no cost to idle: NAV rises
    // from 1000 to 1050 (1000 idle + $50 DEEP), so share price becomes 1.05.
    let out = swap(&registry, &mut vault, &config, &mut scenario, &clk, usd_spot(1), FIFTY_DEEP, 0);
    ...
    let bob_lp = plp::supply(
        ...
        &mut config,
        sync,
        coin::mint_for_testing<DUSDC>(THOUSAND_DUSDC + FIFTY_DUSDC, scenario.ctx()),
        &src,
        &src,
        &clk,
        scenario.ctx(),
    );
    // At share price 1.05, a 1050 DUSDC deposit mints exactly 1000 PLP.
...
// === firewall: withdraw never touches the budget or the protocol reserve ===

#[test]
fun withdraw_does_not_spend_buyback_budget_or_reserve() {
    let (mut scenario, registry, admin_cap, mut vault, mut config, clk) = begin();
    let alice_lp = bootstrap_supply(&mut vault, &mut config, &mut scenario, &clk, THOUSAND_DUSDC);
    seed_buyback(&mut vault, &mut scenario, TWO_HUNDRED_DUSDC, TWO_HUNDRED_DUSDC);

    // A full withdrawal pays only from idle; budget and protocol reserve are untouched.
    let sync = plp::start_pool_sync(&mut config, &vault);
    ...
    assert_eq!(vault.protocol_reserve_balance(), TWO_HUNDRED_DUSDC);
...
fun swap_exceeds_budget_aborts() {
    let (mut scenario, registry, _admin_cap, mut vault, mut config, clk) = begin();
    let alice_lp = bootstrap_supply(&mut vault, &mut config, &mut scenario, &clk, THOUSAND_DUSDC);
    // Budget below the 50_000_000 the swap would pay, though the reserve is ample.
    seed_buyback(&mut vault, &mut scenario, FIFTY_DUSDC - 1, TWO_HUNDRED_DUSDC);
    let out = swap(&registry, &mut vault, &config, &mut scenario, &clk, usd_spot(1), FIFTY_DEEP, 0);
    destroy(out);
    destroy(alice_lp);
    abort
...
#[test, expected_failure(abort_code = plp::EInsufficientProtocolReserve)]
fun swap_exceeds_protocol_reserve_aborts() {
    let (mut scenario, registry, _admin_cap, mut vault, mut config, clk) = begin();
    let alice_lp = bootstrap_supply(&mut vault, &mut config, &mut scenario, &clk, THOUSAND_DUSDC);
    // Budget is ample but the protocol reserve is short: the reserve guard must bind.
    seed_buyback(&mut vault, &mut scenario, TWO_HUNDRED_DUSDC, FIFTY_DUSDC - 1);
    let out = swap(&registry, &mut vault, &config, &mut scenario, &clk, usd_spot(1), FIFTY_DEEP, 0);
    destroy(out);
    destroy(alice_lp);
    abort
