...
use deepbook_predict::{i64, market_oracle::{Self, MarketOracle, MarketOracleCap}, registry};
...
fun unregistered_cap_cannot_update_market_oracle() {
    let ctx = &mut tx_context::dummy();
    let (mut market, _cap, admin_cap, clock) = setup(ctx);
...
fun registered_cap_can_update_market_oracle() {
    let ctx = &mut tx_context::dummy();
    let (mut market, cap_1, admin_cap, clock) = setup(ctx);
    let cap_2 = registry::create_market_oracle_cap(&admin_cap, ctx);
...
fun unregistered_cap_loses_market_oracle_access() {
    let ctx = &mut tx_context::dummy();
    let (mut market, _cap_1, admin_cap, clock) = setup(ctx);
    let cap_2 = registry::create_market_oracle_cap(&admin_cap, ctx);

    registry::register_market_oracle_cap(&mut market, &admin_cap, &cap_2);
    write_svi(&mut market, &cap_2, FIRST_SVI_SOURCE_TIMESTAMP_MS, &clock);
    registry::unregister_market_oracle_cap(&mut market, &admin_cap, object::id(&cap_2));
    write_svi(&mut market, &cap_2, SECOND_SVI_SOURCE_TIMESTAMP_MS, &clock);
    abort 999
}
...
fun self_unregistered_cap_loses_market_oracle_access() {
    let ctx = &mut tx_context::dummy();
    let (mut market, cap, _admin_cap, clock) = setup(ctx);
...
fun setup(ctx: &mut TxContext): (MarketOracle, MarketOracleCap, registry::AdminCap, clock::Clock) {
    let admin_cap = registry::create_admin_cap_for_testing(ctx);
