...
fun contract_price_at_boundary_is_within() {
    let price = certified::new(CENTER, MAX_PRICE_ERROR);
    assert!(price.relative_deviation_within(CONTRACT_MAX_DEVIATION));
}

#[test]
fun contract_price_above_boundary_is_not_within() {
    let price = certified::new(CENTER, PRICE_ERROR_ABOVE_MAX);
    assert!(!price.relative_deviation_within(CONTRACT_MAX_DEVIATION));
}

#[test]
fun pool_nav_at_boundary_is_within() {
    let nav = certified::new(CENTER, MAX_NAV_ERROR);
    assert!(nav.relative_deviation_within(NAV_MAX_DEVIATION));
}

#[test]
fun pool_nav_above_boundary_is_not_within() {
    let nav = certified::new(CENTER, NAV_ERROR_ABOVE_MAX);
    assert!(!nav.relative_deviation_within(NAV_MAX_DEVIATION));
}

#[test, expected_failure(abort_code = plp::ENavTooImprecise)]
fun pool_nav_with_unrepresentable_supply_ask_aborts() {
    let nav = certified::new(std::u64::max_value!(), 1);
