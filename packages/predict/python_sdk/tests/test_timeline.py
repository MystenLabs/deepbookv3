...
def _timelines(config, markets, now):
    # created_expiries (state-free) drives backlog; here it's just the markets we pass.
    return _build_cadence_timelines(_enabled(config), markets, {m.expiry_ms for m in markets}, now)
...
class TimelineTests(unittest.TestCase):
    def test_slot_expiries_and_live_mapping(self) -> None:
        ...
        live = _market(1_800_000_300_000)  # next 5m expiry strictly after now
        timelines = _timelines(config, [live], now)
        ...
        self.assertTrue(five_min.has_live_market)
    ...
    def test_top_of_hour_market_belongs_to_hourly_cadence(self) -> None:
        ...
        timelines = _timelines(config, [market], now)
        five_min = next(t for t in timelines if t.name == "5m")
