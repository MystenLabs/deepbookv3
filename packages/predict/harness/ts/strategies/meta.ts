...
// runtime: per-strategy runner config (tickMs/maxOps/fund/requiresTimeout) + the enabled cadence set (id + window)
// that every keeper runs and the oracle grid must cover.
//   { "strategies": { "<name>": { "tickMs", "maxOps", "fund", "requiresTimeout" } }, "cadences": [ { "id", "windowSize" } ] }
