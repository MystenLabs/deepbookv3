...
// runtime: per-strategy runner config (tickMs/maxOps/fund/gasBudget/requiresTimeout/inventoryGrid/inventoryImpactMaxRate) + the enabled cadence set (id + period + window)
// that every keeper runs and the oracle grid must cover.
//   { "strategies": { "<name>": { "tickMs", "maxOps", "fund", "gasBudget", "requiresTimeout", "inventoryGrid", "inventoryImpactMaxRate" } }, "cadences": [ { "id", "windowSize", "periodMs" } ] }
