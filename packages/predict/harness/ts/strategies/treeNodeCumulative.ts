...
// cap. Then holds so the keeper flush runs with both. NOTE: since RP-25 each value_expiry is
// its OWN transaction, so the cumulative wall this strategy was written to demonstrate no longer
// exists — the per-market budget (C-4) is what a rerun measures now.
...
//     value_expiry) => the wall is max single-market nodes. This is now the shipped shape.
