...
// keeper's flush is measured. NOTE: pre-RP-25 this was ONE PTB (a value_expiry command per active
// market + finish_flush); the snapshot stage is now the only atomic part and each value_expiry is
// its own transaction, so the pool-total object wall this measured is gone. Kept for the compute figures.
