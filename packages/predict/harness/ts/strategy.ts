...
export function makeContext(deps: ContextDeps): StrategyCtx {
  ...
  const envFor = (market: Mkt): { pythSpot: number; bsSpot: number; bsForward: number; svi: any } | null => {
    ...
    // Match load_live_pricer: use Block Scholes' own signed spot for the basis re-anchor,
    // then roll a/b from this SVI observation's source timestamp to quote time.
