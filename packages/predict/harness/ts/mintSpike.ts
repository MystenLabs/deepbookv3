...
// The refresh writes the SAME snapshot the resolver priced against, so on-chain pricing
// matches the off-chain selection (only math drift). Refresh and mint are two PTBs
// because a live pricer may not read an observation written in its own transaction.
...
import { POOL_VAULT_ID, PROTOCOL_CONFIG_ID, depositToAccountTx, executeAllAndWait, executeAndWait, rebalanceExpiryCashTx, refreshOracleAndMintTxs } from "./runtime.js";
...
async function main() {
  ...
  const mintTxs = await refreshOracleAndMintTxs({
  ...
  const mintR = await executeAllAndWait(mintTxs, "mint");
