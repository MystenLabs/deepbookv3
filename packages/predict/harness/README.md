...
Duration-only strategies require `campaign --timeout` and may stop successfully at that bound. Strategies with `maxOps` or semantic completion fail as incomplete if they are still running at the deadline. Strategy metadata is read from the TypeScript registry, so the Python router does not duplicate funding, cadence, or completion configuration.
