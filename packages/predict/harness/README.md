...
A monotone ladder makes the budget and the book rise together, so the fitted per-candidate cost
absorbs any per-book-entry cost of the mint itself (deeper treap, more index pages). The bias is
conservative — it lowers the reported ceiling — but a non-monotone ladder such as
`0:24,600:3000,1200:512,1800:1500,2400:24` decorrelates the two and isolates the pass. The adverse
arm additionally cannot hold a deep book: every mint's pass knocks orders out, and batching makes
that worse rather than better, so its samples may cover a much narrower candidate range than the
ladder suggests. The analysis refuses to report a ceiling in that case rather than extrapolating
from it.
