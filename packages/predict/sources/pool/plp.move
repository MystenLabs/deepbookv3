...
const EInsufficientProtocolReserve: u64 = 20;
...
public struct PoolVault has key {
    ...
    /// Cumulative protocol-reserve DUSDC authorized for DEEP buybacks and not yet
    /// spent. A cap on buyback volume, not a segregated balance: the DUSDC stays in
    /// `protocol_reserve_balance` (already excluded from LP NAV) and is spent down by
    /// `execute_deep_buyback`. Holds `<= protocol_reserve_balance` by construction.
    buyback_budget: u64,
...
/// Return cumulative protocol-reserve DUSDC authorized for DEEP buybacks and not yet spent.
...
///
/// Permissionless, funded by the protocol reserve: the seller's DEEP is priced off
/// `deep_source` (rounded down, minus the configured discount) and folded into the
/// LP-owned `incentive_deep` released balance — valued into NAV on `supply` and paid
/// in-kind on `withdraw` — while the DUSDC payout comes from `protocol_reserve_balance`,
/// capped by `buyback_budget`. The protocol reserve is excluded from LP NAV, so each
/// swap moves value into LP-owned DEEP: the pool subsidizes LPs a DEEP position out of
/// protocol revenue, and idle liquidity (hence withdrawals) is never touched.
/// `(decimals, feed_id)` are the registry's DEEP binding, supplied by
/// `registry::swap_deep_for_dusdc`, which owns the feed-config lookup (`registry`
/// depends on `plp`, not the reverse).
public(package) fun execute_deep_buyback(
    ...
    assert!(vault.protocol_reserve_balance.value() >= dusdc_out, EInsufficientProtocolReserve);

    vault.route_deep_into_incentive(deep, decimals, feed_id);
    vault.buyback_budget = vault.buyback_budget - dusdc_out;
    let payout = vault.protocol_reserve_balance.split(dusdc_out).into_coin(ctx);
...
fun materialize_expiry_profit(
    ...
    // Authorize a slice of the protocol profit for DEEP buybacks. This moves no cash —
    // the DUSDC stays in `protocol_reserve_balance` (already outside LP NAV); the budget
    // only caps how much can later be swapped into DEEP via `execute_deep_buyback`.
    vault.buyback_budget =
        vault.buyback_budget + math::mul(protocol_profit, config.fee_config().buyback_share());
...
#[test_only]
/// Seed the protocol reserve balance directly (the source the buyback pays from).
public fun set_protocol_reserve_for_testing(
    vault: &mut PoolVault,
    amount: u64,
    ctx: &mut TxContext,
) {
    let current = vault.protocol_reserve_balance.value();
    if (amount > current) {
        vault
            .protocol_reserve_balance
            .join(coin::mint_for_testing<DUSDC>(amount - current, ctx).into_balance());
    } else {
        vault.protocol_reserve_balance.split(current - amount).into_coin(ctx).burn_for_testing();
    };
}
