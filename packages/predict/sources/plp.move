...
/// DUSDC, the protocol reserve, and per-expiry cash accounting. It coordinates the
...
/// settled-market sweep with terminal profit materialization). Synchronous PLP
/// supply/withdraw remain pruned pending the async LP redesign; incentives moved to
/// a separate staking contract. DEEP staking is an unrelated trading feature.
...
/// exactly once (a swept settled market contributes 0); `finish_pool_valuation`
/// proves every snapshotted market was valued and returns the LP-attributable
/// pool NAV. Has no abilities, so it must be consumed by the finisher.
...
/// Finish a full-pool valuation: prove every snapshotted market was valued exactly
/// once, release the lock, consume the potato, and return the LP-attributable
/// pool-wide DUSDC NAV (idle + Σ active NAV, net of the pending-protocol-profit
/// exclusion priced from the aggregate profit basis).
public fun finish_pool_valuation(
