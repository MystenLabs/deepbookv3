...
/// Predict-owned market setup and policy configuration.
///
...
/// overrides. At market oracle creation the thresholds and per-asset bounds are
...
const EMarketConfigNotFound: u64 = 3;
...
public struct MarketGrid has copy, drop, store {
    min_strike: u64,
    max_strike: u64,
    tick_size: u64,
}
...
public struct MarketConfig has store {
    ...
    /// with the other scalars on `MarketConfig`; narrowed to `u32` (Lazer's
...
public(package) fun asset_basis_bounds(
    ...
    if (market_config.asset_basis_bounds.contains(asset)) {
        option::some(market_config.asset_basis_bounds[asset])
    } else {
        option::none()
    }
}
...
public(package) fun asset_feed_id(market_config: &MarketConfig, asset: String): Option<u64> {
    if (market_config.asset_feed_ids.contains(asset)) {
        option::some(market_config.asset_feed_ids[asset])
    } else {
        option::none()
    }
}
...
public(package) fun new(ctx: &mut TxContext): MarketConfig {
    MarketConfig {
        market_grids: table::new(ctx),
        market_ask_bounds: table::new(ctx),
...
public(package) fun build_market_oracle_bounds(
    ...
    let (
        ...
        min_basis,
        max_basis,
    ) = market_config.resolve_basis_bounds(underlying_asset);
...
/// oracles; operators retune their own market_oracle bounds if needed.
...
public(package) fun set_asset_basis_bounds(
    ...
    if (market_config.asset_basis_bounds.contains(asset)) {
        let row = &mut market_config.asset_basis_bounds[asset];
        *row = bounds;
    } else {
        market_config.asset_basis_bounds.add(asset, bounds);
    }
}
...
public(package) fun add_market_grid(
    ...
    market_config.market_grids.add(oracle_id, grid);
}
...
/// constraint is enforced by the caller (see `predict::set_market_ask_bounds`).
public(package) fun set_market_ask_bounds(
    ...
    if (market_config.market_ask_bounds.contains(oracle_id)) {
        let row = &mut market_config.market_ask_bounds[oracle_id];
        *row = bounds;
    } else {
        market_config.market_ask_bounds.add(oracle_id, bounds);
    }
}
...
public(package) fun clear_market_ask_bounds(
    ...
    let oracle_id = market_oracle.id();
    if (market_config.market_ask_bounds.contains(oracle_id)) {
        market_config.market_ask_bounds.remove(oracle_id);
    };
}
...
public(package) fun ask_bounds_override(
    ...
    if (market_config.market_ask_bounds.contains(oracle_id)) {
        option::some(market_config.market_ask_bounds[oracle_id])
    } else {
        option::none()
    }
}
...
public(package) fun assert_valid_strike(
    ...
    let oracle_id = market_oracle.id();
    let (min_strike, tick_size, max_strike) = market_config.grid_params(oracle_id);
...
public(package) fun assert_range_key_matches(
    ...
    if (range_key.lower_strike() != constants::neg_inf!()) {
        market_config.assert_valid_strike(market_oracle, range_key.lower_strike());
    };
    if (range_key.higher_strike() != constants::pos_inf!()) {
        market_config.assert_valid_strike(market_oracle, range_key.higher_strike());
    };
}

/// Load the configured strike-grid parameters for an market_oracle.
public(package) fun grid_params(market_config: &MarketConfig, oracle_id: ID): (u64, u64, u64) {
    assert!(market_config.market_grids.contains(oracle_id), EMarketConfigNotFound);
    let grid = market_config.market_grids.borrow(oracle_id);
...
fun resolve_basis_bounds(
    ...
    if (market_config.asset_basis_bounds.contains(underlying_asset)) {
        let b = market_config.asset_basis_bounds[underlying_asset];
