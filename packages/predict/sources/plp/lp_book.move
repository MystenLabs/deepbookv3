...
/// supplies first then withdrawals, processing at most `max_requests_per_flush`
/// live requests total. Cancelled requests are removed at cancel time and never
/// spend flush capacity.
public(package) fun drain<LP>(
    ...
    while (processed < max && !book.supply_queue.is_empty()) {
    ...
    while (processed < max && !book.withdraw_queue.is_empty()) {
...
/// LP shares minted for `amount` DUSDC at the frozen flush mark.
