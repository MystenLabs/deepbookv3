...
///
/// `store` exists for exactly one caller: the full-pool flush freezes one `Pricer`
/// per market in `plp::PoolValuation` so every market is marked at a single instant
/// even though valuation spans transactions. Every other pricing path must load a
/// fresh `Pricer` in its own transaction — a stored one is a *deliberately* stale
/// mark, sound only inside the flush's frozen snapshot, where the valuation lock
/// holds the books still.
