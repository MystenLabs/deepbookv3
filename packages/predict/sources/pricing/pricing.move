...
/// under finite +/-5 bp forward shifts. Dividing by the stored vendor-calibrated
/// reference keeps every tenor on one scale while the finite smile-consistent
/// digital evaluations preserve the intended short-end saturation.
public(package) fun loading(
    pricer: &Pricer,
    lower: Strike,
    higher: Strike,
    probability: u64,
    reference_sensitivity: u64,
): u64 {
    ...
    math::div_down(sensitivity, reference_sensitivity)
}
