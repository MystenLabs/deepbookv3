...
/// Return aggregate DUSDC received from an expiry market into this manager.
...
public(package) fun new_self_owned(
    ...
    let predict_trade_cap = manager.mint_initial_trade_cap(manager_id, ctx);
    let predict_deposit_cap = manager.mint_initial_deposit_cap(manager_id, ctx);
    let predict_withdraw_cap = manager.mint_initial_withdraw_cap(manager_id, ctx);
...
/// Initial cap mint used inside `new_self_owned` (no owner check; the
/// constructor is the only place that can call this).
fun mint_initial_trade_cap(
    self: &mut PredictManager,
    manager_id: ID,
    ctx: &mut TxContext,
): PredictTradeCap {
    ...
    let cap_id = id.to_inner();
    self.allow_listed.insert(cap_id);
    event::emit(PredictTradeCapMinted { predict_manager_id: manager_id, cap_id });
    PredictTradeCap { id, predict_manager_id: manager_id }
}

fun mint_initial_deposit_cap(
    self: &mut PredictManager,
    manager_id: ID,
    ctx: &mut TxContext,
): PredictDepositCap {
    ...
    let cap_id = id.to_inner();
    self.allow_listed.insert(cap_id);
    event::emit(PredictDepositCapMinted { predict_manager_id: manager_id, cap_id });
    PredictDepositCap { id, predict_manager_id: manager_id }
}

fun mint_initial_withdraw_cap(
    self: &mut PredictManager,
    manager_id: ID,
    ctx: &mut TxContext,
): PredictWithdrawCap {
    ...
    let cap_id = id.to_inner();
    self.allow_listed.insert(cap_id);
    event::emit(PredictWithdrawCapMinted { predict_manager_id: manager_id, cap_id });
