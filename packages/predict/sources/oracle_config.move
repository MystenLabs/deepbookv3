...
public(package) fun assert_live_oracle(
    ...
    assert!(now <= oracle.spot_timestamp_ms() + oracle_config.staleness_threshold_ms, EOracleStale);
    assert!(
        now <= oracle.basis_timestamp_ms() + oracle_config.basis_staleness_threshold_ms,
        EOracleStale,
    );
}
