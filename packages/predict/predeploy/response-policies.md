...
- **Pinning tests:** `settlement_flow_tests.move` — `explicitly_settled_redeem_pays_terminal_payout` (account-auth settled redeem pays the terminal payout and leaves exact market cash), `settled_redeem_requires_explicit_settlement`, `settled_redeem_twice_aborts`, and `explicit_settlement_then_standalone_rebalance_sweeps_market` (the settled sweep completes without any position being redeemed); `expiry_market_gate_tests.move` — `redeem_settled_on_live_order_aborts`.
