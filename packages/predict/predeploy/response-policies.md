...
## RP-29: Retired — the inventory charge cannot block a close (DBU-732)

- **Retired** by the move to a re-anchored, charge-only inventory measure. The policy this entry recorded — a close whose inventory charge exceeded everything it released aborted, with the escrow senior to fee revenue — existed because the charge funded a rebate escrow, so collecting less than the measure moved would have left the escrow short of a later claim.
- **What replaced it:** there is no rebate and no escrow, so the charge is ordinary revenue. It is now **junior to every ordinary deduction** and clamped to whatever the payout still holds after fee, builder fee and penalty, so a close can always pay its fee and the charge can never block an exit. Collecting less than the measure moved costs the pool a little steering signal and breaks no invariant.
- **Pinning tests:** `inventory_charge_flow_tests.move` — `a_close_that_unbalances_the_book_is_charged_from_its_payout` (the close pays out of its payout, after the ordinary fee).
- **Reopen when:** a rebate returns. A two-sided transfer needs the escrow back, and with it the seniority ordering and the abort this entry described.
...
- **Pinning tests:** `inventory_charge_flow_tests.move` — the full custody suite runs with the inventory charge as the only inventory mechanism; `cash_backing_flow_tests.move` — `cash_sheet_exact_after_every_flow`.
