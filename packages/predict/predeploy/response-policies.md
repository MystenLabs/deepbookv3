...
> **RETIRED 2026-08-18 — the trading-loss rebate and DEEP staking were removed.** The trigger state (a settled market with unresolved rebates priced at claim-time `active_stake`) is unreachable: there is no rebate reserve, no `ExpiryTradingSummary`, and no claim. P-9 stays resolved — this entry remains its tombstone. ⚠ 2026-08-26: the settled-market cleanout no longer survives in any form; `redeem_settled_permissionless` was deleted (RP-29), so the gas-incentive measurements below describe no shipped code path and the re-measurement this entry's reopen clause owed is superseded rather than satisfied. The entry is kept verbatim for the decision record; nothing in it describes shipped behavior. Removal record: `docs/design/decisions.md` § "Staking and the trading-loss rebate removal".
...
- **Response:** accept — (a) the rebate is priced at claim-time active stake, and (b) resolution +
  ...
  is paid the storage rebate to run it) rather than on any protocol-run keeper. ⚠ Both entrypoints
  are now deleted; neither describes shipped behavior. No contract change;
...
- **Pinning tests:** untested — the entry is retired and governs no live code path. Its four
  ...
  exercised. ⚠ 2026-08-26: the two settled-redeem pins named here,
  `deauthorized_predict_app_blocks_permissionless_settled_redeem` and
  `owner_auth_settled_redeem_survives_predict_app_deauth`, were deleted with the
  entrypoint (RP-29); the gate they covered no longer exists in Predict. The claim-time-stake pricing was never
...
## RP-29: The permissionless settled redeem, deleted — duty inventory (2026-08-26)

- **Trigger state:** not a runtime state. `move.md` requires that deleting or weakening a guard is preceded by an inventory of what it *incidentally* bounded, recorded here rather than only in a commit message. Removing `expiry_market::redeem_settled_permissionless` deletes an entrypoint rather than an assert, so the inventory covers what the entrypoint's *existence* incidentally provided.
- **Controller:** protocol (the entrypoint existed at all) × external (any searcher could call it). After removal, only the account's own authority can close a settled position.
- **Blast radius:** per account. A holder who never redeems keeps one `positions` row and, if they won, their payout backing inside the expiry. No shared path degrades.
- **What the entrypoint incidentally provided, and where each duty now lands.**
- **A reaper for the account's `positions` table.** Stated purpose: let a keeper sweep settled positions. Incidental duty: `predict_account::remove_position` has exactly two callers, both redeem paths, so this was the only way an account's settled rows could be cleared by anyone other than the holder. It bounded nothing arithmetically — the table is read only by key (`contains`, index), never iterated or counted, and no flow asserts it is empty (the one that did, `EExpirySummaryHasOpenPositions`, died with the rebate — RP-28). Duty now lands on the holder, who is paid the storage rebate for clearing their own rows. *Residual:* unbounded per-account growth for holders who never redeem, an RPC/UX cost with no on-chain consumer.
- **A path to release an expiry's settled payout liability without the holder.** Stated purpose: return reserved cash to the pool. Incidental duty: **none — this was never true.** `required_cash = payout_liability + inventory_impact_reserve` and `process_settled_close` decrements the liability by exactly the payout paid, from the same integer atoms, so `balance - required_cash` is invariant under a settled redeem. The pool-claimable free cash a settled market can return is identical whether or not anyone redeems; `sweep_settled_expiry` deactivates the expiry and materializes profit on its first pass regardless. The reserved amount is owed to the holder, not to LPs, and `pool_accounting` already latches it as realized loss at that first sweep.
- **The only consumer of `predict_account::generate_auth_as_app`.** Stated purpose: derive Predict app-auth for the keeper path. Incidental duty: it was the sole call into `account_registry`'s app-authorization gate from Predict, which made the account admin's authorization hygiene part of Predict's solvency story. ⚠ It was never a per-account opt-out: `authorize_app`/`deauthorize_app` are global and `AccountAdminCap`-gated (`account_registry.move:113`), so no holder could switch this off for their own account — the storage deposit was theirs to lose with no way to decline. Both properties are removed rather than re-homed: the helper is deleted with the entrypoint, so Predict holds no package-level authority to mutably load any account. `PredictApp` survives as the data-slot witness only — `account::attach`/`borrow_data_mut` take a `Permit<PredictApp>` with no registry check — so the registry whitelist is now a no-op for Predict and the fixture step that set it is gone.
- **A measurable subject for the cleanout gas incentive.** Incidental duty: RP-11's reopen clause owed a re-measurement of the survivor cleanout under the derived-state model. That measurement can no longer be taken against a keeper path. The harness instrument is retargeted rather than deleted — `cleanupEconomics` and `devtools/ts/runtime.ts::cleanoutAccountTx` now price the owner clearing their own settled book, which is the incentive that actually governs whether the rows get reclaimed. A retargeted run is not comparable to the recorded P-9 numbers for three reasons: the old PTB also carried `claim_trading_loss_rebate_permissionless`, DBU-592 moved the liquidated order's book-storage release out of the cleanout, and the owner path adds one `account::generate_auth` command per position. Only the third is new here, and it is small — recorded per-position compute is ~0.05-0.1M MIST against a ~3.29M rebate, so it cannot flip the sign.
- **Response:** delete the entrypoint and its helper; no replacement guard is required. The one behavior change worth stating plainly: a settled position is now cleared only by its holder's authority, so the storage deposit the holder paid at mint is refunded to the holder rather than to whoever sent the transaction first. This is the intended change.
- **Risk profile:** `BEST-GUESS` — no runtime state is involved. The cash-invariance claim is verified against source (`expiry_cash::required_cash`, `strike_exposure::process_settled_close`, `expiry_market::release_settled_pool_cash`) and pinned by the settled-sweep tests below; the no-consumer claim is a grep of the contracts, off-chain services, infra, and app repos at HEAD.
- **Pinning tests:** `settlement_flow_tests.move` — `explicitly_settled_redeem_pays_terminal_payout` (account-auth settled redeem pays the terminal payout and leaves exact market cash), `settled_redeem_requires_explicit_settlement`, `settled_redeem_twice_aborts`, `settled_sweep_retains_payout_backing_for_an_unredeemed_winner` (the settled sweep deactivates the expiry and returns everything above payout liability while a winner is still open, and the holder is paid in full afterwards), and `explicit_settlement_then_standalone_rebalance_sweeps_market` (the same sweep against a market that never minted); `expiry_market_gate_tests.move` — `redeem_settled_on_live_order_aborts`.
- **Reopen when:** a protocol-run keeper or an integrator needs to clear positions on a holder's behalf (re-add as an owner-opt-in delegation, not ambient app-auth); OR unredeemed settled rows are observed to degrade a real consumer rather than only open-interest aggregates; OR the package moves from fresh publishes to compatible upgrades, after which the entrypoint cannot be re-added under the same name with a different signature.
