...
- **Trigger state:** a live close whose inventory-skew charge is larger than everything the close releases — `redeem_amount` plus the skew rebate. Reaching it needs a book that is flat before the close (so closing unbalances it and is charged rather than rebated) and a leg priced far enough out of the money that its payout is worth less than the charge.
- **Controller:** market — the payout is the oracle-priced range value and the charge is derived from the book's shape under the frozen measure; neither is caller-controlled.
- **Blast radius:** the caller's own close, on one position — but reachable by a third party, not only by the holder's own trading. The charge is a function of global book state, so an actor who shapes the book between the holder's quote and execution can flip a close from rebated to charged and push it past the bound. The stranded value is near zero by construction and settles in full at expiry, so the harm is denial of an early exit rather than loss. No protocol-side effect.
- **Response:** `abort` (`EInventoryChargeExceedsCloseProceeds`), the single-user user-recoverable rung. The position settles at expiry for what it is worth, which in the reachable regime is near zero; what the caller loses is the early exit.
- **Ordering:** the escrow backs future rebates rather than being revenue, so it is **senior to the trading fee**. The charge comes out of the payout first and the fee is clamped against what remains — `min(redeem_amount, gross_proceeds - inventory_charge)` — so the cap binds only when the payout alone cannot cover both. A close whose rebate absorbs the charge pays the fee it would have paid without skew, and no fee revenue is diverted to the closer.
- **Why not clamp the charge:** a clamp would move the potential without collecting it, leaving the escrow below what a future rebate can claim. That converts a bounded single-user abort into an abort on someone else's trade, which is strictly worse on the blast-radius ladder.
- **Mint side:** the mirror code `EInventoryRebateExceedsMintCost` is unreachable at any admissible rate. A rebate is bounded by `rate * quantity / 2` and the premium alone is at least `min_entry_probability * quantity`; both sides are upgrade-required constants, pinned by `protocol_config_bounds_tests::max_skew_rebate_stays_below_the_minimum_premium`.
- **Pinning tests:** `inventory_skew_flow_tests.move` — `a_close_that_unbalances_the_book_is_charged_and_the_drain_refunds_it` (the close-side charge collects through the clamp chain and the drain refunds it exactly), `a_round_trip_refunds_the_charge_exactly_across_an_oracle_move` (the frozen measure makes the refund exact), `completing_a_set_rebates_the_first_legs_charge_exactly` (mint-side rebate bounded by the escrow); `protocol_config_bounds_tests.move` — `max_skew_rebate_stays_below_the_minimum_premium`, `a_skew_rate_above_twice_the_fee_floor_cannot_be_set` and `a_fee_floor_below_half_the_skew_rate_cannot_be_set` (the relation is enforced at both template setters, so the creation-time snapshot assert is an unreachable tripwire). The abort itself is pinned by `close_of_a_worthless_leg_that_unbalances_the_book_aborts`: a complete set plus a spot far below the range makes the closed leg worthless, so its charge exceeds everything the close releases.
- **Windowless note:** the windowed variant of this mechanism carried a second stranding class — a market whose reference tick never lands is permanently unmintable behind its window gate. The probability-weighted form has no window and no gate: the measure freezes from the first mint's own validated pricer, atomically with that mint, so no ordering constraint exists to strand behind.

---

## RP-30: Guards deleted with the capital-occupancy charge — duty inventory (DBU-732)

- **Change:** the inventory-impact (capital-occupancy) mechanism from the original occupancy PR is removed in favor of the probability-weighted inventory-skew charge: the potential `phi(L)`, the `inventory_impact_scale` snapshot, the charge/rebate terms and quote fields, the isolated `inventory_impact_reserve`, its config rate and template setter, the event fields, and the payout tree's `complement_max_payout` read that existed only for it. The two mechanisms priced different objects (gross cash occupancy vs payout-profile shape); the removal is a product decision to run one inventory charge, not a claim of redundancy.
- **`expiry_cash::EInventoryImpactRebateExceedsReserve`.** Stated purpose: an impact rebate spends only its own escrow. Incidental duty: none beyond its escrow — the skew escrow carries the identical guard (`EInventoryRebateExceedsReserve`, renumbered to 1) for the surviving mechanism, and ordinary cash was never reachable by either rebate.
- **`strike_exposure::EInvalidInventoryImpactScale`.** Stated purpose: a market cannot be created with a zero impact scale. Incidental duty: it was the only positivity check on the snapshotted `max_expiry_allocation` reaching the exposure. That value no longer reaches the exposure at all; pool accounting's own funding caps still consume it, and a zero allocation there yields a market the pool never funds rather than an arithmetic hazard.
- **`config_constants::EInvalidInventoryImpactMaxRate`.** Stated purpose: bound the admin template rate. Incidental duty: none — pure range check on a deleted field; its admin entrypoint is deleted in the same change, so no setter is left unbounded. Survivor codes renumbered (`EInvalidReferralFeeRate` 22→21, `EInvalidInventorySkewRate` 23→22) per the pre-deploy no-gaps rule.
- **The bare backing assert `inventory_impact_reserve >= inventory_impact_potential`.** Stated purpose: the impact escrow covers its potential. Incidental duty: none independent — it was one conjunct of `assert_cash_backing`; the cash-covers-liability half and the skew half survive unchanged, and `required_cash`/`free_cash` now fold the skew reserve alone.
- **Retained:** `strike_payout_tree::range_max_payout` loses its production callers with the potential but stays: it is the only rotation-level exerciser of the max-prefix aggregate that `payout_reserve_terms` reads at the root for cash backing, and the reopen condition below would need it back verbatim. Package-only surface, so retention carries no deploy commitment.
- **Response:** every guard is removed with the state it guarded or carried by the surviving skew twin. The behavior change worth stating plainly: closes no longer receive an occupancy rebate, so `gross_proceeds = redeem_amount + inventory_rebate` on the close path, and a market's cash floor is `payout_liability + inventory_reserve`.
- **Risk profile:** `BEST-GUESS` — static reachability of deleted expressions verified by grep at HEAD plus the full suite; no runtime state involved.
- **Pinning tests:** `expiry_cash_tests.move` — `skew_rebate_cannot_spend_ordinary_cash`, `skew_reserve_folds_into_required_and_out_of_free_cash`, `settlement_release_turns_residual_escrow_into_surplus` (rewritten to the skew escrow); `inventory_skew_flow_tests.move` — the full custody suite now runs with skew as the only inventory mechanism; `cash_backing_flow_tests.move` — `cash_sheet_exact_after_every_flow`.
- **Reopen when:** a capital-occupancy or utilization charge returns — the escrow, its rebate guard, and the backing conjunct come back together, and the close path must re-derive its `gross_proceeds` composition.
