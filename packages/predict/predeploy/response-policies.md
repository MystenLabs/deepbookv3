...
- **Risk profile:** `MEASURED` for the compute slope at the old store (`evidence/c1-price-memo-2026-07-01.md`); `UNMEASURED` for the inline-vector walk — P-30's remeasure prices it before any cap raise.
- **Pinning tests:** `valuation_capacity_tests.move` — `the_node_cap_stays_usable`.
- **Reopen when:** P-30's remeasure lands (size the cap from it), `value_expiry` gains a child-loading step, or the store changes again.
