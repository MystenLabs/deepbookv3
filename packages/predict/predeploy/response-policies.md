...
- **Blast radius:** the mark is assembled by the staged flush (RP-29) and every
  LP fill in `finish_flush` prices at it. The mark is exact as of the flush's
  snapshot instant; the drain executes later in the same flush, so fills carry a
  staleness bounded by the operator's cadence and `max_valuation_window_ms`
  (RP-29 ratifies that bound).
...
- **Blast radius:** the flush's atomic snapshot stage aborts (and reverts whole, leaving no flag engaged) while a market is already expired and unsettled at its instant, because no substitute NAV mark exists for a settlement-dependent market; a market expiring only after the snapshot is valued at its frozen pre-expiry mark instead (RP-29).
...
- **Exact Block Scholes production:** `block_scholes_store::apply_spot_batch` independently inserts a positive `u64`-representable signed spot into permanent exact history whenever `source_timestamp_ms` is a whole-minute boundary, even when that observation does not advance `latest`. The dedicated `insert_at` performs the same first-admissible-writer exact insertion without changing `latest`; non-minute, zero, and over-wide observations are no-ops for exact history. Market expiries are whole-minute aligned, so the fallback asks for a representable exact key.
...
- **Reopen when:** the grace period changes, expiries cease to be minute-aligned, either source changes exact-history semantics, Block Scholes source quality is no longer accepted for terminal settlement, or a valuation-safe representation for expired-unsettled markets removes the need to block the flush. (Partly met: the per-market SETTLEMENT gate was removed — the frozen mark is settlement-invariant, so a market settles the instant it expires even mid-flush; the snapshot-stage refusal to STAMP an already expired-unsettled market is deliberately retained, so "settle first" stays the resolution there.)
...
- **Reasoning:** both changes only ever convert an abort into a priced result, on
  a path that is mandatory (the flush must price every active market to drain the
  LP queues; since RP-29 it does so one market per transaction). Per
...
## RP-29: The flush is resumable with trading live — the mark is the snapshot instant, reconstructed exactly (resolves C-1)

- **Trigger state:** any full-pool flush: the joint object budget C-1 measured (every active market's payout-tree children summed in one PTB against Sui's 1,000-cached-object ceiling), and every state a multi-transaction flush window can produce — trades, oracle movement, an expiry, LP queue traffic, or a keeper death between the snapshot and the drain.
- **Controller:** market (book growth and trading are permissionless; the ceiling is a network constant) and operator (window length, keeper liveness).
- **Blast radius:** the flush itself and the LP fills it prices. Trading is never gated during the resumable stage — mint and close stay live on every market from the seal to the finish; the one refusal is a trade composed INTO the atomic snapshot PTB (see (h)), which only the flush-starter can build.
- **Response:** the flush runs in three stages. One atomic PTB (`SnapshotStage` hot potato, so atomicity is type-enforced) records the active set, the start time, each LP queue's `next_index` cutoff, and the per-queue drain budgets, freezes one `Pricer` per live market and stamps those markets, sweeps settled members (their recoverable cash lands in idle before the capture), and at the seal freezes the vault-side figures — idle, the profit-basis credits and debits, the pending protocol cut. Valuation then spans transactions, one market per transaction: the stamp carries the two cash rows NAV reads, copied eagerly at the snapshot instant, and the payout tree holds its own snapshot — each node captures its boundary quantities lazily before its first mutation under the flush's generation (an emptied node is retained as a live-zero husk until the walk consumes it) — so trades record nothing, and `value_expiry` reads the market's SNAPSHOT-INSTANT NAV directly: the SAME linear walk as the live read, over the captured terms (identical per-node rounding and monotonicity observation by construction), against the stamp's captured cash — then clears the stamp, releasing the tree snapshot and its husks. `finish_flush` proves completeness and drains each queue only below its cutoff. `value_expiry` is MEASUREMENT-ONLY for every member and moves no cash, and cash maintenance (`rebalance_expiry_cash`) stays fully available post-seal — every term the mark reads (frozen vault figures, stamped cash, tree shadows) was captured at the snapshot instant, so no in-window move can reach it and nothing is corrected for. Post-valuation trades are invisible to the recorded figure, which is the same as-of-snapshot semantics. The mark every fill prices at is therefore exactly the pool NAV at one instant, with concurrent trading provably cancelled out.
- **What the multi-transaction window changes, ratified here:** (a) the mark is exact as of the SNAPSHOT instant and the drain executes later — fills carry that staleness, bounded by the operator's cadence and by `max_valuation_window_ms` (RP-1 notes it); (b) a market whose expiry passes mid-window is valued at its frozen pre-expiry mark — well-defined because it was live at the instant the flush prices, and settlement is not gated by the flush — a mid-window expiry settles immediately, before or after its `value_expiry`, and the frozen mark is invariant to it (`value_expiry`/`snapshot_nav` read only the frozen stamp/pricer/shadow that settlement never mutates); the snapshot stage still refuses a market already expired-unsettled at its instant, and that abort reverts the atomic stage so the flag is never stranded (RP-4's blast radius); (c) LP request CANCELS are gated during a flush — the frozen mark is on-chain readable mid-window, so an ungated cancel of an eligible request is a free look at a stale price; new requests stay ungated because the cutoff quarantines them (the supply-cancel gate is pinned closed by `cancelling_a_request_during_a_flush_aborts`; `cancel_withdraw_request` takes the identical `assert_not_valuation_in_progress` + `assert_no_pending_valuation` pair and is not separately unit-drivable because a filled supply's PLP never reaches account custody in the test accumulator — the documented `lp_flow_tests` limitation); (d) a stalled keeper costs LP-fill latency and the flush set's settlement latency, never a trading pause. There is no abort entrypoint: `value_expiry` and `finish_flush` are permissionless once the snapshot seals — the frozen mark and the per-queue drain budgets are both committed at `start_pool_valuation`, so a stranger can neither reprice a fill nor grief by finishing with a zero budget (finish takes no budget), and `value_expiry` is idempotent so a stranger racing the keeper is a no-op. Recovery from a stall is a fresh `start_pool_valuation`, which discards the in-flight flush and re-snapshots (folding stop into start); `finish_flush` refuses a flush older than `max_valuation_window_ms` for everyone including the operator, and the discard visits no markets (stamps go stale by flush ordinal and are lazily discarded); (e) mid-window trading on a stamped market is UNBUDGETED — snapshot capture is per-node copy-on-write with no per-trade record, so no per-trade log, cap, or compute budget exists and the retired per-market delta log with its `max_valuation_log_ops` cap is superseded (`mid_window_trading_has_no_budget` pins the absence of the wall). One bounded residual at the node-cap edge: a boundary emptied mid-window is retained as a husk that still counts toward RP-30's admission cap, so near the cap a close-then-fresh-tick-mint sequence can hit `EMaxPayoutTreeNodes` where the same sequence outside a window would not — per-market, worth at most the husk count, self-healing at that market's next consumed valuation, and never blocking closes or same-tick revivals. (f) A mid-window top-up spends REAL idle while the mark prices snapshot idle, so an eligible withdrawal larger than the idle that physically remains fills partially at the same price and carries (RP-23's mechanics, unchanged) — a liquidity-timing shift, never a mispricing or an abort; the carry machinery is unit-pinned in `lp_book_tests` (idle-bounded partial fills) and the mark's invariance by the maintenance pins here, while the end-to-end conjunction is not behaviourally drivable in unit fixtures (the test accumulator root carries no funds — the documented `lp_flow_tests` limitation). (g) The standalone settled sweep runs freely mid-window — a market valued as live this flush that expires and settles mid-window lands its cash in real idle, and the mark cannot see it because idle was frozen at the seal (`a_market_expiring_mid_flush_values_at_the_frozen_mark_then_settles` pins the sweep landing and the preserved mark); frozen-settled members are swept inside the snapshot stage itself, before the capture. (h) Inside the still-open snapshot PTB (start → seal, one transaction, composable only by the starter's own PTB) `rebalance_expiry_cash` is structurally refused (`ESnapshotStageOpen`, pinned by `a_rebalance_inside_the_open_snapshot_stage_aborts`): a cross-move there could land between a market's stamp and the seal's vault capture and the two frozen figures would disagree about where that cash was at the instant. The same open-stage refusal covers the trade paths: `mint`/`redeem` abort `ESnapshotInProgress` (a config flag set at `start_pool_valuation`, cleared at `seal_valuation_snapshot`) so the keeper cannot compose a trade into its own snapshot PTB, where a mid-stamp cash move would skew the seal's figures — pinned by `a_mint_composed_into_the_open_snapshot_stage_aborts` and `a_redeem_composed_into_the_open_snapshot_stage_aborts`; other traders cannot reach this state because they cannot interleave within one PTB. Refusal replaces the retired per-market maintenance-ordering record — there is no recording machinery left to get the ordering wrong — and it also closes the old residual (a market created and funded inside the snapshot PTB is now refused at the funding call rather than silently understating the mark). (i) `lp_pool_value` is a pure function of frozen inputs with one policy clamp: the sticky realized `credits - debits` term can leave the held-out total exceeding gross after LPs withdraw against a mark that later collapses, so the mark floors at zero rather than underflow-aborting the finish; a 0/dust mark makes non-executable queue heads refund inside `lp_book::drain`. Of the four frozen vault figures, idle, the profit-basis credits, and the profit-basis debits are drift-pinned by the mid-window surplus-sweep and top-up metamorphic tests (a finish reading any of them live would move the mark); the pending-protocol-cut term is verified by inspection (`finish_flush` reads `frozen_pending_protocol_profit`, never the live figure) rather than metamorphically pinned, because draining the carried cut mid-window needs a live surplus sweep at a post-expiry clock, which the deferral fixture cannot live-price without oracle-refresh plumbing it does not carry. (j) SETTLEMENT IS NOT GATED BY THE FLUSH — the per-market `EMarketPendingValuation` gate on `try_settle` is removed, so a market settles the instant it reaches expiry even mid-window and even before its own `value_expiry`; the frozen mark is settlement-invariant. Duty inventory for that removed guard (the guard made "settled AND carrying a current-flush stamp" unreachable; every consumer of that now-reachable state was enumerated before removal): `snapshot_nav` reads only the stamp's frozen cash rows, the frozen pricer, and the frozen shadow — never live cash, never the settled liability; `EMarketNotPendingValuation` stays structurally unreachable because only `value_expiry` clears a current stamp and `reconcile_stale_valuation_stamp` clears only non-current ones; `walk_linear_frozen`'s `EStaleValuationSnapshot` is untouched because no settlement path deactivates the tree snapshot; `record_settlement` and `process_settled_close` never call `insert_range`/`remove_range`, so the frozen shadow survives settlement and settled redemption; `release_settled_pool_cash` cannot underflow (its backing assert precedes the subtraction and the impact escrow is already zeroed); `assert_cash_backing`'s escrow clause cannot abort because `inventory_impact_potential` early-returns zero once settled; `materialize_expiry_profit` mutates only live figures the finish never reads; and `deactivate_expiry_if_present` cannot strand the flush because `expected_expiry_markets` is a copy taken at start and `registered_expiries` is permanent, so a swept-and-deactivated member is still valuable. Cash is counted exactly once: `frozen_idle_balance` is captured at the seal, strictly before any post-seal sweep. Accepted residue: a market settled and swept mid-window whose flush is then superseded before its `value_expiry` keeps an inert stale stamp and its retained husks until any later `try_settle` or trade reconciles it — nothing walks them and `is_pending_valuation` already reports false. Operational note: `active_expiry_markets` is no longer a safe proxy for a flush's expected set mid-window, because a member can now be deactivated before it is valued — a tool must reuse the market list it snapshotted rather than re-reading the active set, or it will skip an unvalued member and wedge `finish_flush` on `EMissingExpiryValuation` until someone calls the permissionless `value_expiry`. What is NOT changed: settlement is still blocked by oracle unavailability (both exact sources absent), and the snapshot stage still refuses an already expired-unsettled market, so RP-4's cross-market LP-liveness brick stands.
- **Risk profile:** `MEASURED` for the wall this removes (`evidence/c1-object-cache-flush-2026-07-07.md`: one market aborts at ~982 nodes, two at 586 each abort combined — the joint budget is gone by construction, and the per-market case is bounded by RP-30's derived cap); `BEST-GUESS` for the window economics (fill staleness is bounded by `max_valuation_window_ms`, not measured; the valuation transaction prices at most the node cap's boundaries — the retired delta log's extra compute term is gone, closing C-5 by construction).
- **Pinning tests:** `valuation_corrections_tests.move` — `a_mint_between_snapshot_and_valuation_leaves_the_mark_unchanged`, `an_oracle_move_plus_a_trade_mid_window_leave_the_mark_unchanged`, `a_full_close_deleting_boundaries_mid_window_leaves_the_mark_unchanged`, `a_trade_after_a_markets_valuation_is_invisible_to_the_flush`, `a_request_submitted_after_the_snapshot_waits_for_the_next_flush`, `cancelling_a_request_during_a_flush_aborts`, `settling_mid_flush_before_value_expiry_leaves_the_mark_unchanged`, `settling_and_sweeping_mid_flush_before_value_expiry_leaves_the_mark_unchanged`, `settling_and_redeeming_mid_flush_before_value_expiry_leaves_the_mark_unchanged`, `a_market_expiring_mid_flush_values_at_the_frozen_mark_then_settles`, `a_superseded_flushs_snapshot_never_leaks_into_the_next_flush`, `mid_window_trading_has_no_budget`, `value_expiry_moves_no_cash_for_a_live_market`, `a_mid_window_surplus_sweep_leaves_the_mark_unchanged`, `a_mid_window_top_up_leaves_the_mark_unchanged`, `maintenance_after_a_markets_valuation_leaves_the_mark_unchanged`; `pool_valuation_flow_tests.move` — `multi_market_pool_nav_is_exact_with_a_mid_flush_rebalance`, `a_market_created_and_funded_mid_flush_leaves_the_mark_unchanged`, `valuation_split_across_transactions_marks_at_the_snapshot_instant`, `a_member_deactivated_mid_flush_still_folds_its_frozen_mark`, `finish_aborts_when_a_member_deactivated_mid_flush_is_unvalued`.
- **Reopen when:** the object-cache limit stops being network-invariant, a flow outside mint/close mutates a market's NAV-visible rows at trade time (the copy-on-write capture must precede it), or LP fills need fill-instant marks (which reintroduces the pause-vs-staleness tradeoff this entry settles).

## RP-30: The per-market node cap is derived from the transaction object budget

- **Trigger state:** a mint whose new strike boundaries would push one market's payout tree past what a single `value_expiry` transaction can load.
- **Controller:** market (boundary-creating mints are permissionless) — so the response is an admission bound, not an assert in a shared path.
- **Blast radius:** without the cap, one market's tree could exceed the object budget and its valuation transaction could never land, stalling every flush at the completeness proof until that market expires — a pool-wide LP-fill stall from one market's growth.
- **Response:** `max_payout_tree_nodes` is derived, not chosen: `object_cache_budget (1,000) − valuation_base_children_reserve (40) = 960`, checked at mint admission (`strike_payout_tree::insert_range`). Deriving ties the cap to the budget it must fit inside, so the two cannot drift. Precondition carried in the constant's doc: one `value_expiry` per transaction, never batched with another market's or with `finish_flush`. Snapshot husks (boundaries emptied mid-window and retained for the frozen walk) are ordinary tree nodes already inside this cap — retention converts a live node, never adds one — and every husk is removed by the next `value_expiry` to consume a snapshot on that market (its own flush's normally; a later flush's for husks stranded by a superseded flush), so retention is bounded by that market's next consumed valuation and the cap's meaning is unchanged by the snapshot mechanism. RP-29(e) carries the admission residual this creates at the cap edge.
- **Risk profile:** `MEASURED` for the ceiling itself (`evidence/c1-object-cache-flush-2026-07-07.md`); the 40-child reserve is `UNMEASURED` and generous — source inspection puts the real figure at 1-2 (C-2 tightens it).
- **Deferred, tracked (husk-cleanup compute):** the cap is derived from the OBJECT-cache budget, not from `value_expiry`'s COMPUTATION cost. Its dominant compute term is the frozen walk (O(nodes), the same walk the single-market compute memo `evidence/c1-price-memo-2026-07-01.md` sizes). `release_snapshot`'s husk cleanup adds an O(husks · log n) term (collect + a rebalancing `detach_tick` per husk) that is UNMEASURED at the cap edge. It is left unmeasured deliberately: husks form only when positions close in the gap between the snapshot transaction and that market's `value_expiry`, which the keeper submits back-to-back, so the organic husk count per window is a handful; the ~960-husk worst case is reachable only by landing ~960 closes in that seconds-long gap after pre-opening ~960 positions, and its consequence is bounded — one `value_expiry` rolls back, the operator starts a fresh flush (which discards the stalled one and re-snapshots), and the husks are cleaned by that later flush (RP-29(e)/(g)), never a fund loss. If the cleanup ever exceeds the compute cap it is a recoverable liveness hiccup, not a stall a fresh start cannot clear. Measure it (a split-flush localnet run: snapshot leg, ~960 closes, then the `value_expiry` leg's computation) before raising the cap, before shortening the keeper's inter-leg cadence enough to widen the husk window, or if organic husk counts are ever observed in the hundreds; if it does not fit, switch `release_snapshot` to an O(n) rebuild or lower the cap.
- **Pinning tests:** `valuation_capacity_tests.move` — `the_node_cap_stays_usable`.
- **Reopen when:** `object_runtime_max_num_cached_objects` changes, `value_expiry` gains a child-loading step, C-2's measurement moves the reserve, or the husk-cleanup compute measurement above is triggered.

## RP-31: A market outside the flush's active set is skipped, not fatal

- **Trigger state:** the keeper's off-chain market list contains a market the on-chain active set no longer holds — the routine race where a settled market is swept (permissionless `rebalance_expiry_cash`) between the list read and the flush transactions.
- **Controller:** market/keeper timing — permissionless sweeps race any off-chain read.
- **Blast radius:** on the old shape this aborted the whole flush (`EExpiryMarketNotActive`), observed recurring in production (`evidence/rp31-flush-active-set-race-2026-07-30.md`); every queued LP fill waited on a keeper retry.
- **Response:** `snapshot_expiry_pricer` and `value_expiry` silently return for a market not in `expected_expiry_markets`. The caller's list is only a hint about which commands to emit; the completeness proof is untouched — sealing requires a frozen entry for every EXPECTED market and finishing requires every expected market valued, so a market that genuinely belongs to the flush still cannot be skipped. The reverse direction (a market the set holds that the keeper omits) still aborts, at the seal or the finish, deliberately.
- **Risk profile:** `MEASURED` — the failing shape is the on-chain observation in `evidence/rp31-flush-active-set-race-2026-07-30.md`.
- **Pinning tests:** `pool_valuation_flow_tests.move` — `a_swept_market_left_in_the_keepers_list_is_skipped_not_fatal`, `a_stale_market_alongside_a_live_one_is_skipped_and_the_live_one_still_values`.
- **Reopen when:** the active set stops being recorded on-chain at the start (the skip is only safe because membership is read on-chain, never supplied).
...
## RP-36: All-in budget sizing searches the charged total; the fit is exact to one lot (DBU-834)
...
- **Controller:** user for the budget; market for the surface, the fee-incentive balance, the EWMA state, and the book the impact charge is read against.
- **Blast radius:** the single mint transaction or quote; no shared or mandatory path.
- **Response:** proceed — `mint_exact_cost` makes the total the sizing input. A binary search over lot counts evaluates `mint_quote_at`, the same sum the mint charges, against execution-time state, so the fill is the largest lot-rounded quantity whose all-in cost fits. Unless sizing saturates at the lot cap (`order::max_quantity_lots`, where the domain binds rather than the budget — RP-13 carries the same caveat for the premium shape), the unspent remainder is below one more lot's all-in cost.
- **Why the fit is exact:** every all-in term is nondecreasing in quantity while pre-trade state is fixed. Premium and each fee leg are `mul_down` of a quantity-independent rate; the trader-paid fee is `fee - min(mul_down(fee, fee_incentive_subsidy_rate), incentives)`, whose subsidy gains at most one unit per fee unit because that rate is an upgrade-required 0.2; the builder fee is a `min` of nondecreasing terms; the surcharge's firing condition is quantity-independent; and the impact charge is monotone because the prospective liability `max(M, R + q) + lambda * (T + q - that)` rises at `lambda` while `R + q <= M`, rises at 1 once the candidate carries the max itself with the gap `T - R` frozen, and both arms evaluate to `M + lambda * (T - R)` exactly at the switch `q = M - R`. The arms therefore meet where they join and neither falls, **independently of `backing_buffer_lambda`** — the bound is not load-bearing here, contrary to an earlier reading of this mechanism. The premium-only fit bounds the search domain from above because every other term is nonnegative.
- **Failure asymmetry:** `lo` only ever advances to a candidate whose cost was verified against the bounds, so a returned fill can never be over-budget or inadmissible; were monotonicity ever broken, the result would be a non-maximal fill, not an overspend. That is the property to preserve.
- **The maximum-payout bound is part of the search, not a post-hoc abort:** the marginal impact rate rises with liability, so on a high-impact market the largest budget-fitting fill can be one whose all-in cost exceeds its own maximum payout (reachable at an admin-set `inventory_impact_max_rate` above roughly 0.5, with a budget the account can afford; inert at the shipped rate of 0). The probe therefore bounds each candidate by `max_cost.min(quantity)` and sizing steps down to the largest fill satisfying both, while `compute_mint_quote` still enforces RP-33 on whatever is admitted. Aborting instead was rejected: it would fire on the `std::u64::max_value!()` budget the entrypoint documents, and the read-only quote would abort identically, leaving an SDK no way to discover a workable budget. When no fill at all satisfies the payout bound the budget buys nothing and admission rejects the empty fill (`EPremiumBelowMinimum`), which is also what a dust budget does.
- **Probability bound:** none, for RP-19's reason — `min_quantity` against the budget bounds all-in price per contract at `max_cost / min_quantity`, which is a tighter statement than the premium-only version of the same bound.
...
- **Risk profile:** n/a (sizing semantics, not a probabilistic risk). Cost, stated honestly: the search runs up to ~32 probes bounded by the lot cap. The range is priced once and both payout-tree reads are hoisted out of the loop, so no probe touches storage — but each probe does re-derive the quantity-independent fee rates, which costs a `sqrt_down` per finite leg (seven u128 divides each) plus one more inside the congestion penalty. Hoisting those would mean a probe that no longer routes through the function that charges, which is the invariant this entry exists to protect; the recompute is the price of that guarantee.
- **Pinning tests:** `mint_exact_cost_tests.move` — `budget_below_the_next_lot_mints_the_largest_fitting_fill` and `budget_at_the_next_lot_all_in_cost_spends_it_exactly` (sizing pinned from both sides, the second at zero dust), `premium_budget_sizing_overspends_the_same_figure` (the premium shape's overspend on the same figure), `congestion_surcharge_after_the_quote_resizes_instead_of_aborting` and `price_move_after_the_quote_resizes_instead_of_aborting` with `premium_budget_mint_aborts_when_the_surcharge_lands_after_the_quote` (state moving between quote and execution resizes here and aborts there), `builder_fee_is_sized_inside_the_budget`, `builder_fee_at_its_own_rate_cap_sizes_exactly` (the other arm of that `min`), `sponsor_subsidy_is_sized_inside_the_budget`, `subsidy_capped_by_the_sponsored_balance_still_sizes_exactly`, `two_finite_legs_size_exactly`, `inventory_impact_is_sized_inside_the_budget`, `impact_above_the_curve_kink_still_sizes_exactly`, `impact_over_a_disjoint_book_sizes_exactly`, `impact_on_a_range_that_already_holds_exposure_sizes_exactly` (both impact arms and both liability branches, charges derived by hand), `overshoot_past_the_maximum_payout_steps_down_instead_of_aborting` and `market_where_every_fill_is_loss_making_sizes_to_nothing` (the payout bound inside the search, from both sides), `all_in_cost_never_falls_across_the_impact_kink_or_the_branch_switch`, `all_in_cost_never_falls_across_the_subsidy_cap`, `all_in_cost_never_falls_at_the_maximum_backing_buffer` (the monotonicity property walked lot by lot through every shape change), `zero_budget_aborts`, `dust_budget_below_the_minimum_premium_aborts`, `empty_balance_caps_the_budget_to_zero`, `oversized_budget_and_balance_saturate_at_the_lot_cap`, `fill_whose_cost_equals_its_maximum_payout_mints`, `account_quote_is_the_exact_debit_of_the_mint_it_sizes`, `account_quote_caps_the_budget_to_the_account_balance`, `whole_balance_budget_leaves_less_than_one_lot_unspent`.
- **Reopen when:** a mint charge becomes non-monotone in quantity (a volume discount, a per-order flat component that shrinks with size, an impact curve that can fall as liability rises), the subsidy rate rises above one, a sizing mode arrives where `min_quantity` no longer bounds price per contract, or the search's probe stops routing through the function that charges.
