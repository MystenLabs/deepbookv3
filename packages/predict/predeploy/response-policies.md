...
- **Trigger state:** a live close whose inventory-skew charge is larger than everything the close releases — `redeem_amount` plus both inventory rebates. Reaching it needs a book that is flat before the close (so closing unbalances it and is charged rather than rebated) and a leg priced far enough out of the money that its payout is worth less than the charge.
...
- **Ordering:** the escrow backs future rebates rather than being revenue, so it is **senior to the trading fee**. The charge comes out of the payout first and the fee is clamped against what remains — `min(redeem_amount, gross_proceeds - skew_charge)` — so the cap binds only when the payout alone cannot cover both. A close whose rebates absorb the charge pays the fee it would have paid without skew, and no fee revenue is diverted to the closer.
