...
- **Response:** `abort` (`ESkewChargeExceedsCloseProceeds`), the single-user user-recoverable rung. The position settles at expiry for what it is worth, which in the reachable regime is near zero; what the caller loses is the early exit.
- **Ordering:** the escrow backs future rebates rather than being revenue, so it is **senior to the trading fee**. The charge comes out of the payout first and the fee is clamped against what remains — `min(redeem_amount, gross_proceeds - skew_charge)` — so the cap binds only when the payout alone cannot cover both. A close whose rebate absorbs the charge pays the fee it would have paid without skew, and no fee revenue is diverted to the closer.
...
- **Mint side:** the mirror code `ESkewRebateExceedsMintCost` is unreachable at any admissible rate. A rebate is bounded by `rate * quantity / 2` and the premium alone is at least `min_entry_probability * quantity`; both sides are upgrade-required constants, pinned by `protocol_config_bounds_tests::max_skew_rebate_stays_below_the_minimum_premium`.
...
- **`expiry_cash::EInventoryImpactRebateExceedsReserve`.** Stated purpose: an impact rebate spends only its own escrow. Incidental duty: none beyond its escrow — the skew escrow carries the identical guard (`ESkewRebateExceedsReserve`, renumbered to 1) for the surviving mechanism, and ordinary cash was never reachable by either rebate.
...
- **Response:** every guard is removed with the state it guarded or carried by the surviving skew twin. The behavior change worth stating plainly: closes no longer receive an occupancy rebate, so `gross_proceeds = redeem_amount + skew_rebate` on the close path, and a market's cash floor is `payout_liability + skew_reserve`.
