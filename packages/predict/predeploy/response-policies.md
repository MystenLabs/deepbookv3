...
- **Risk profile:** `BEST-GUESS` — the timestamp mapping and arithmetic policy are deterministic and pinned. The provider's source cadence per series, and therefore the frequency of a 5s or 60s source-age breach, is not measured; that measurement is operational validation of the accepted fail-closed policy, not a prerequisite for its correctness. Whether linear roll-down is the best calibration model remains owned by the still-open O-1 calibration work.
...
- **Risk profile:** `BEST-GUESS` for the atomic path — unit-pinned under
  ...
  previously noted without acting is now closed: the default is 3_000, held
  strictly below the 5_000 Block Scholes price bound so a re-anchor cannot
  substitute a Pyth spot staler than the bound the spot it replaces had to
  clear. The residual cross-transaction window is unchanged — it is bounded by
  the push cycle, not by the freshness budget.
- **Pinning tests:**
