...
- **Reopen when:** the object-cache limit stops being network-invariant, a flow outside mint/close mutates a market's NAV-visible rows at trade time (the correction hook must follow it), or LP fills need fill-instant marks (which reintroduces the pause-vs-staleness tradeoff this entry settles).
...
- **Response:** `max_payout_tree_nodes` (960) is checked at mint admission (`strike_payout_tree::insert_range`). With boundaries stored inline on the market object (the store prototyped in #1269), a `value_expiry` loads no per-strike children, so the old derivation from the object-cache budget is void; the binding budget is the valuation transaction's COMPUTE — one pricer evaluation per record, measured at 15-54% of the 5M cap for a ~1,000-record book (`evidence/c1-price-memo-2026-07-01.md`) — and the market object's size. The cap keeps its pre-restructure value until P-30's localnet remeasure prices the new walk; raising it is measurement-gated. Precondition carried in the constant's doc: one `value_expiry` per transaction, never batched with another market's or with `finish_flush`. Snapshot husks are ordinary records already inside this cap — retention converts a record, never adds one — and every husk is removed by the next `value_expiry` to consume a snapshot on that market, so retention is bounded by that market's next consumed valuation. RP-29(e) carries the admission residual this creates at the cap edge.
- **Risk profile:** `MEASURED` for the inline walk at the cap (`evidence/p30-bigbook-gas-2026-08-26.md`: the full staged cycle at a 950-record book is 20.47% of one transaction's compute — wall ~4,500-4,900 records, ~4-5x headroom) and for the mint-serialization slope the inline store adds at large books (flat on the AVL; 63,000 CU / 1.26% of cap at 950 records).
- **Pinning tests:** `valuation_capacity_tests.move` — `the_node_cap_stays_usable`.
- **Reopen when:** a cap raise is proposed (re-run `p30-bigbook-gas-2026-08-26.md`'s scenario at the candidate cap; compute admits ~2,000 records, then the mint slope and the 250KB object size bind), `value_expiry` gains a child-loading step, or the store changes again.
