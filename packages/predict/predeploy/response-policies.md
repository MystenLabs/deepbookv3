...
- **Response:** proceed — `mint_exact_cost` makes the total the sizing input. A binary search over lot counts evaluates `mint_quote_at`, the same sum the mint charges, against execution-time state, and returns the largest lot-rounded quantity whose all-in cost fits `max_cost`. When the budget is what limits the fill, the unspent remainder is below one more lot's all-in cost. A fill limited by something else leaves more, and that is intended: a fill stepped down from the maximum-payout bound (below), a fill saturated at the lot cap (`order::max_quantity_lots`; RP-13 carries the same caveat for the premium shape), or one bounded by expiry cash backing.
