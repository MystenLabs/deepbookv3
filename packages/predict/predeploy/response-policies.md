...
- **Controller:** protocol (all of these guard protocol-written state).
- **The `+ rebate_reserve` term in `expiry_cash::required_cash` (and its mirror in `free_cash`).** Stated purpose: an expiry must hold cash for rebates it may owe on top of its payout liability. Incidental duty: it made expiry cash *strictly* conservative — the reserve was cash the market could not sweep, so it also absorbed any rounding gap between the payout liability and the actual settled payout, and it kept the settled sweep from returning cash that a later claim would need. Both duties die with the claim: no flow pays out of that reserve any more, the settled payout is derived from the same packed atoms as the liability (R1 bit-equal pairing, unchanged), and the surviving `assert_backing` still requires `balance >= payout_liability + inventory_impact_reserve` after every cash movement (D033 later removed the escrow term, leaving `balance >= payout_liability`). The removal *loosens* what the market may sweep by exactly the amount that used to fund rebates, which is the intended behavior change; it does not loosen what backs a winner's payout.
...
## RP-29: `inventory_grid::EExpectedPayoutUnderflow` deleted for the grid refresh — duty inventory (2026-08-20)

- **Trigger state:** not a runtime state. `move.md` requires an inventory of what a deleted guard *incidentally* bounded. Adding `inventory_grid::refresh` (D033) made this guard reachable on an ordinary close, so it was replaced with saturation; this entry is that inventory.
- **Controller:** protocol.
- **The guard.** `quote_close` asserted `expected_delta <= grid.frozen_expected_payout` before subtracting a close's expected payout from the grid's running centering term. Stated purpose: the running term cannot go negative. Incidental duty: it aborted with a named code rather than at the raw `u64` subtraction one frame deeper, and while the grid was immutable for a market's life it was also an equality proof — opens and closes both derived their delta from the same snapshot, so the sum of the parts had to fit the whole.
- **Why it had to go.** `refresh` installs the cell-mirror integral, which floors at each payout change point, while opens accumulate a per-span floor each. The two groupings legitimately disagree by rounding dust, so after a refresh the last close out of a book can derive marginally more than the grid holds. Keeping the assert would abort that close — a liveness failure on the exit path, caused by rounding, with no economic content.
- **Response:** the subtraction saturates at zero in both `quote_change` and `apply_change`, a close that empties the mirror clears the term outright, and the constant is deleted. The surviving bound is the accounting identity, not an assert: the term is only a centering subtrahend inside `capital_from_components`, which already floors at zero, and understating it can only *raise* `K`. So the failure mode the guard prevented (a wrapped term collapsing `K` to nothing) is unreachable, and the residual error errs toward charging.
- **Risk profile:** `BEST-GUESS` — the dust bound is a rounding argument over the two grouping schemes, not a measured quantity. It is atoms wide per boundary and cannot accumulate, because `refresh` overwrites the term outright rather than adjusting it.
- **Pinning tests:** `inventory_impact_tests.move` — `a_close_after_a_refresh_cannot_underflow_expected_payout` (drains a refreshed book and requires the term to reach zero rather than abort); `re_cutting_onto_the_same_snapshot_reproduces_the_grid` (a refresh onto the same snapshot reproduces the incrementally maintained maxima and `K` exactly).
- **Reopen when:** the centering term is ever read as a payable quantity rather than a subtrahend, or `refresh` starts adjusting it incrementally instead of overwriting it.
