...
- **Response:** fail closed on model age, regardless of transport activity.
  ...
  retransmissions of an unchanged tuple; the store orders each series by that
  model time (the batch envelope only breaks ties between retransmissions), so
  the roll-down anchor is the tuple's own signed model time and a changed tuple
  carries a new one. Predict computes
...
## RP-23: Block Scholes stores are permanent per underlying (resolves P-26)
