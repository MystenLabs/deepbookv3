...
- **Response:** proceed — `mint_exact_cost` makes the total the sizing input. A binary search over lot counts evaluates `mint_quote_at`, the same sum the mint charges, against execution-time state, so the fill is the largest lot-rounded quantity whose all-in cost fits and the unspent remainder is below one more lot's all-in cost. Exactness rests on every term being nondecreasing in quantity while pre-trade state is fixed: premium and each fee leg are `mul_down` of a quantity-independent rate; the trader-paid fee is `fee - min(mul_down(fee, subsidy_rate), incentives)`, whose subsidy gains at most one unit per fee unit because the rate is below one; the builder fee is a `min` of nondecreasing terms; the surcharge's firing condition is quantity-independent; and the impact charge is monotone because the prospective liability's point max and total both grow with quantity while `backing_buffer_lambda <= 1` keeps a max-driven step from shrinking the buffered gap by more than it adds. The premium-only fit bounds the search domain from above because every other term is nonnegative.
- **Bounds that still apply to the sized fill, rather than moving it:** a fill whose all-in cost would exceed its own maximum payout aborts `EMintCostAboveMaxPayout` (RP-33) instead of stepping down to a smaller one, and a budget too small to admit the minimum premium aborts `EPremiumBelowMinimum`. Sizing chooses what fits the budget; it is not a search for a quantity that satisfies protocol admission. The budget is capped to the account's settled balance first, so an unfunded account sizes to zero and rejects on admission rather than underflowing the withdrawal.
...
- **Risk profile:** n/a (sizing semantics, not a probabilistic risk). Search cost is bounded by the lot cap at ~32 probes of the cost sum; the range is priced once and the payout-tree reads are taken once, outside the loop, so a probe adds arithmetic only.
- **Pinning tests:** `mint_exact_cost_tests.move` — `budget_below_the_next_lot_mints_the_largest_fitting_fill` and `budget_at_the_next_lot_all_in_cost_spends_it_exactly` (sizing pinned from both sides, the second at zero dust), `premium_budget_sizing_overspends_the_same_figure` (the premium shape's overspend on the same figure), `congestion_surcharge_after_the_quote_resizes_instead_of_aborting` with `premium_budget_mint_aborts_when_the_surcharge_lands_after_the_quote` (fee state moving between quote and execution resizes here and aborts there), `builder_fee_is_sized_inside_the_budget`, `sponsor_subsidy_is_sized_inside_the_budget`, `subsidy_capped_by_the_sponsored_balance_still_sizes_exactly` (the one non-linear fee term), `inventory_impact_is_sized_inside_the_budget`, `impact_above_the_curve_kink_still_sizes_exactly`, `impact_over_a_disjoint_book_sizes_exactly`, `impact_on_a_range_that_already_holds_exposure_sizes_exactly` (both impact branches, charges derived by hand), `zero_budget_aborts`, `dust_budget_below_the_minimum_premium_aborts`, `empty_balance_caps_the_budget_to_zero`, `oversized_budget_and_balance_saturate_at_the_lot_cap`, `fill_whose_cost_equals_its_maximum_payout_mints`, `sized_fill_above_its_maximum_payout_aborts`, `account_quote_is_the_exact_debit_of_the_mint_it_sizes`, `account_quote_caps_the_budget_to_the_account_balance`, `whole_balance_budget_leaves_less_than_one_lot_unspent`.
