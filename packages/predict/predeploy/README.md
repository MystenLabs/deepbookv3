...
| `check.py` | The system linter: pinning tests exist, ID cross-refs resolve, MEASURED links evidence, evidence is anchored and referenced, no dead paths in live predeploy docs | Immutable evidence preserves paths from its audited revision; run the checker on any diff touching this directory or guards |
