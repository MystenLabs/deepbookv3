...
The stored pointer is the 99 `strike / forward` ratios. A quote rematerializes `ratio × F_live` against the frozen SVI shape, so spot drift no longer needs `refresh_inventory_grid`. That entrypoint is **removed**. The first charged mint inverts the ladder on-chain; there is no lifecycle-cap cut and no grid keeper. A position's expected payout is derived from the quote's live-forward view at close instead of stored at mint. Time-decay of the smile can still unbalance rung masses; that remaining gap is shape-stickiness, not a keeper cadence.
