...
Updated 2026-08-21. This is the live work register governed by the [predeploy lifecycle and update rules](./README.md#lifecycle).
...
The stored pointer is the 99 `strike / forward` ratios. A quote rematerializes `ratio × F_live` against the frozen SVI shape, so spot drift no longer needs `refresh_inventory_grid`. That entrypoint is **removed**. The lifecycle cap still submits the ratios once on an empty book. A position's expected payout is derived from the quote's live-forward view at close instead of stored at mint. Time-decay of the smile can still unbalance rung masses; that remaining gap is shape-stickiness, not a keeper cadence.

`L` remains unchanged for settlement backing and early-redemption liquidity and is not stacked into the inventory fee. The potential scale `B_K` is derived in frozen-grid capital units and the single rate `r_K` absorbs required inventory compensation and lockup rent.

**Implementation acceptance:** Grid initialization and every re-derivation are authenticated and verify each bucket in `[0.99%, 1.01%]`; rolling bucket updates match full recomputation; narrow spikes cannot poison centering beyond actual expected payout; mints and closes charge symmetrically when they increase the potential; charges are never negative and splitting one order into many collects the same total; payout backing is unchanged; integer arithmetic is deterministic and overflow-safe; and both the maximum-size trading path and a re-derivation remain within gas, object-access, and object-size limits.

**Remaining work:** Three things gate a nonzero rate.

1. *Refresh cost.* The refresh runs one range-max query per bucket plus one `walk_linear`, so its distinct dynamic-field footprint is bounded by the payout-tree node count — the same bound the NAV flush already meets.

   **MEASURED 2026-08-20 — blocking.** [`p32-refresh-gas-2026-08-20.md`](evidence/p32-refresh-gas-2026-08-20.md) records two independent `capacity-tree` runs with the harness grid lane that this measurement required. Computation is not the constraint: the refresh costs 1,561,000,000 MIST at 972 tree nodes, 31% of the 5,000M-unit cap, on a fit of 1,384,090 per node that would only reach the cap near 3,448 nodes; the second run peaks at the same 31% with a slope within 3%. From roughly 1,000 nodes it instead fails in `dynamic_field::borrow_child_object` with two thirds of its computation budget unused, attributed to the object-runtime cached-objects limit because a 200-node unit test rules out a structural read error, the same run's flush failures share the shape and the established C-1 cause, and `walk_linear` loads every node of the tree. Since `constants::max_payout_tree_nodes` is also 1,000, a market at its permitted maximum cannot be refreshed in one transaction, and making the computation cheaper does not move a ceiling that counts children loaded rather than work per child. Resolving this needs a refresh that does not read the whole tree in one transaction — chunking across transactions, or a coordinate that does not require a full walk — not a constant-factor optimization.

   This ceiling is C-1's, not a new one the grid introduces. Both calls hit it through the same `walk_linear` loading one dynamic-field child per distinct strike tick, and C-1 already measures a single 1x market aborting the flush at ~982 nodes, so a market at the permitted `max_payout_tree_nodes` cannot be NAV-valued either. The refresh is the easier instance: it carries one market per transaction, where the flush must fit every active market cumulatively and already aborts with two markets at 586 nodes each.

   **Scoping decision 2026-08-20:** P-32 now takes its own remedy rather than waiting on C-1, reversing the earlier note here. The two needs are not the same. NAV is solvency and must be exact, so a coarse substitute is inadmissible for the flush and C-1 still needs resumability or an exact aggregate. The inventory coordinate is already an average of five range-maxima over 1%-probability buckets, so it tolerates a coarse substitute that the flush cannot. Whatever C-1 chooses would likely also unblock the refresh, and if it lands first this remedy is redundant — that redundancy is accepted rather than blocking the charge behind a separate deploy item.

   **MEASURED 2026-08-20 — remedy viable, one gate left.** [`p32-cell-array-sizing-2026-08-20.md`](evidence/p32-cell-array-sizing-2026-08-20.md) scores holding a coarse fixed-cell copy of the payout profile inline beside `boundaries` and `bucket_maxima`, so a refresh rebuilds both the maxima and `E` while loading no per-node children. A `Table` entry costs one child and an inline vector costs none, so this removes the ceiling by construction rather than by tuning. At 2,048 geometric cells — 16 KB inline, one `u64` of net payout each — median per-trade charge error against reading the tree is 0.3% to 1.6% through 90% of market life, charge correlation is at least 0.993, aggregate billing lands within 1.1%, and missed charge is under 0.25% of the total. Geometric spacing makes the result scale-invariant, so a ±2 standard-deviation spot move reproduces the baseline to within a thousandth; nearest snapping beats conservative widening on every measure; the ordering holds under a Student-t law with 4 degrees of freedom. The last ~1% of market life still degrades to 5.4% median error with 4.3% of billed trades missed, which needs a policy — accept the bounded bias, stop refreshing below a remaining-time threshold, or stop charging near expiry.

   **LANDS IN SOURCE / MEASURED 2026-08-21 — refresh gas no longer blocking.** The 2,048-cell mirror is `inventory_cells`, held inline on the grid; `refresh` and the per-trade quote no longer read the payout tree. [`p32-cell-array-gas-2026-08-21.md`](evidence/p32-cell-array-gas-2026-08-21.md) records a `capacity-tree` run of 30 refreshes: initialize at 1% of the 5,000M-unit cap, then 4.5% at 96 orders up to a 13% peak (626M MIST) at the 1,000-node ceiling, with no object-cache abort. Twelve-order mint batches that rewrite the 16 KB array averaged 9.2% of the cap (38M per mint). One refresh aborted `inventory_grid:2` (mass check) and a later tick on the same market succeeded; the flush still hit C-1's `dynamic_field:0` ceiling, which this path no longer shares. The campaign analyzer flagged those flush aborts and a vacuous declared-wall miss because gRPC left `executionErrorSource` empty — that is C-1's verdict, not a refresh failure.

   **MEASURED 2026-08-21 — nonzero-rate user mint.** [`p32-user-mint-rate-on-2026-08-21.md`](evidence/p32-user-mint-rate-on-2026-08-21.md) pairs 40 single-leg mints with no grid against 40 on a gridded market that snapshotted a 2% max rate. Off costs 1.56M MIST mean; on costs 22.43M (0.45% of the cap); the increment is 20.87M (0.42% of the cap), about 14× the ordinary mint, and does not grow steeply across a 40-order ATM book. Keeper init (40.7M) and refresh (75.6M → 105M) stay off the user path. User computation is no longer a gate for turning the rate on; calibration and liquidation policy still are.

   The same run found a second blocker independent of gas, since **resolved in source**: supplying equal-mass boundaries as absolute prices is not operable against a live pricer, because the mass check left about 0.25 basis points of spot drift between the generator's snapshot and the observation the contract prices against. The two entrypoints now take the 99 interior boundaries as `strike / forward` and materialize the absolute ladder against the forward the transaction itself resolves. Since `compute_nd2` reads a strike only as `ln(strike) - ln(forward)`, a bucket's mass is a function of those ratios alone and no spot move can invalidate a cut. [`p32-ratio-boundaries-2026-08-20.md`](evidence/p32-ratio-boundaries-2026-08-20.md) measures the worst bucket error flat at 108–196 out of a 100,000 tolerance across drifts from zero to 250 basis points, against 47 of 100 buckets failing at one basis point under absolute prices. The surviving error is the SVI roll-down over the submission delay, at roughly 3,300 per second, so the budget is about 30 seconds on a 2.75-hour market instead of a sub-second race. Pinned by `one_ratio_ladder_stays_valid_after_the_forward_moves` and its negative companion.

   If a workable refresh still rules out the ~10%-of-lifetime cadence the freeze-policy measurement assumes, restrict inventory impact to the longer cadences and re-measure the compensation band at the cadence that fits. Roll-down error scales with how fast remaining time is shrinking, so the shortest workable cadence is still unestablished and the grid lane's two-hour restriction stands.
2. *Liquidation policy.* Decide whether a risk-increasing liquidation deducts an available inventory charge from liquidation proceeds or is explicitly treated as uncompensated forced risk.
3. *Calibration and charge shape.* `B_K` derivation in frozen-grid capital units and `r_K` calibration remain open, and the capped convex potential must first earn its extra state against a flat charge on marginal centered capital. Part 8 of [`inventory_impact_value_ceiling.ipynb`](../simulations/inventory_impact_value_ceiling.ipynb) supplies the target: with income fixed by contract quantity and width, `R` spans 10.1x across flow shapes today, and `r_K` of 2% leaves a residual 4.3x spread. Extend that notebook before changing source to compare `max(0, Phi(K_after) - Phi(K_before))` against `r_flat * max(0, K_after - K_before)` on identical generated books, using the 100-bucket range-max coordinate for charges and fine-cell economic capital for the denominator. Match aggregate inventory income rather than nominal rates, sweep convex scale against central-book capital and book thickness, and report the cross-shape `R` spread, fully clustered `R`, aggregate charge versus ordinary fees, and per-trade median/p95/maximum charge over paired seeds. Flat is the default because it removes the stored expected-payout level, priced refresh walk, scale parameter, and RP-29 runtime condition; retain convexity only if the best scale reduces the matched-income `R` spread by at least 25% relative to flat without increasing the maximum per-trade charge, consistently across the central and adjacent thickness settings. Choose flat only if convexity misses the spread threshold throughout; if it clears that threshold but fails the charge-concentration condition, keep the rate at zero and record the frontier for an explicit policy decision.

   **MEASURED 2026-08-20 — unresolved.** [`p32-flat-vs-convex-2026-08-20.md`](evidence/p32-flat-vs-convex-2026-08-20.md) records 240 paired generated books. At matched aggregate inventory income and the best tested scale, convexity reduced the return spread relative to flat by 28.0%, 27.4%, and 27.4% at 100, 200, and 400 trades, clearing the 25% threshold with paired-seed standard errors of 0.3–0.5 points. It also raised the maximum per-trade charge at every thickness, so it failed the charge-concentration condition. The central 200-trade result is the policy frontier: flat produced a 5.25x return spread and $162 maximum charge; convex produced 3.81x and $278. The configured rate remains zero until the simpler state versus stronger targeting trade-off is decided.

**Output contract:** Land the implementation, pinning tests, implementation-aligned notebook, gas evidence, `B_K` derivation, and `r_K` calibration together before enabling a nonzero rate. Resolve this item with exactly one grid formula and no additional `L`-based taker charge.
