    ...
    for (const event of receipt.events) {
        ...
        if (name === "OrderMinted") {
            ...
            const replacementRef = replacement !== null && row.action === "redeem_live"
                ...
                : null;
            updates.push({ type: "live_order_redeemed", order_ref: ref, order_sequence: orderSequence(id), quantity_closed: decimal(value.quantity_closed), remaining_quantity: decimal(value.remaining_quantity), replacement_order_ref: replacementRef, replacement_order_sequence: replacement === null ? null : orderSequence(replacement), redeem_amount: decimal(value.redeem_amount), trading_fee: decimal(value.trading_fee), builder_fee: decimal(value.builder_fee), penalty_fee: decimal(value.penalty_fee), inventory_charge: decimal(value.inventory_charge), inventory_rebate: decimal(value.inventory_rebate), inventory_reserve: decimal(value.inventory_reserve), onchain_timestamp_ms: decimal(value.onchain_timestamp_ms), ...sourceTimestamps(value) });
...
async function stateSnapshot(state: SimState): Promise<Record<string, string>> {
    ...
    return {
        ...
        inventory_reserve: value.inventoryReserve.toString(),
