...
| `AdminCap` holder | Fees, LTV, freshness thresholds, per-expiry funding caps, profit share, pause switches, underlying registration, lifecycle-cap minting, and genesis-bootstrapping the pool | Cannot touch a holder's position, a manager's balance, or pool custody directly |
