...
Each expiry holds its own DUSDC in `ExpiryCash`, and the invariant the protocol maintains is that an expiry's cash always covers its **payout liability plus its unresolved trading-loss rebate reserve**. Surplus above that required amount is the only cash the pool may sweep back, and the rebalancer keeps a buffer above the required amount plus a fixed cash floor. Two limitations follow:

- The invariant is enforced at each cash operation by the expiry, but the *payout liability* it is checked against uses a **conservative max-live backing** term for live markets (an upper bound on any future live payout) rather than an exact per-order live value. This intentionally avoids runtime scans and clock-passing; it can over-reserve relative to the true current liability, which is conservative for solvency but means the expiry may hold more cash than strictly needed.
- Backing is only as good as funding. The pool tops an expiry up toward its target, but funding is capped per-expiry (admin-set) and bounded by idle pool liquidity. If an expiry's liability grows faster than the pool can fund it within those caps, the rebalancer tops up only what room and idle balance allow.
...
The conservative max-live backing reserve is the cushion that makes this safe: because each leveraged order reserves at least its maximum future live payout, a lagging liquidator does not by itself make the expiry under-backed. The risk it does create is the NAV-overstatement described under LP risk if valuation runs while orders are still above the index but the per-order recoverability assumption is stale.
