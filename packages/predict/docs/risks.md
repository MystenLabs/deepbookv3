...
| --- | --- | --- |
| Pyth Lazer feed (propbook `PythFeed`) | The spot price live pricing is built on | Cannot write Block Scholes data; missing/unavailable/non-advancing source data is rejected; stale spot goes unused (falls back to the surface forward), while fresh unusable positive-only spot aborts live pricing |
| Block Scholes source (propbook `BlockScholesFeed`) | The volatility surface (SVI) and the spot/forward basis used to price every range | Cannot move custody, mint/redeem, create or compact markets, or start a flush; stale, missing, or Predict-unsafe surfaces abort at live pricing |
...
| `AdminCap` holder | Fees, LTV, freshness thresholds, per-expiry funding caps, profit share, pause switches, underlying registration, lifecycle-cap minting, and starting the pool flush | Cannot touch a holder's position, a manager's balance, or pool custody directly |
...
**Pyth Lazer spot.** A `PythFeed` stores the latest source-native payload from one Pyth Lazer feed, updated permissionlessly from a verified Lazer payload. The feed rejects missing/unavailable source fields and stale publisher timestamps; its positive-only `spot()` helper aborts if the stored source value cannot be represented as a positive normalized spot. These checks reject malformed and replayed updates, but they do not — and cannot — judge whether the price Pyth published is *correct*. If the underlying Lazer feed is manipulated, halted, or returns a thin print, Predict consumes whatever it delivers while the spot is fresh and usable. The structural defence is freshness: when the Pyth spot goes stale, live pricing stops using it and falls back to the Block Scholes surface forward rather than serving an old spot.
...
- register which Propbook underlyings Predict may build markets on and the minimum tick size for each.
