...
| Block Scholes operator (`MarketOracleWriterCap`) | The volatility surface (SVI) and the spot/forward basis used to price ranges; the data feeding the fallback settlement source | Cannot move custody, mint/redeem, create or compact markets, push zero/future/stale-timestamped Block Scholes data, or trigger settlement with any special authority (settlement is permissionless and source-gated) |
| Market-lifecycle operator (`MarketLifecycleCap`) | Creating future expiry markets and compacting settled ones | Cannot write prices or any oracle data, move custody, or mint/redeem; admin-revocable on the pool's allowlist |
| `AdminCap` holder | Fees, LTV, freshness thresholds, per-expiry funding caps, profit share, pause switches, feed/incentive creation, oracle writer and market-lifecycle cap minting | Cannot touch a holder's position, a manager's balance, or pool custody directly |
...
**Block Scholes volatility surface.** Range probabilities are not read directly from spot. They come from a pricing curve built on a forward (spot × basis) and an SVI volatility surface, both written by a trusted off-chain operator holding a `MarketOracleWriterCap` authorized on that oracle. This is the protocol's most concentrated trust assumption: the SVI parameters and the forward shape the probability of every range, and therefore every entry price, live NAV mark, and liquidation decision. The contracts require a registered writer cap, nonzero spot and forward values, a strictly advancing source timestamp, and a source timestamp that is not future-dated. They do not constrain the `forward / spot` basis or the size of a spot/forward step between pushes. A correct-but-adversarial operator can steer prices anywhere that passes those structural checks; a compromised cap is therefore an unbounded live-pricing risk until the cap is unregistered or the protocol is paused.
...
**The live mark switches sources at the Pyth staleness boundary.** While the Pyth spot is fresh, live pricing anchors the forward to it (spot × basis); the moment it crosses the staleness threshold, pricing falls back to the Block Scholes forward. That switch is discontinuous and externally observable, so a trader can time a mint or redeem to whichever side of the boundary gives the more favorable mark. The jump is bounded by the current Pyth↔Block Scholes divergence and is eroded by the trade fee on both legs; it is accepted behavior, not a defect.
...
- **The settlement source is one of two, with Pyth preferred.** Settlement can come from the trusted Block Scholes operator's spot when Pyth is unavailable. Because both paths drive the same terminal write and the operator-supplied path is the fallback, a Pyth outage around expiry shifts the terminal price onto the operator, who is then trusted for the single number that resolves the market.
- **Settlement is permissionless, and cannot be forced early or from a stale source.** Anyone can trigger settlement (`settle_with_randomness`) once a valid source exists — no capability gates it, and the operator's only influence is through the price data its writer cap pushes. Nobody can settle before expiry or with a stale source, and once recorded the price is immutable.
...
- mint and revoke `PauseCap`s, enable and disable package versions, and create Pyth sources and incentive-asset bindings;
- mint Block Scholes writer caps and register/unregister them per oracle, and mint and revoke the market-lifecycle caps that gate market creation and compaction;
- choose which feed prices each market and incentive asset, i.e. the Pyth feed a market can bind to must be admin-registered.
...
- **Valuation requires syncing every active market.** A supply or withdrawal must complete a full pool sync that processes each active expiry exactly once; the valuation aborts if any active market is skipped. This is correct but it means LP actions depend on every active market's oracle being fresh enough to value at that moment. In particular, an expired-but-unsettled market pauses pool sync — supply and withdraw are unavailable for the (normally short) settlement-finalization window, and that window extends if settlement sources or keepers stall.
- **The pending-protocol-profit exclusion can floor LP value at zero.** The pool excludes the protocol's not-yet-materialized profit share from PLP value. The realized side of that exclusion is sticky — it does not shrink when LPs withdraw idle against a high active mark — so if active NAV later collapses, the exclusion can exceed gross pool value. The subtraction is clamped, so this can no longer abort supply or withdraw; the consequence is that LP-attributable value reads as zero (withdrawals pay nothing on the DUSDC leg, supply is rejected at zero pool value) until marks recover. The protocol seeding the pool and never fully exiting is the operational backstop for the surrounding zero-supply edge cases (no minimum PLP supply is enforced on-chain).
- **Protocol profit-taking is one-directional across expiries.** When an expiry materializes terminal profit, prior terminal losses are netted first — but the netting only looks backward. A profitable expiry that settles before a later loss-making one pays the protocol its full share, and that share is not clawed back against the later loss, so across a sequence of expiries the protocol can end up over-taken relative to aggregate P&L. The protocol's reserve is also **write-only by design**: there is no withdrawal path in the current package; any future use (such as a buy-and-burn or a solvency backstop) would arrive via a package upgrade.
- **Pool cash funded into live markets is not directly withdrawable.** DUSDC sent into an expiry backs that market's reserve and returns to idle only through rebalancing sweeps or settlement. Idle itself carries no standing earmark: LPs may withdraw whatever idle remains after the sync — and a withdrawal always runs the full pool sync first, including every market's top-up toward its reserve target, so an exit cannot front-run the refill of a live market. A withdrawal larger than the remaining idle aborts.
- **Withdrawals carry an uncertainty-band fee, and the fee is the only stress brake.** A withdrawing LP pays a fee proportional to the pool's aggregate live-valuation uncertainty band (the unverified under-floor exposure surfaced during the sync), scaled by an admin-tunable multiplier (`withdraw_fee_alpha`) and the LP's share, capped at the payout. The fee is retained in idle for the LPs who remain, so exiting during a period of high valuation uncertainty cannot extract value at the expense of those who stay; it is zero when the book is fully verified. The fee **attenuates but does not hard-bound** the residual: the protocol deliberately ships without a valuation halt gate, so in an extreme correlated move that out-runs the per-sync liquidation budget, an exiting LP can still capture a fee-attenuated slice of overstated NAV. That residual is LP-vs-LP mispricing, never insolvency — trader payouts are backed by the separate cash reserve. See [liquidity and NAV](./concepts/liquidity-and-nav.md) and [configuration](./design/configuration.md).
- **Incentives are paid in-kind, valued by their own feeds.** Admin-deposited SUI/DEEP incentives vest linearly and are folded into NAV (priced from their bound Pyth feeds) and paid out **in-kind, pro-rata** on withdrawal. The in-kind payout needs no oracle, so a stale incentive feed cannot block an exit — but a new depositor's share price *does* depend on those feeds being fresh (a stale incentive feed blocks new supply until it updates), and a withdrawing LP receives SUI/DEEP rather than DUSDC for the incentive portion. While PLP supply is nonzero, a fresh depositor cannot capture already-vested incentive (pricing is against total NAV) and the still-vesting remainder stays with remaining holders; if every holder fully exits mid-stream, however, the orphaned still-vesting remainder is captured by the next bootstrap supplier — the protocol's own seed position is the mitigation. Incentive valuation also assumes admin binds sane assets: a pathological decimals/price combination on a bound feed would abort valuation (and with it new supply) while that incentive is held.
...
- **Trading-loss rebates are also keeper-resolved.** Rebate claims are permissionless by design so a keeper can sweep every manager's settled rebate after each expiry. Two consequences: if the sweep stops, owed rebate cash sits escrowed inside settled expiries (safe, but invisible to pool NAV) until it resumes; and a prompt sweep means the rebate resolves at whatever active stake the manager holds when the keeper claims — managers do not choose their claim timing.
- **Pool-sync cadence is also an exit-liquidity lever.** The rebalancer that refills each market's reserve (and with it the early-exit buffer) runs inside the permissionless pool sync. Anyone can trigger a sync, so a capped early exit is normally retryable within a block or two — but in the limit, exit-buffer refill timeliness is keeper-supplied, like liquidation.

The settlement-floor reserve is the cushion that makes this safe: the floor counts every open order's payout at each settlement price it can win, so a lagging liquidator does not by itself make the expiry under-backed at settlement — a position that should have been liquidated still has its settlement payout covered, and removing it late can only release reserve. The risk it does create is the NAV-overstatement described under LP risk if valuation runs while orders are still above the index but the per-order recoverability assumption is stale.
...
- **Aggregate NAV still rests on keeper timeliness.** The pending-protocol-profit exclusion underflow is now structurally clamped, and the active mark is backed by the limited-recourse verified/unscanned bucket split plus the withdraw band fee — but the residual described under LP risk (fee-attenuated overstatement during a budget-out-running stress move) is accepted, not eliminated.
