...
| --- | --- | --- |
| Pyth Lazer feed (propbook `PythFeed`) | The spot price live pricing is built on | Cannot write Block Scholes data; missing/unavailable source fields abort during decode; non-advancing or future timestamps are no-ops; absent, stale, or unusable normalized spot goes unused (falls back to the BS forward) |
| Block Scholes source set (propbook BS spot/forward/SVI feeds) | The SVI curve and the spot/forward basis used to price every range | Cannot move custody, mint/redeem, create markets, or start a flush; stale, missing, or Predict-unsafe BS inputs abort at live pricing |
...
| `AdminCap` holder | Fees, LTV, freshness thresholds, cadence allocation caps, profit share, pause switches, underlying registration, lifecycle-cap minting, and genesis-bootstrapping the pool | Cannot touch a holder's position, an account balance, or pool custody directly |
...
| `PauseCap` holder | Emergency one-way pauses: pause global trading, pause one market's minting | Cannot unpause anything, change config, or move funds |
...
**The live mark switches forward sources at the Pyth staleness boundary.** While the Pyth spot is fresh, live pricing anchors the forward to it (`spot × basis`); the moment it crosses the staleness threshold, pricing falls back to the Block Scholes forward feed. That switch is discontinuous and externally observable, so a trader can time a mint or redeem to whichever side of the boundary gives the more favorable mark. The jump is bounded by the current spot-vs-BS-forward divergence, Predict's pricing-safe basis envelope, and the trade fee on both legs; it is accepted behavior, not a defect.
...
- **Fills are FIFO.** The flush fills supplies first, then withdrawals FIFO until idle is dry, each queue bounded by its own operator-supplied budget (independent, so a supply backlog can't starve withdrawals). A withdrawer can be deferred to a later flush if idle is exhausted, and an LP can cancel a still-pending request at any time before it fills.
- **A degenerate head request currently blocks the flush.** There is no per-request failure isolation yet: a head request whose fill is not executable at the frozen mark — it would mint zero shares or pay zero, reachable only when pool NAV is at or near zero — aborts the whole flush until that request's owner cancels it. This is a known liveness limitation of the current implementation; the intended behavior is to skip or refund such requests so the flush completes regardless of queue contents.
...
- set the liquidation LTV and admission/floor inputs that determine which leveraged positions are admitted and when they are liquidated;
...
- **Higher leverage is gated by entry price.** The protocol restricts how much leverage a low-probability range may take with a smooth probability-sensitive admission cap. This caps the most fragile combinations but does not remove liquidation risk from any leveraged order. At creation, an order must sit strictly above its liquidation threshold (`entry_value > floor_shares / liquidation_ltv`), so a leveraged order can never be created already at or beyond its own liquidation point.
...
- **Reward incentives are outside the pool.** The current `PoolVault` holds no LP-owned SUI/DEEP incentive balance and reads no incentive-price oracle. DEEP held by the vault is custodial trading stake for account benefits, not PLP capital.
- **DEEP staking is not LP capital and is freely withdrawable.** Account-staked DEEP is held in custody for trading benefits but is excluded from PLP redemption; it can be unstaked at any time (both active and inactive amounts) with no penalty.
...
- **Payouts and live redeems round in the reserve-favoring direction.** A live redeem deducts the closed slice's pro-rata static floor with a saturating floor at zero, and the settled payout is `quantity − floor_shares`.
...
- **Reserved backing is computed from the same atoms as payouts.** The payout tree stores quantity and static floor shares, and derives net payout as `quantity − floor_shares` for reserve and settlement. A partial close removes the closed slice's quantity and floor atoms, leaving the survivor's reserve terms in the tree. The net effect is that a payout can never exceed the cash reserved to back it; rounding error is biased toward the protocol retaining dust, not toward over-paying.
...
- **Trading-loss rebates can be keeper-resolved.** Rebate claims have an owner-auth path and a Predict app-auth keeper path, so a keeper can sweep settled rebates while `PredictApp` remains authorized. Two consequences: if the sweep stops, owed rebate cash sits escrowed inside expiries (safe, but invisible to pool NAV) until it resumes or owners claim; and a prompt sweep means the rebate resolves at whatever active stake the account holds when the keeper claims.
