...
| --- | --- | --- |
| Pyth Lazer feed (propbook `PythFeed`) | The spot price live pricing and incentive valuation are built on | Cannot write Block Scholes data; rejected if zero, negative, or non-advancing; goes unused (falls back to the surface forward) when stale |
| Block Scholes operator (propbook writer cap) | The volatility surface (SVI) and the spot/forward basis used to price every range | Cannot move custody, mint/redeem, create or compact markets, start a flush, or push zero/future/stale/arbitrage-violating surface data |
| Market-lifecycle operator (`MarketLifecycleCap`) | Creating future expiry markets, compacting settled ones, and starting the pool flush as a deployer | Cannot write prices or any oracle data, move custody, or mint/redeem; admin-revocable on the registry allowlist |
| `AdminCap` holder | Fees, LTV, freshness thresholds, per-expiry funding caps, profit share, pause switches, feed/incentive creation, lifecycle-cap minting, and starting the pool flush | Cannot touch a holder's position, a manager's balance, or pool custody directly |
| Flush starter (`AdminCap` or `MarketLifecycleCap`) | When the pool is valued and the LP queues drain, i.e. the oracle state at which PLP is priced | Cannot set the mark to anything but the pool's exact NAV at that instant, or fill at an off-mark price |
...
**Pyth Lazer spot.** A `PythFeed` stores the latest normalized spot from one Pyth Lazer feed, updated permissionlessly from a verified Lazer payload. The feed rejects a decoded price that is zero/negative or whose publisher timestamp does not strictly advance. These checks reject malformed and replayed updates, but they do not — and cannot — judge whether the price Pyth published is *correct*. If the underlying Lazer feed is manipulated, halted, or returns a thin print, Predict consumes whatever it delivers while the spot is fresh. The structural defence is freshness: when the Pyth spot goes stale, live pricing stops using it and falls back to the Block Scholes surface forward rather than serving an old spot.

**Block Scholes volatility surface.** Range probabilities are not read directly from spot. They come from a pricing curve built on a forward and an SVI volatility surface, both written to a `BlockScholesFeed` by a trusted off-chain operator holding a writer cap on that feed. This is the protocol's most concentrated trust assumption: the SVI parameters and the forward shape the probability of every range, and therefore every entry price, live NAV mark, and liquidation decision. The feed enforces universal math validity at ingest — positive spot and forward, SVI no-arbitrage (`|ρ| ≤ 1`), a bounded sigma, and a strictly advancing source timestamp — but it does not constrain the `forward / spot` basis or the size of a step between pushes. A correct-but-adversarial operator can steer prices anywhere that passes those structural checks; a compromised cap is therefore an unbounded live-pricing risk until the cap is unregistered or the protocol is paused.
...
**The live mark switches forward sources at the Pyth staleness boundary.** While the Pyth spot is fresh, live pricing anchors the forward to it (`spot × basis`); the moment it crosses the staleness threshold, pricing falls back to the Block Scholes surface forward. That switch is discontinuous and externally observable, so a trader can time a mint or redeem to whichever side of the boundary gives the more favorable mark. The jump is bounded by the current spot-vs-surface-forward divergence and is eroded by the trade fee on both legs; it is accepted behavior, not a defect. (There is no oracle deviation breaker or basis bound: an earlier circuit-breaker design was removed, so the operator's surface is trusted up to the structural ingest checks above.)

## The privileged flush

The single most important LP-facing trust assumption is **who decides when the pool is valued**. PLP is not priced against a continuously-published pool price; it is priced only during a **flush**, and a flush can be started only by the operator `AdminCap` or by a market deployer (`MarketLifecycleCap`). It is intended to be a cron-driven operation.
...
## Settlement is deferred (settlement-v2)

**Terminal settlement is not implemented in the current package.** A market never reports itself settled (`is_settled()` is always false), and asking for a settlement price aborts. The settled-redeem and settled-sweep code paths are present but unreachable; they are kept for the settlement-v2 work that will read the terminal price from the propbook feeds' minute history. The consequences are real and must be understood before use:

- **No market resolves today.** Positions can be minted, live-redeemed, and liquidated, but there is no terminal payout path. A market that reaches its expiry simply cannot be settled or compacted until settlement-v2 ships.
- **Flush-liveness precondition.** Because no market ever settles, the settled-market sweep that would drop an expired market from the active set is dead code. A market that crosses its expiry therefore stays in the active set, and the flush — which values every active market through `current_nav`, and `current_nav` asserts the market is still live — then **aborts pool-wide** once that market is past expiry, bricking all LP fills. There is no solvency-safe NAV for a past-expiry-but-unsettled market (the one mark must equal a settlement-dependent value that does not yet exist), so the market cannot simply be valued at zero or at free cash without breaking the dual-use mark. **Until settlement-v2, the operator MUST NOT let an active market cross its expiry across a flush** — in practice, create only far-dated markets and settle/retire them before expiry. This is a documented, deferred precondition, not a bug; the real fix is settlement-v2 restoring the sweep.

```mermaid
stateDiagram-v2
    [*] --> Active: market created (expiry in future)
    Active --> Live: minting, live redeem, liquidation (no terminal path today)
    Active --> Settled: settlement-v2 (deferred)
    Settled --> [*]: settled redeem + sweep (deferred)
    note right of Active
      A market must not cross its expiry
      across a flush until settlement-v2:
      current_nav aborts on a past-expiry
      market and bricks the flush.
    end note
```
...
- set the maximum net DUSDC the pool may fund into any one expiry, and the withdraw-uncertainty fee multiplier;
- mint and revoke `PauseCap`s, enable and disable package versions, and create incentive-asset bindings;
- mint and revoke the `MarketLifecycleCap`s that gate market creation, compaction, and starting a flush as a deployer;
- **start a pool flush**, choosing the oracle snapshot at which PLP is priced;
- choose which feeds each market binds to (a market is bound to an admin-blessed `PythFeed` and `BlockScholesFeed` at creation).
...
- write or alter oracle data — the propbook feeds are a separate package and the surface writer cap is not the `AdminCap`.
...
- **Live NAV is conditional on the liquidation policy keeping leveraged orders above their floors.** Each market's exact `current_nav` subtracts the leveraged book's exact floor correction from the range value. This is correct under an explicit precondition: every active leveraged order must be above its floor at valuation time. The flush maintains this by running a bounded liquidation pass before each active market is valued. If that health policy ever fails to keep the book above floor (for example, an underfunded keeper flow during a stress event combined with a too-small per-pass budget), the per-order correction can be incomplete, and a NAV computed in that state would be too high. LPs should treat the flush mark as valid *conditional on the book being healthy at flush time*, not as an unconditional solvency proof.
- **Valuation requires valuing every active market.** A flush must process each active expiry exactly once; it aborts if any active market is skipped or cannot be valued. This means LP fills depend on every active market's feeds being fresh enough to value at the flush instant — a stale surface on any one active market blocks the whole flush. It is also the mechanism behind the [settlement-v2 flush-liveness precondition](#settlement-is-deferred-settlement-v2): a past-expiry-but-unsettled active market aborts the flush outright.
- **The pending-protocol-profit exclusion can floor LP value at zero.** The pool excludes the protocol's not-yet-materialized profit share from PLP value. The realized side of that exclusion is sticky — it does not shrink when LPs withdraw against a high active mark — so if active NAV later collapses, the exclusion can exceed gross pool value. The subtraction is clamped, so this cannot abort the flush; the consequence is that LP-attributable value reads as zero (withdrawals pay nothing, supply mints nothing) until marks recover. The protocol seeding the pool and never fully exiting is the operational backstop for the surrounding zero-value edge cases (no minimum PLP supply is enforced on-chain).
- **Protocol profit-taking is one-directional across expiries.** When an expiry materializes terminal profit, prior terminal losses are netted first — but the netting only looks backward. A profitable expiry that resolves before a later loss-making one pays the protocol its full share, and that share is not clawed back against the later loss, so across a sequence of expiries the protocol can end up over-taken relative to aggregate P&L. The protocol's reserve is also **write-only by design**: there is no withdrawal path in the current package; any future use (such as a buy-and-burn or a solvency backstop) would arrive via a package upgrade. (This path becomes live only with settlement-v2, since materialization happens on the settled sweep.)
- **Pool cash funded into live markets is not directly withdrawable.** DUSDC sent into an expiry backs that market's reserve and returns to idle only through rebalancing sweeps (or, under settlement-v2, settlement). The rebalancer tops each active market toward its reserve target during every flush, and a flush always runs before withdrawals are paid, so an exit cannot front-run the refill of a live market. LPs may withdraw only against whatever idle remains after that rebalance; a withdrawal larger than the available idle is deferred or partially filled by the FIFO drain.
- **Withdrawals carry an uncertainty-band fee, and the fee is the only stress brake.** A withdrawing LP pays a fee proportional to the pool's aggregate live-valuation uncertainty (the unverified under-floor exposure surfaced during the flush's liquidation passes), scaled by an admin-tunable multiplier (`withdraw_fee_alpha`) and the LP's share, capped at the payout. The fee is retained for the LPs who remain, so exiting during a period of high valuation uncertainty cannot extract value at the expense of those who stay; it is zero when the book is fully verified. The fee **attenuates but does not hard-bound** the residual: the protocol deliberately ships without a valuation halt gate, so in an extreme correlated move that out-runs the per-flush liquidation budget, an exiting LP can still capture a fee-attenuated slice of overstated NAV. That residual is LP-vs-LP mispricing, never insolvency — trader payouts are backed by the separate per-expiry cash reserve.
- **Incentives are paid in-kind, valued by their own feeds.** Admin-deposited SUI/DEEP incentives vest linearly and are folded into NAV (priced from their bound Pyth feeds) and paid out **in-kind, pro-rata** on withdrawal. The in-kind payout needs no oracle, so a stale incentive feed cannot block an exit — but a supplier's share price *does* depend on those feeds being fresh (a stale incentive feed blocks new supply until it updates), and a withdrawing LP receives SUI/DEEP rather than DUSDC for the incentive portion. While PLP supply is nonzero, a fresh supplier cannot capture already-vested incentive (pricing is against total NAV) and the still-vesting remainder stays with remaining holders; if every holder fully exits mid-stream, however, the orphaned still-vesting remainder is captured by the next bootstrap supplier — the protocol's own seed position is the mitigation. Incentive valuation also assumes admin binds sane assets: a pathological decimals/price combination on a bound feed would abort valuation (and with it the flush) while that incentive is held.
...
- **Payouts and live redeems round down; the holder eats at most one unit (ulp).** A live redeem deducts a round-down floor amount with a saturating floor at zero, and the settled payout (under settlement-v2) is `quantity − floor(floor_shares × terminal_floor_index)`. The winner can lose at most one fixed-point unit to rounding.
...
- During the flush that values active markets, a bounded liquidation pass runs first to re-establish the above-floor precondition the exact `current_nav` correction depends on. If the budget is too small for the number of under-floor orders during a fast move, a single flush may not fully clean the book.
...
The settlement-floor reserve is the cushion that makes this safe: the floor counts every open order's payout at each settlement price it can win, so a lagging liquidator does not by itself make the expiry under-backed — a position that should have been liquidated still has its (future) settlement payout covered, and removing it late can only release reserve. The risk it does create is the NAV-overstatement described under LP risk if a flush values the book while orders are still above the index but the per-order recoverability assumption is stale.

## Maturity and limitations

Predict is pre-deployment software. Beyond the per-topic caveats above:

- **Settlement is deferred to settlement-v2.** As above, no market resolves today, and an active market must not cross its expiry across a flush. This is the single largest functional gap and the operator's standing responsibility until v2 ships.
- **The interface is still changing.** Module boundaries, function signatures, events, and config shapes are not frozen. Integrations built against the current code should expect breaking changes. In particular, the off-chain indexer/server is not yet rewired for the current event set (new LP/flush events, the tick `range_key`, the propbook feed bindings on `MarketCreated`, and the removed oracle events).
...
- **Liquidation flow test coverage is limited.** The bounded-liquidation and floor-decay paths are the most safety-critical and the least exercised; the exact-NAV correction depends on this flow, so its limited coverage is a real maturity risk until it is hardened.
- **The exact NAV still rests on keeper timeliness.** The pending-protocol-profit exclusion underflow is structurally clamped, and the withdraw band fee attenuates exits during valuation uncertainty — but the residual described under LP risk (fee-attenuated overstatement during a budget-out-running stress move) is accepted, not eliminated. The privileged flush closes the *timing* attack on the mark, not the *keeper-throughput* dependence of the book health the mark assumes.
- **Valuation quantity math has a documented u64 envelope.** The per-strike valuation accumulator multiplies quantity by strike price in u64: a maximum-size single mint overflows (and cleanly aborts) only at strikes above roughly $430k, and the aggregate valuation fold would need on the order of $123–184M of concentrated high-strike open interest in one expiry — far above the default per-expiry funding cap — before it could abort valuation. Accepted and bounded by the funding cap rather than widened.
