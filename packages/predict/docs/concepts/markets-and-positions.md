...
`mint_exact_cost` is the fixed-spend variant. The caller fixes the total, and sizing happens inside it: the market caps `max_cost` to the account's available USDC, then searches for the largest lot-rounded quantity whose **all-in** cost — premium plus trader-paid trading fee, builder fee, congestion surcharge and inventory-impact charge — fits that figure. The search evaluates the same cost computation the mint charges, against fee and book state at execution, so no fee estimate is needed off-chain and no padding is required against fee or price state moving after a quote: the debit never exceeds the budget. When the budget is what limits the fill, what is left over is less than one more lot's all-in cost; a fill limited by something else leaves more. In particular, because the marginal inventory-impact rate rises with liability, the largest fill a budget could pay for is sometimes one that would cost more than it could ever pay out, and sizing then steps down to a fill that clears that bound — which can leave most of the budget unspent. `min_quantity` is the slippage guard for this shape: the spend is fixed, so an adverse move in price, surcharge, subsidy or impact arrives as fewer contracts, and a fill below the floor aborts instead of settling for a smaller position. It bounds the all-in price paid per contract at `max_cost / min_quantity`, which is why there is no `max_probability` argument. `quote_mint_exact_cost_for_account` is the read-only twin, returning the fill the same budget would buy and that fill's cost decomposition — the natural source for a `min_quantity` floor. A budget that cannot buy any admissible fill aborts rather than minting nothing: too small for the minimum premium, or a market where every size would cost more than it could pay.
