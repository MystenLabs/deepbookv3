...
Trading and capital movement are mediated by `AccountWrapper` plus account `Auth`. Minting, live redemption, LP request/cancel flows, builder-code config, and the settled exit all consume account auth — the owner's own, or that of another app the owner has authorized. Predict issues no package-level authority of its own, so no keeper can act on a position. The full account and authorization model is documented in [architecture](../design/architecture.md).
