...
`mint_exact_cost` is the fixed-spend variant. The caller fixes the total, and sizing happens inside it: the market caps `max_cost` to the account's available USDC, then searches for the largest lot-rounded quantity whose **all-in** cost — premium plus trader-paid trading fee, builder fee, congestion surcharge and inventory-impact charge — fits that figure, and aborts if the result is below `min_quantity`. The search evaluates the same cost computation the mint charges, against fee and book state at execution, so no fee estimate is needed off-chain and no padding is required against fee or price state moving after a quote: the debit never exceeds the budget, and unless sizing saturates at the lot cap, what is left over is less than one more lot's all-in cost. It carries no `max_probability` argument for the same reason as `mint_exact_amount` — `min_quantity` against the budget bounds the price paid per contract, here all-in. `quote_mint_exact_cost_for_account` is its read-only twin, returning the fill the same budget would buy and that fill's cost decomposition. Sizing also respects the maximum-payout bound: because the marginal inventory-impact rate rises with liability, the largest fill a budget could pay for is sometimes one that would cost more than it could ever pay out, and the search steps down to the largest fill that clears both bounds. A budget that cannot buy any admissible fill — too small for the minimum premium, or a market where every size is loss-making — aborts rather than minting nothing.
