...
Both paths emit **`LiveOrderRedeemed`** (carrying `quantity_closed`, `remaining_quantity`, `replacement_order_id` when present, and the separate `skew_charge`/`skew_rebate`). `min_proceeds` applies to the final credited amount: gross redeem plus skew rebate minus skew charge and ordinary fees. Live redeem requires `account::Auth` (owner auth, or app-auth via the account registry).
