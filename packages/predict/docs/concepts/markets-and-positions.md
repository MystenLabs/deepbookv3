...
Both paths emit **`LiveOrderRedeemed`** (carrying `quantity_closed`, `remaining_quantity`, `replacement_order_id` when present, `inventory_impact_charge`, `k_before`, and `k_after`). `min_proceeds` applies to the final credited amount: gross redeem minus ordinary fees and any inventory charge. Live redeem requires `account::Auth` (owner auth, or app-auth via the account registry).
