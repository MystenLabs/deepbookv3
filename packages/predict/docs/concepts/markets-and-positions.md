...
Trading and capital movement are mediated by `AccountWrapper` plus account `Auth`. Minting, live redemption, LP request/cancel flows, builder-code config, and the settled exit all consume account auth — the owner's own, or an `Auth` issued by an app the account registry has authorized. Registry app-authorization is global and gated by `AccountAdminCap`, not by the account owner, and app auth carries no owner restriction; whether an authorized app acts only on its user's behalf is that app's own discipline. Predict holds no such authority, so Predict itself cannot act on a position. The full account and authorization model is documented in [architecture](../design/architecture.md).
...
Settlement records an exact Propbook spot at the market expiry: Pyth first on every attempt, then the exact Block Scholes minute-boundary spot when Pyth remains unavailable at least 30 seconds after expiry; see [pricing and oracles](./pricing-and-oracles.md). `try_settle` is the single permissionless transition and records the price plus terminal payout liability together. `redeem_settled`, pool rebalance, and pool valuation only consume the current phase, so transaction builders compose `try_settle` before them when settlement may be due. If neither exact source is usable, the market remains unsettled, standalone rebalance moves no cash, and live pricing rejects the past-expiry market.

### Settled redeem

After settlement, a position is closed for its settled payout through `redeem_settled`, which requires account auth. A settled close is always full — the entrypoint takes no quantity. Payout is the full `quantity`, credited to the order's account and zero when the settlement price lies outside `(lower, higher]`. It emits **`SettledOrderRedeemed`** with the payout; the settlement price is carried by `MarketSettled`. Closing live (unsettled) risk through the settled path aborts. Nobody but the account's own authority can clear a settled position, so an unredeemed position stays open until its holder acts.
