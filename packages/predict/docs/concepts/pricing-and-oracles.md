...
A `PythFeed` is bound to exactly one Pyth Lazer feed (identified by a `u32` feed id) and stores the latest source-native price payload for that feed — one global spot source, not a per-expiry value. Its permissionless `update` decodes a verified Lazer payload, finds the matching feed, reads its `(price, exponent, feedUpdateTimestamp)` fields, and stores the raw sign/magnitude fields plus the per-feed generation time. Raw getters expose those fields; `normalized_spot()` is the Propbook-normalized read that converts to Predict's 1e9 fixed-point scaling (`price_1e9 = magnitude × 10^(exponent + 9)`) and returns `none` when no latest row exists or the raw value cannot produce a positive normalized spot.

Two timestamps are recorded on every accepted update, and the distinction is load-bearing:

- The **publisher timestamp** — Pyth's per-feed `feedUpdateTimestamp` (converted from microseconds to milliseconds, rounding up). This is when that feed's price was generated; it can be older than the enclosing Lazer update timestamp when Pyth carries a price forward.
- The **on-chain landing timestamp** — `clock.timestamp_ms()` captured when the update *landed on chain*.

Propbook rejects a missing per-feed timestamp or one later than the enclosing update. The generic lane records a latest update only when the publisher timestamp is positive, not in the future relative to the on-chain landing time, and strictly advances the previous latest row. Future, zero, stale, or duplicate source timestamps are no-ops rather than aborts. A carried update therefore retains the older price-generation time instead of being re-dated as fresh. Freshness is a read-time concern: Predict compares the `source_timestamp_ms` in the returned `OracleRead` against the current clock and the configured freshness window.
...
The read is an **exact whole-millisecond lookup**, not an at-or-after scan: `normalized_spot_at(expiry)` is an equality lookup on the lane's exact-history table, and the only writer, `propbook::pyth_feed::insert_at`, keys the settling row at the whole-millisecond envelope and accepts a print only when its per-feed `feedUpdateTimestamp` is within `constants::max_settlement_carry_ms` at-or-before that boundary. A price carried from beyond the window therefore aborts before it can claim the insert-only key, and no keeper can round an off-grid print onto the settling key. `market_manager::record_expiry_creation` still requires each expiry to land on its cadence period; grid alignment makes the key representable and the bounded window makes it producible whenever Pyth generated any value within it. Pool-flush liveness depends on the resolution relayer and on within-window data availability; if no qualifying row exists, the market remains unsettled rather than accepting a beyond-window stale terminal price.
