...
The read is an **exact whole-millisecond lookup**, not an at-or-after scan: `normalized_spot_at(expiry)` is an equality lookup on the lane's exact-history table, and the only writer, `propbook::pyth_feed::insert_at`, requires the per-feed `feedUpdateTimestamp` to equal the enclosing Lazer timestamp and to be a whole millisecond. A carried row therefore aborts before it can claim the insert-only key, and no keeper can forge or round a near-grid print onto the settling key. `market_manager::record_expiry_creation` still requires each expiry to land on its cadence period, but grid alignment alone does not guarantee Pyth generated a new per-feed value at that instant. Pool-flush liveness depends on both the resolution relayer and exact per-feed data availability; if no qualifying row exists, the market remains unsettled rather than accepting a carried terminal price.
