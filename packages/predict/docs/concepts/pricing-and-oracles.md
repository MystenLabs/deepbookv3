...
The provider's SVI `a` and `b` values describe variance over the horizon that remained when the current tuple was published — for republications too, since a publish with an unchanged model time carries the calibration rolled to that publish (see the store-contract paragraph above). Let `T_published` be the stored observation's `source_timestamp_ms` — the envelope time of the batch that last carried it — `T_expiry` the market expiry, and `t` the current Sui clock. Before evaluating any strike, Predict derives:
