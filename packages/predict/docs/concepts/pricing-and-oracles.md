...
SVI is its own per-expiry series and has a looser freshness threshold than BS spot/forward. Each stored observation carries the provider's two clocks directly: `published_at_ms` is the batch envelope time, advancing on every provider flush — the clock pricing freshness asserts against and the SVI roll-down anchors on — and `model_timestamp_ms` is the time the series data is "as of", held fixed across retransmissions of an unchanged value and retained on trade events as provenance. A retransmission of an unchanged tuple arrives with a newer envelope and its original model time: the provider thereby re-asserts the value as current, so freshness and the roll-down anchor advance while the model provenance stays honest. Pricing halts only when envelopes stop arriving within the freshness window — a stopped transport — never merely because a quiet series was not re-derived. A batch whose entries do not advance a series is a clean skip rather than an abort, so concurrent relayers and replayed batches cannot fail each other, roll a series' model data back, or move its stored envelope time backwards.
...
The provider's SVI `a` and `b` values describe variance over the horizon that remained when the current tuple was published. Let `T_published` be the stored observation's `published_at_ms` — the envelope time of the batch that last carried it — `T_expiry` the market expiry, and `t` the current Sui clock. Before evaluating any strike, Predict derives:
