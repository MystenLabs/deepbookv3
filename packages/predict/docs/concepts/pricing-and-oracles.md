...
SVI is its own per-expiry series and has a looser freshness threshold than BS spot/forward. Each stored Block Scholes observation carries one provider source timestamp plus its Sui execution time: spot and forward map the signed update's `value_timestamp` to `source_timestamp_ms`, while SVI maps `svi_timestamp`. The source timestamp orders the series, gates freshness, is reported in trade events, and anchors SVI roll-down. The signed batch timestamp is emitted only in `BlockScholesBatchIngested` for transport observability. A batch whose entries do not advance a series is a clean skip rather than an abort, so concurrent relayers and replayed batches cannot fail each other or renew a carried observation's freshness.

The stores validate the series id and record the source-native payload. `apply_spot_batch` updates latest under strict source-timestamp ordering and independently inserts exact history when the spot update's `value_timestamp` is a minute boundary; the dedicated `insert_at` writes only exact history. Non-minute exact insertion is a no-op, and the first valid row at an exact key is permanent. The stores do not impose Predict's pricing-safe envelope at ingest; Predict applies its own read-time checks before using a row.

Terminal settlement reads exact Pyth history first and can read Block Scholes exact spot history after its grace period (see [Settlement](#settlement)).
...
The provider's SVI `a` and `b` values describe variance over the horizon identified by the tuple's signed `svi_timestamp`. Let `T_source` be that timestamp as stored in `source_timestamp_ms`, `T_expiry` the market expiry, and `t` the current Sui clock. Before evaluating any strike, Predict derives:

    remaining_ms = T_expiry - t
    anchor_tte_ms = T_expiry - T_source
    a_eff_1e18 = sign(a) × floor(abs(a) × 1e9 × remaining_ms / anchor_tte_ms)
    b_eff_1e18 = floor(b × 1e9 × remaining_ms / anchor_tte_ms)

The multiplications use `u256` intermediates and produce 1e18-scaled `u128` magnitudes; both magnitudes round down, so signed `a` rounds toward zero. `rho`, `m`, and `sigma` are copied unchanged. At the source anchor the tuple is unchanged at 1e18 precision. An identical provider retransmit with the same `svi_timestamp` does not replace the stored tuple or re-anchor the roll-down; only a newer source tuple does. As time advances, `a_eff` and `b_eff` decrease; a tuple whose source timestamp ages out of the SVI freshness window halts pricing. Live pricing is already forbidden at or after expiry.
...
- **`use_pyth_spot_for_forward` picks the formula.** This admin setting (default on) decides whether the Pyth spot enters live pricing at all. With it off, the forward is the fresh Block Scholes forward observation on every load, the Pyth spot is read but unused, and the two rules below do not apply. Calibration is what moves this setting. The two inputs trade off along axes that have not been measured against each other: the Block Scholes forward is the model's own forward, while the Pyth spot is the higher-frequency price and remains the preferred exact settlement source before Block Scholes becomes eligible after the 30-second grace period. Neither the accuracy comparison nor the latency comparison is recorded under `predeploy/evidence/` yet, which is why the choice is a setting rather than a decision. See [configuration](../design/configuration.md) for how the setting is applied.
...
**Read-time freshness (`PricingConfig`, global).** Three admin-tunable maximum ages gate live pricing, each compared against its observation's own clock (Pyth's normalized `source_timestamp_ms`; Block Scholes spot/forward `value_timestamp` and SVI `svi_timestamp`, each stored as `source_timestamp_ms`):
...
Pyth is the exclusive source before `expiry + 30_000` and remains preferred afterward. A missing, zero, or wider-than-`u64` Block Scholes exact value is unavailable rather than an abort. Once either source succeeds, `try_settle` passes its price to `StrikeExposure::record_settlement`, which records the terminal price and exact remaining payout liability together, and `MarketSettled.settlement_source` reports `0` for Pyth or `1` for Block Scholes. If neither exact source is usable, `try_settle` returns false and the market remains unsettled; settled redeem and pool sweep consume only recorded state, while past-expiry live valuation still aborts rather than substituting an approximate mark.

Both reads are equality lookups: there is no latest, nearest, rounded, interpolated, or at-or-after fallback. Pyth `insert_at` keys a verified Lazer envelope at its exact whole-millisecond timestamp and rejects excessive carried-price age; Block Scholes `apply_spot_batch` and `insert_at` key a verified spot only when its signed `source_timestamp_ms` is a whole-minute boundary, with non-boundary batches ignored for exact history. `market_manager::record_expiry_creation` requires each expiry to align with its cadence period, and every supported cadence is a multiple of the same 60-second resolution, so both exact key types are representable. After the grace period every attempt still checks Pyth first, but the first successful transaction is final: a Block Scholes settlement may win before a Pyth row that arrives later. The trust and liveness consequences are documented in [risks](../risks.md).
