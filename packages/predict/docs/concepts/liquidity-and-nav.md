...
### Adding USDC without shares

`add_usdc_to_plp` is the one way USDC enters the pool outside a supply request or the one-time genesis lock. Anyone may call it; the payment joins idle immediately, no PLP is minted, and the caller receives nothing back and keeps no claim on it. Because `total_supply` does not move, the whole amount sits inside the value the next flush's mark divides by an unchanged share count, so NAV per PLP rises for every current holder — which is what makes it the path for incentives, operator top-ups, and any outside party wanting to pay the pool. It does **not** touch the profit basis: that basis tracks cash the pool sent to and got back from expiries, so an outside contribution is neither a debit nor a credit and the protocol reserve takes no share of it.

Four admission gates apply.

- **A bootstrapped pool.** Before the genesis lock there is no share base to credit, so the contribution is refused (`ENotBootstrapped`), as supply and withdraw requests are.
- **A minimum size** (`min_usdc_contribution`), matching the supply-request floor.
- **A price ceiling of 10 USDC per PLP.** Pool cash after the contribution — idle plus the cash the pool has sent into active markets and not yet received back — may price at most 10 USDC per PLP, a tenth of the 100 USDC per PLP the drain requires. Above the drain's ceiling every supply and withdraw head is refunded, and because `total_supply` grows only through a supply fill, nothing would bring the price back down; the pool would never take liquidity again. Since a contribution mints no shares, an unbounded one against a small share base reaches that state cheaply, so it is refused at the source rather than discovered at the next flush. The contribution ceiling sits well inside the drain's because LP fills after a contribution only push the price up: shares round down, and retained supply and withdraw fees stay in the pool. The check is one-sided — a contribution can only move the price up — and it scales with the share base: a pool holding 10 PLP accepts at most 100 USDC of pool cash, while one holding 500,000 PLP accepts 5,000,000. Cash sent into markets counts because the mark counts it; otherwise moving idle into a market would reopen the ceiling. Each market's count floors at zero, so a market that has already returned more than it was sent can absorb idle without it counting, up to that returned profit. Seeding a large incentive balance therefore means supplying real liquidity first and contributing after.
- **No flush in flight.** Unlike `rebalance_expiry_cash`, contributions are gated for the whole flush: the seal freezes idle mid-flush, so an ungated contribution would land on one side or the other of that capture purely on transaction timing, deciding by accident whether that flush's queued withdrawals were paid out of it. Refusing it outright makes every contribution fully captured by the next snapshot instead.
...
The two are disjoint: the moment a cut materializes it leaves `exclusion` (its profit enters `profit_basis_debits`) and, if not immediately movable, enters `pending_protocol_profit`. Incentive value is not part of this figure (incentives are out of the pool entirely). A no-shares contribution enters `idle_USDC` alone and moves neither profit-basis term, so it raises `pool_nav` by its full amount.
...
Because materialized profit is netted across expiries, it does not say how any one expiry did. The settled sweep therefore also emits `ExpiryPnl`, the pool's lifetime net cash result on that expiry: `received_from_expiry − sent_to_expiry` as an `in_profit` flag and an absolute `amount`, alongside the market's underlying, cadence-period start (`period_start_ms`), expiry, and settlement price. Every expiry emits it on its first settled sweep, even one that returns no cash, and again on any later sweep that returns more; each emission carries lifetime totals, so the latest supersedes earlier ones. The figure is gross of the protocol/LP split and of `net_losses_to_fill`, so a losing expiry reports its loss even though nothing materializes. Like profit materialization, it counts the sponsor fee subsidies mints moved into expiry cash as received; subtract the expiry's `OrderMinted.fee_incentive_subsidy` total to isolate the trading result. Cash held back for unredeemed winning payouts counts as paid out.
