...
A dead keeper stalls the flush; trading continues. The cost is queued LP fills waiting and the flush-set markets' settlement deferred. `abort_valuation_privileged` discards the in-flight valuation immediately on lifecycle authority; `abort_valuation` is permissionless once the flush has been in flight longer than `max_valuation_window_ms` (admin-tunable within 1 minute–4 hours, default 10 minutes — past it anyone may discard the flush, so a fill's mark staleness tracks the operator's cadence and this deadline rather than being hard-capped by it, since `finish_flush` is not itself deadline-gated). Abort discards the partial NAV — the frozen marks are sound only as a simultaneous set, so a later flush re-snapshots — while cash already moved by snapshot-stage settled sweeps stays moved (an invariant-preserving per-market operation). Market stamps go stale and are lazily discarded by the next trade or settle attempt, so neither abort nor finish visits stamped markets.
