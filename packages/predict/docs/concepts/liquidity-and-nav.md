...
- **`free_cash = cash_balance − inventory_reserve`** — the expiry's DUSDC net of the isolated skew escrow it still owes. Skew escrow is not LP value while live.
...
Each expiry has a **required cash** floor of `payout_liability + inventory_reserve`. The pool rebalances each active expiry toward a target derived from a **rebalance band** around that requirement:
...
```
cash_balance ≥ payout_liability + inventory_reserve
```
...
The independent `inventory_reserve` is the cumulative skew potential collected from concentrating trades minus rebates paid to flattening ones. It is excluded from NAV and pool sweeps, and the market additionally asserts `inventory_reserve ≥ rate × D(W)` — with equality, because charges and rebates are exact differences of the same floored potential. Settlement releases the residual earmark because the rebate path is no longer reachable. See [fees and rebates](./fees-and-rebates.md#inventory-skew-charge-and-rebate).
