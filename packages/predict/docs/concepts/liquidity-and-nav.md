...
- **The executable price ceiling.** Pool cash after the contribution — idle plus the cash the pool has sent into active markets and not yet received back — must still price within the band the drain requires — at most 100 USDC per PLP. Above that ceiling every supply and withdraw head is refunded, and because `total_supply` grows only through a supply fill, nothing would bring the price back down; the pool would never take liquidity again. Since a contribution mints no shares, an unbounded one against a small share base reaches that state cheaply, so it is refused at the source rather than discovered at the next flush. The check is one-sided — a contribution can only move the price up — and it scales with the share base: a pool holding 10 PLP accepts at most 1,000 USDC of pool cash, while one holding 500,000 PLP accepts 50,000,000. Cash sent into markets counts because the mark counts it; otherwise moving idle into a market would reopen the ceiling. Seeding a large incentive balance therefore means supplying real liquidity first and contributing after.
