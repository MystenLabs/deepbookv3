...
Anyone may run a liquidation pass; no capability or admin authority is required. Both entrypoints take the market's bound `PythFeed` and `BlockScholesFeed`, and a pass is gated only on the package version being allowed for the market, the protocol not being mid-valuation, those feeds being the ones bound to the market (`assert_feeds`), and the market being active (`assert_active`, derived from its own expiry). There are two entry shapes: a bounded pass that the caller hands a budget, and a single-order attempt by ID. Both re-derive the threshold from current feed state and liquidate only orders that are genuinely under their floor; an order that is checked but still solvent is left untouched.
...
The knock-out is a *discretely monitored* barrier: it is enforced by bounded keeper passes, not continuous observation. Because scans are bounded, the protocol never proves in a single transaction that *every* leveraged order is above its floor. Live pool NAV subtracts each leveraged order's floor from its own range value, capped at it (`min(quantity × range_price, floor_shares × index_now)`, summed as the NAV correction — see [leverage and the floor](./leverage-and-floor.md)). That per-order cap is correct only under the precondition that each active leveraged order is individually above its floor: an order *below* its floor is economically exhausted (its true recoverable value is zero), but its `min` still subtracts the full range value, leaving the floor's excess uncredited — so an under-floor order left in the live indexes would let the book's reported NAV overstate recoverable value.

The bounded-budget design holds that precondition by policy rather than by exhaustive per-valuation proof: the head slice continuously re-checks the largest positions, the passive slice sweeps the remainder over time, and the inline passes on every mint and redeem keep the book trimmed under normal flow. **Live NAV is therefore exact conditional on the health policy keeping leveraged orders above their floor.** In a regime where price moves faster than the budgeted passes can clear under-floor positions — for example a sharp move that pushes many large leveraged orders under simultaneously, or a starved market with no trading flow — the live NAV reported to liquidity providers can temporarily overstate recoverable value until subsequent passes catch up. This is the principal LP-facing risk of the bounded scan; it is covered in [../risks.md](../risks.md). Admin tuning of the two budgets and the liquidation LTV trades gas cost per pass against how tightly the floor invariant is maintained.
