...
| `MarketLifecycleCap` | `market_lifecycle_cap` | create expiry markets (`registry::create_and_share_expiry_market`); also the **sole** authority to start the privileged pool flush (`plp::start_pool_valuation`) and to discard one immediately (`plp::abort_valuation_privileged`) | minted and revoked by `AdminCap` against the `Registry` allowlist |
