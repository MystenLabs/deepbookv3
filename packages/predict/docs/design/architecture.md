...
| `MarketLifecycleCap` | `market_lifecycle_cap` | create expiry markets (`registry::create_and_share_expiry_market`); also the **sole** authority to start the pool flush (`plp::start_pool_valuation`); a fresh start discards any in-flight flush, so there is no separate abort entrypoint | minted and revoked by `AdminCap` against the `Registry` allowlist |
