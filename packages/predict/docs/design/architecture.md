...
- **Per-expiry working cash** lives in each `ExpiryMarket`'s embedded `ExpiryCash`. It must always cover the expiry's payout liability; the market re-asserts this backing invariant after every cash movement.
