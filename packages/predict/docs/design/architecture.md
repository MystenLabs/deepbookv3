...
- **Creation-time coverage.** `create_and_share_expiry_market` takes Propbook's `&OracleRegistry`, the current feed objects, and a `propbook_underlying_id`, then asserts that Propbook currently has canonical Pyth, BS value store, and BS SVI store bindings for that underlying and deployable expiry. It loads the live surface, inverts the padded 1–99 belly, and snapshots the underlying ID, derived tick geometry, and the deployable expiry and reference-tick source timestamp. Pairing spot, forward, and SVI to one underlying/expiry is therefore a Propbook registry claim, not a market-deployer claim.
