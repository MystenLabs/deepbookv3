...
The flush is **privileged**, not permissionless: only a market-deployer `MarketLifecycleCap` proof starts one (the sole flush authority; the root-`AdminCap` path was removed), and only the starter may value markets and finish. The cap-holder is trusted not to manipulate the live oracle around the snapshot — the single frozen mark prices both supply and withdraw, so it must equal true recoverable value, which the exact snapshot-instant reconstruction guarantees. A stalled flush is discarded, never completed by a stranger: `abort_valuation_privileged` immediately on lifecycle authority, or permissionless `abort_valuation` after `max_valuation_window_ms`; abort discards the partial NAV (cash moved by valuation rebalances stays, stamps go stale and are lazily dropped by the next trade). While a flush is in flight, keeper cash flows, market creation, LP request cancels, and most config setters are gated; trading and new LP requests are not. Cash rebalancing and the settled-market sweep are otherwise standalone, permissionless, per-market entrypoints, because neither needs the exactly-once completeness proof. See [liquidity and NAV](../concepts/liquidity-and-nav.md).
