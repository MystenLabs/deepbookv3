...
## Block Scholes exact-settlement model carry (2026-08-31)

- **Exact Block Scholes history rejects a model older than one minute before its envelope.** `insert_exact_spot` (shared by `apply_spot_batch` and `insert_at`) no-ops when `source_timestamp_ms - model_timestamp_ms` exceeds `constants::max_block_scholes_settlement_carry_ms` (60s). `model` is when the estimate was generated; `source` is the signed flush that keys the minute. The bound is compiled and exact-insert-only: a long-held estimate cannot permanently own an expiry minute, while `latest` still accepts it and ages out at read time. *Rejected:* reusing Pyth's 2s `max_settlement_carry_ms` (Block Scholes exact keys are whole minutes, not millisecond ticks); aborting on excess carry (that would fail a latest-path batch that also touches a minute boundary). *Consequence:* after the 30s Pyth-preference window, a Block Scholes fallback still requires a representable exact row; a too-old model leaves the market unset and retryable.
