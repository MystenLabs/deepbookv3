...
- **The collateral type is `usdc::usdc::USDC` everywhere; its address is network-specific.** Testnet and localnet use `packages/usdc`; Mainnet replacements in the Predict and Sessions manifests consume Circle's pinned upstream source and its native-USDC publication records. The local test currency is never published to Mainnet, and its `Published.toml` never claims Circle's address. Dependency-source verification remains a separate deployment gate; a correctly resolved address does not prove bytecode equality. *Rejected:* generic collateral across all protocol signatures, a house stablecoin on Mainnet, and using the local test-currency implementation as Circle's source.
...
## Pre-expiry no-trade window (2026-09-04)

- **Live quotes, mints, and live redeems abort inside `no_trade_window_ms` of expiry; the default is 2s and `0` disables it.** A binary's probability moves further per unit of spot as expiry nears, so a fixed oracle staleness is worth progressively more to whoever sees the move first — the edge scales as `sqrt(staleness / time_to_expiry)` and grows without bound as the remaining time goes to zero. No finite fee can price that region, and no achievable oracle cadence removes it: halving staleness only cuts the edge by ~29%. The window bounds the region instead of charging for it. The check lives in `assert_live_flow_allowed`, the one gate every live mint, quote, and live redeem already shares, so it cannot be missed by a new trade entrypoint that routes through it. *Consequence:* a holder cannot close in the final seconds and rides to settlement — safe rather than trapping, because settlement is permissionless and `redeem_settled` stays open. Settlement and settled redemption are deliberately unaffected, so the window delays a close rather than stranding the position. This does NOT resolve O-1: the window bounds where the adverse region is reachable, it does not calibrate pricing inside it, and the near-expiry fee ramp it leaves untouched ships disabled (`expiry_fee_max_multiplier` at `1x`). *Rejected:* pricing the region through the near-expiry fee ramp alone (the ramp is linear while the edge diverges, so it always undershoots the last moments, and covering the mid-region requires a multiplier that overtaxes ordinary trading by 3-4x); snapshotting the window per market at creation like the retired `no_leverage_window_ms` (a live global value can be widened on markets already trading, which is what an incident needs); blocking only mints (the same stale price is exploitable in both directions).
- **The setter is not gated on `assert_not_valuation_in_progress`, matching `set_trading_paused`.** The knobs that are so gated can be trapped by a stalled flush (RP-29's recovery path), and a safety control that cannot be widened during the incident it exists for is not a safety control. Nothing in the flush reads the window — it is consulted only on the live trade path — so mutating it mid-valuation cannot skew a frozen mark.

## No-shares USDC contributions to the pool (2026-09-22)

- **`plp::add_usdc_to_plp` joins USDC to idle and mints nothing, and the protocol takes no cut of it.** Incentives, operator top-ups, and any outside party paying the pool previously had no entrypoint. The adjacent paths each fall short for a different reason: `request_supply` mints shares against the deposit net of the supply fee, so only that fee reaches existing holders — zero as shipped, and even at the 5% ceiling a contributor would have to deposit 20x what they meant to give; `lock_capital` is one-time, admin-only, and mints 1:1; and `sponsor_fee_incentives` is excluded from NAV in both the pool reserve and each market's `fee_incentive_balance` (`current_nav` reads `market.cash`, which that balance is not part of), reaching pool value only as trades consume the subsidy into `market.cash`, capped at `fee_incentive_subsidy_rate` of each trade's fee and by the per-market live target and lifetime cap. The only pre-existing route for an arbitrary third party was losing trades on purpose, which needs a live mintable market and leaks the protocol's cut of the resulting terminal profit. The two shipped mechanisms that already move value to holders without minting shares are the supply and withdraw fee legs, both byproducts of LP activity rather than something an outside party can direct. The contribution joins `Ledger.idle_balance` — the same balance the flush drain reads — and leaves `total_supply` untouched, so the whole amount falls inside `gross_pool_value` under an unchanged share count and raises NAV per PLP for every current holder. It does not touch the profit basis: `profit_basis_debits`/`credits` track cash sent to and returned from expiries, so an outside contribution is neither, `lp_pool_value` leaves `exclusion` unchanged, and the protocol reserve's default 10% share of profit takes none of it. *Consequence:* the contributed USDC is ordinary pool capital from the instant it lands — fundable into expiries, payable to withdrawals, and indistinguishable from fee income once in idle; the only record of who funded it is `UsdcAddedToPlp`. *Rejected:* crediting the contribution to `profit_basis_credits` so the protocol shares in it (an outside gift is not expiry P&L, and it would make the reserve's cut depend on donor generosity); an `idle_balance_after` field on the event (idle has no canonical post-state stream — `ExpiryCashRebalanced` moves idle without reporting it — so a second source would drift, per D035); reusing `min_supply_request` as the floor (it is a queue-admission policy that may diverge; `min_usdc_contribution` is its own knob at the same 10 USDC).
- **A contribution is refused when it would leave pool cash above 10 USDC per PLP (`EContributionExceedsPriceCeiling`).** This is the gate that makes the entrypoint safe to expose permissionlessly. RP-2 refunds every supply and withdraw head at a mark outside `[0.01, 100]` USDC/PLP, and `total_supply` grows only through a supply fill, so an above-ceiling pool can never price its way back down — the state is terminal. RP-2's risk profile rests on that state being unforceable ("attacker must win oracle-priced bets"); a contribution moves pool value up by an arbitrary amount with no bet to win, and against the 10 PLP genesis-lock share base about 1,000 USDC would have ended the pool's LP layer permanently, for anyone. The guard tests `ceil(cash/10) <= supply` (`constants::contribution_price_ceiling_factor`) on the pool cash the contribution is about to create: idle plus the net cash deployed into active expiries (`pool_accounting::deployed_expiry_cash`). *Why a tenth of the band, not the band:* LP fills after a contribution only push the price up — shares round down, and retained supply and withdraw fees stay in the pool — so a pool contributed to exactly the band's 100 USDC/PLP leaves it on the next uneven or fee-charged fill; the first version did exactly that (found in the second external review). A tenth leaves LP activity nine times the pool's cash of room: rounding leaves under one PLP base unit's worth per fill, a supply fee at the 5% cap moves the price at most about 1.05x per flush, and retained withdraw fees cross only once they add up to that nine times. Deliberate supply-and-withdraw round trips can add them up; that route needs no contribution, predates this entrypoint, and costs the griefer the full amount (RP-2). Trading fees are the other one-way inflow the gate cannot see: with the ceiling at the band a single 50-contract mint carried a ceiling-parked pool out of it, while at a tenth it takes about ten times pool cash in fees. *Why pool cash and not pool NAV:* NAV needs a flush to compute, and pool cash is precisely the protocol-controlled share of it. It errs toward refusing: a market that lost cash to traders still counts what the pool sent it, so a contribution can be refused at or below parity until that market is settled and swept. Idle alone is not: the mark counts cash the pool moved into markets, and `rebalance_expiry_cash` is permissionless, so an idle-only test could be emptied into a market and passed again (found in the external review of #1315; the first version shipped with it). Parking stays neutral only while a market has returned no more than it was sent, because net funding floors at zero per market; RP-2 records that bounded gap; the guard's duty is to stop this entrypoint from *manufacturing* the degenerate ratio, which is the maintainable direction RP-2 already names, while market-driven NAV moves stay owned by RP-1/RP-2. *Why a one-sided test:* a contribution can only move the price up, so the band's floor is unreachable from here. *Why an abort is on-ladder:* this is a single-user, voluntary, user-recoverable action, not a shared or mandatory path — the contributor simply contributes less, or supplies real liquidity first and contributes after. *Consequence:* the admissible contribution scales with the share base (10x supply, minus current pool cash), which constrains seeding a nearly empty pool and is slack for a funded one. *Rejected:* leaving it to RP-2 at the fill site (RP-2 responds to a degenerate mark that already exists; it cannot un-brick one this entrypoint created); bounding a single contribution as a fraction of pool value (repeated contributions walk past any per-call fraction); gating the entrypoint behind `AdminCap` (it would make the path operator-only, which is the opposite of its purpose).
- **Requires a bootstrapped pool (`ENotBootstrapped`).** At `total_supply == 0` there is no share base to credit: the USDC would raise the value of the genesis lock's permanently non-withdrawable PLP and be unrecoverable by anyone. The same gate already guards `request_supply` and `request_withdraw`.
- **Refused for the whole flush (`assert_not_valuation_in_progress`), not merely the open snapshot stage.** `rebalance_expiry_cash` is allowed mid-window because the seal froze idle alongside every market's stamped cash, so an in-window move genuinely cannot reach the mark. An external contribution is different: it is new value, not a move between two frozen figures, so landing before the seal pays that flush's queued withdrawals and landing after it does not — decided by nothing but when the contributor's transaction executed. Refusing it for the flush's whole life makes every contribution fully captured by the next snapshot, which is also why it needs no `ESnapshotStageOpen` check of its own. *Rejected:* the `assert_snapshot_stage_closed` gate `rebalance_expiry_cash` uses (it admits exactly the post-seal window whose payout ambiguity is the problem).

## Settled-redeem keeper allowlist (2026-09-25)

- **`redeem_settled_permissionless` accepts only senders on an admin-managed allowlist, which ships empty.** Until now any sender could redeem any account's settled order without the owner's signature. The payout always went to the owner's account, but the caller chose when every account's positions closed and kept the storage rebate freed by deleting them. The entrypoint now aborts with `ENotSettledRedeemKeeper` unless `ctx.sender()` is in a `VecSet<address>` that `AdminCap` edits through `protocol_config::add_settled_redeem_keeper` / `remove_settled_redeem_keeper`. The name stays because a `public` function cannot be renamed or removed in an upgrade. *Rejected:* a new entrypoint that takes a keeper capability or a separate allowlist object, with the old one reduced to a permanent abort. It adds public surface, forces every keeper onto a new call target, and the upgrade policy still forbids adding a parameter to the existing function, so the sender is the only authority signal the deployed signature can check.
- **The set lives in a dynamic field on `ProtocolConfig`.** The entrypoint already takes `&ProtocolConfig`, and the struct's layout is frozen after deploy. The field is created on the first add, and an absent field reads as an empty set, so no migration transaction is needed. A `VecSet` fits a list of a few operator keepers: membership is a linear scan, and only `AdminCap` can grow it. *Rejected:* a new `ProtocolConfig` field (layout change, not upgrade-compatible); one dynamic field per address (constant-time lookup, but the set could no longer be read back as one value).
- **Granting is version-gated; revoking is not.** This follows the registry's cap allowlists: `add_settled_redeem_keeper` runs `assert_version` like every other grant, while `remove_settled_redeem_keeper` bypasses it, so admin can drop a compromised keeper during an emergency freeze.
- **The owner path never consults the allowlist.** `redeem_settled` with owner auth stays open to every owner, so an empty allowlist, a removed keeper, or a deauthorized `PredictApp` delays a settled close but cannot strand the payout. *Consequence:* a settled order nobody redeems stays open. Its payout backing stays in the expiry market's cash, which the settled-market sweep already keeps back, so the pool's free cash is unaffected; the cost is the position's storage until the owner or a listed keeper redeems it.
- **The allowlist binds only once older packages are retired.** Every earlier package version still carries the open entrypoint, and each keeps passing `assert_version` until the watermark moves past it. `current_version` becomes 4 with this change, and the gate takes effect only when admin calls `bump_version_watermark` from the upgraded package. That bump also retires every other older-version entrypoint, so integrators must be on the new package first.

## Admin-tunable fee-incentive subsidy rate and reserve withdrawal (2026-09-25)

- **The share of each mint's trading fee paid from sponsor-funded incentives is an admin setting read live at mint time.** It shipped as an upgrade-required 20%, which left no way to run a promotion at a different depth, or to stop spending sponsorship without a package upgrade. `protocol_config::set_fee_incentive_subsidy_rate` now sets it within `[0, 0.5]`, and every quote and mint reads `ProtocolConfig`'s current value, so a change reaches markets already trading, like `referral_fee_rate`. It is not a contract term: it changes who pays the trading fee (and, on a referred mint, shrinks the referral computed on the trader-paid part), not the fee charged or what a position pays out, so there is nothing to freeze per market. *Storage:* Predict is live on Mainnet, where adding a field to `ProtocolConfig` is not an upgrade-compatible change, so the rate lives in a dynamic field (`FeeIncentiveSubsidyRateKey`). An absent field reads `default_fee_incentive_subsidy_rate`, the fixed rate earlier versions charged, so the upgrade needs no migration call and behaves identically until the admin first sets it. *Consequence:* package versions before 4 compiled in the fixed 20% and never read the field, so until `bump_version_watermark` retires them anyone can mint through an older package at 20% — lowering the rate, including to zero, binds only after that bump. *Why the ceiling is one half:* at 100% a trader with a self-owned builder code could mint both sides of a market paying no trading fee, so volume and points metrics would be free to inflate against the sponsor's balance; at one half every mint pays at least half its fee. Any ceiling must also stay at or below one: above it the subsidy could exceed the fee and underflow the trader-paid fee, and at or below it the subsidy gains at most one unit per fee unit, which keeps the trader-paid fee nondecreasing in quantity and `mint_exact_cost`'s budget search exact (RP-36). *Why not valuation-gated:* the flush never reads the rate, and a subsidy consumed mid-flush lands in market cash after the snapshot captured it, matching the `set_referral_fee_rate` exception. *Rejected:* a per-market template snapshot (a promotion or a stop would not reach live markets, and each market would need its own dynamic field); a new struct field (breaks the Mainnet upgrade); a 100% ceiling (fee-free wash volume); raising the ceiling above one (breaks the bound above).
- **The admin can withdraw the pool-level fee-incentive reserve, and only the reserve.** Sponsorship was previously irrevocable: anything a sponsor paid in stayed until it was spent on subsidies. `plp::withdraw_fee_incentives` lets `AdminCap` take any amount of `PoolVault.fee_incentive_reserve` as a `Coin<USDC>`. The reserve is excluded from PLP NAV, so a withdrawal takes nothing from LPs or holders; it is a trust assumption for sponsors, whose contributions are pooled and not earmarked, and `docs/risks.md` records it as the one admin withdrawal path on `PoolVault`. *Why not live markets' balances too:* each market holds at most its live target share of its allocation cap at a time, that balance returns to the reserve when the market settles and is swept, and setting the rate to zero stops it being spent meanwhile. Recalling it would add a second admin path into every live market and a policy for crediting the per-expiry lifetime counters back. *Lifetime counters are not credited back* by the reserve withdrawal either: they bound what a market may ever receive, and a withdrawal does not change what any market already received. *Why not valuation-gated, in either stage of a flush:* no flush figure, frozen or live, reads the reserve. `sponsor_fee_incentives` keeps its existing gate; loosening it is out of scope here. *Wind-down:* a zero rate stops spending but not allocation — permissionless rebalances keep moving the reserve into new markets — so stopping incentives entirely is a zero rate plus either a zero live target (below) or a reserve withdrawal. *Rejected:* refunding to the original sponsor (the reserve keeps no per-sponsor accounting, and adding it would mean new per-sponsor state); a withdraw-all variant (a PTB can pass `fee_incentive_reserve()` straight into the amount).

## Admin-tunable fee-incentive allocation rates (2026-09-25)

- **How much of the sponsor reserve each market may hold, and receive over its life, are admin settings.** Both were upgrade-required constants: each live rebalance topped a market's incentive balance up to 2% of its cadence `max_expiry_allocation`, and a market could receive at most 10% of it over its life. With the subsidy rate adjustable, those fixed slices decided whether the configured rate actually held: at Mainnet's 10,000 USDC allocation cap a market held 200 USDC, which covers only 1,000 USDC of fees at 20% (400 at the 50% ceiling), so a busy market lost its discount between rebalances while the reserve still held funds. The only admin lever that moved them was the allocation cap itself, which is also each market's pool risk budget. `protocol_config::set_fee_incentive_live_target_rate` and `set_template_fee_incentive_lifetime_cap_rate` now set them within `[0, 1]`, stored in dynamic fields on `ProtocolConfig` for the same upgrade-compatibility reason as the subsidy rate, with the old fixed shares as defaults so the upgrade changes nothing until an admin sets them. *The live target is read at every rebalance,* so a change reaches markets already trading; it never claws back a balance already allocated, so lowering it only stops further top-ups, and `0` stops allocation while leaving the reserve in the pool. *The lifetime cap stays a per-market snapshot* taken when the market is created (`RegisteredExpiry.fee_incentive_lifetime_cap` already stores it), so a change reaches only later markets; on Mainnet's 1- and 5-minute cadences that is within minutes, and it needs no new per-market state. *The live target may not exceed the lifetime cap* (`EFeeIncentiveLiveTargetExceedsLifetimeCap`), checked in both setters against the other's current value; above it the cap would bind first and the target would read as a promise the cap breaks. *Neither is valuation-gated:* the flush reads neither, and the reserve and the market balances they move between are outside PLP NAV. Both setters emit `FeeIncentiveAllocationRatesUpdated` with both post-state values. *Rejected:* per-cadence settings (new per-cadence dynamic fields, and nothing yet needs the 1- and 5-minute cadences to differ); a live lifetime cap recomputed at every rebalance (leaves the stored snapshot as dead state for a difference measured in minutes on current cadences); moving the allocation cap to fund incentives (it is the market's risk budget).
