...
- **The skew escrow is senior to the trading fee on a close.** `process_live_close` subtracts the skew charge from `redeem_amount` first and clamps the trading fee against what remains, rather than clamping the fee at the full payout and asserting the charge fits afterwards. *Rationale:* the fee clamp exists because an expiry-ramped fee can exceed a deep out-of-the-money redeem, and a close must never cost more than it releases; stacking the charge on top of that clamp meant any skew charge on such a close aborted, stranding a position the clamp was written to keep closable. The escrow backs future rebates rather than being revenue, and `move.md` orders trader backing ahead of protocol revenue, so the charge is the senior claim. *Consequence:* the pool trades fee revenue for escrow in that regime, which is neutral overall — residual escrow becomes pool surplus at settlement. `ESkewChargeExceedsCloseProceeds` survives for the residual case where the charge exceeds the whole payout; `predeploy/response-policies.md` RP-29 owns that response.
