...
## No-shares USDC additions to the pool (2026-09-21)

- **`plp::add_usdc_without_shares` joins USDC to idle and mints nothing, and the protocol takes no cut of it.** Incentives, operator top-ups, and any outside party paying the pool previously had no entrypoint: `request_supply` mints offsetting shares and leaves the mark where it was, `lock_capital` is one-time and admin-only, and `sponsor_fee_incentives` funds a reserve deliberately excluded from NAV. The addition joins `Ledger.idle_balance` — the same balance the flush drain reads — and leaves `total_supply` untouched, so the whole amount falls inside `gross_pool_value` under an unchanged share count and raises NAV per PLP for every current holder. It does not touch the profit basis: `profit_basis_debits`/`credits` track cash sent to and returned from expiries, so an outside gift is neither, `lp_pool_value` leaves `exclusion` unchanged, and the protocol reserve's default 10% share of profit takes none of it. *Consequence:* the added USDC is ordinary pool capital from the instant it lands — fundable into expiries, payable to withdrawals, and indistinguishable from fee income once in idle; the only record of who funded it is `UsdcAddedWithoutShares`. *Rejected:* crediting the addition to `profit_basis_credits` so the protocol shares in it (an outside gift is not expiry P&L, and it would make the reserve's cut depend on donor generosity); an `idle_balance_after` field on the event (idle has no canonical post-state stream — `ExpiryCashRebalanced` moves idle without reporting it — so a second source would drift, per D035); reusing `min_supply_request` as the floor (it is a queue-admission policy that may diverge; `min_usdc_addition` is its own knob at the same 10 USDC).
- **Refused for the whole flush (`assert_not_valuation_in_progress`), not merely the open snapshot stage.** `rebalance_expiry_cash` is allowed mid-window because the seal froze idle alongside every market's stamped cash, so an in-window move genuinely cannot reach the mark. An external addition is different: it is new value, not a move between two frozen figures, so landing before the seal pays that flush's queued withdrawals and landing after it does not — decided by nothing but when the contributor's transaction executed. Refusing it for the flush's whole life makes every addition fully captured by the next snapshot, which is also why it needs no `ESnapshotStageOpen` check of its own. *Rejected:* the `assert_snapshot_stage_closed` gate `rebalance_expiry_cash` uses (it admits exactly the post-seal window whose payout ambiguity is the problem).
