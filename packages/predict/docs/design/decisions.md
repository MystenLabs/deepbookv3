...
## Inventory skew (2026-08-20)

- **Skew is a second charge alongside capital occupancy, not a replacement for it.** `inventory_skew_rate` prices the standard deviation of the payout profile over a window centred on the market's reference tick; `inventory_impact_max_rate` continues to price the reserve a position ties up. *Rationale:* the two measure different scarce things. Occupancy reads `L = M + lambda(T-M)`, which confounds level and shape — `M` already carries most of the concentration signal, so a trade that flattens the book and one that fills an idle corner are charged identically whenever both leave `M` fixed and add the same to `T`. Skew reads shape alone. *Rejected:* replacing occupancy's input with a shape measure — that would stop charging for buffer consumption, which a balancing trade genuinely causes. *Rejected:* stacking a shape term onto `L` — `M` is already inside it, so concentration would be priced twice through one rate.
- **The charge is the change in one book-level statistic, and it carries its own sign.** A mint that flattens the book is rebated and a close that unbalances it is charged, so the direction cannot be implied by the flow. *Consequence:* this is the first Predict adjustment whose sign is data rather than lifecycle. *Rejected:* a range-local skew multiplier, for the reason D032 already records — its rates depend on other ranges, so a cross-range cycle extracts value. The statistic here is a function of book state, so cycles cancel exactly.
- **Standard deviation, not log-sum-exp.** *Rationale:* measured discrimination between balancing and concentrating flow was better (-0.41 against -0.15), and it needs only the existing `sqrt_down` rather than `exp`/`ln`, a max-offset overflow trick, and a lazily-propagated exponential tree. Its accumulator updates are exact integer algebra, so telescoping is exact rather than approximate. The variance is taken in the single-division form for accuracy rather than for sign safety — both forms are non-negative in integer arithmetic, but dividing first loses each mean's fractional part before squaring, an error of order `max(W)`. *Rejected:* probability-weighted variance read live — the two legs of a round trip would be scored under different probabilities, so cycles would stop cancelling.
- **Translation invariance is what closes the guaranteed-payout trade.** Adding the same payout at every price in the window leaves the statistic unchanged, so buying every outcome at once earns no rebate. This is structural rather than a tuned bound.
- **The window is a fraction of the reference tick, scaled by the square root of the tenor.** A tick is a price over `tick_size`, so a fraction of the reference tick is the same fraction of price and one configured value spans every underlying and price level. Scaling by `sqrt(cadence_period / one_day)` makes the window cover the same number of standard deviations at every cadence, so a new cadence needs no new configuration. *Rejected:* a per-cadence tick count — it needs maintenance per underlying and silently goes wrong when price level or tick size changes. *Known limitation:* the square-root-of-time rule assumes independent, identically distributed returns; crypto returns are fat-tailed and cluster, which makes the true short-horizon deviation larger than the rule implies, so the derived window runs slightly narrow at short tenors. The anchor is sized with headroom rather than at the target. The rule is also cadence-invariant but not volatility-invariant: the fraction is a percentage, so a sustained regime shift moves the coverage and the anchor has to be retuned. Deriving the anchor from implied volatility read once at market creation would remove that, at the cost of an oracle dependency in market setup.
- **The window freezes with the reference tick.** A moving window would score the two legs of a round trip against different domains and break telescoping. *Consequence:* a market whose spot travels outside its window measures skew over a region the action has left, and the statistic decays toward zero. The failure is graceful — it under-charges rather than mis-charging.
- **A window narrower than one tick reads zero.** Skew over a handful of settlement outcomes is undefined, and no charge is the safe response. *Consequence:* a market whose plausible range spans too few ticks carries no skew charge at all.
- **Skew charges sit in their own escrow, released at settlement.** `required_cash = payout_liability + inventory_impact_reserve + skew_reserve`, and `free_cash` nets out both. Each adjustment is the difference of two rounded potentials, not the rounded difference, so cumulative collections equal the current potential exactly: a rebate can never exceed what the same book already paid in, no pre-funding is needed, and the reserve cannot go negative. Rounding the difference instead lets collections fall a unit short per trade, and `assert_cash_backing` then rejects a legitimate trade.
- **The window anchor is 15% daily, chosen on escape rate rather than fee level.** *Rationale:* the window is frozen at creation, so spot leaving it is the one failure a market cannot recover from — the statistic decays and under-charges for the rest of the market's life. Replay over Feb–Jun 2026 BTC puts path escape at 1.7% per market at 10% and 0.4% at 15%, and at 4.2% versus 1.3% in the most volatile month. *Consequence, and the reason this is nearly free:* widening the window barely moves the charge. A one-sided digital runs to the end of the ladder, so the fraction of the window it covers does not change when the window widens; measured, 10% → 20% moves the mean charge by 1.03–1.10x for digital flow and 0.84x for bounded-range flow, not the `1/sqrt(N)` a fixed-width range would suggest. So the window is sized on escape and the rate is sized on steering, near-independently. *Rejected:* 10%, which the σ-coverage framing had suggested — coverage is ~3.8σ there, not the ~5σ assumed, and realised escapes run 40–90x the Gaussian rate at that width. Measurements: `predeploy/evidence/dbu732-skew-calibration-2026-08-20.md`.
- **The rate ceiling is a safety bound, not a representability envelope.** `max_inventory_skew_rate` is `0.5%`, not `100%`. *Rationale:* the statistic is uniform over ticks while every amount it is checked against — premium, redeem proceeds, the fee — is proportional to the contract's probability. A marginal adjustment is `rate * c * quantity` with `c = sqrt(f(1-f)) <= 1/2`, so it does not scale with probability. Once `rate * c` passes a low-probability contract's premium, the wing positions this mechanism exists to encourage stop being mintable, and worse, stop being closable — both guards abort with no clamp. A partial close does not rescue the case that actually reaches the abort: the charge is convex in the quantity closed, so it is never worse per unit at smaller sizes and is strictly better whenever the profile is uneven across the closed range — but the state that makes a close maximally unbalancing is a book already flat over the window, and there the charge is exactly linear, so at a zero inventory-impact rate every fraction aborts alike. With impact enabled a smaller close does help: its rebate is concave in the closed quantity, so the proceeds per unit rise as the size falls while the charge per unit stays flat. Holding the rate at or below `0.5%` keeps `rate * c` at or below `0.25%`, half the default `min_fee`, so the ordinary fee dominates any rebate: wing trades stay admissible, closes stay affordable, and harvesting the escrow stays unprofitable. *Consequence:* the bound is relational — raising `min_fee` or `min_entry_probability` widens the margin and lowering either narrows it, so the two must be read together.
- **The measure is direction-blind, deliberately.** Trimming a peak and filling a dip both flatten the profile and both earn a rebate, even though only the first reduces what the pool can owe. *Rationale:* a direction-aware measure would need the pool's own position sign, which is not what a payout profile encodes. *Consequence:* a trade that raises required cash backing can still be rebated; the rate ceiling is what keeps that rebate below the trade's own fee. *Descoped:* an asymmetric measure that only pays for peak reduction.
- **A mint has no outbound leg, so a skew rebate reduces the withdrawal rather than paying the trader.** If the rebate would exceed everything else the mint owes, the trade aborts (`ESkewRebateExceedsMintCost`) rather than clamping. *Rationale:* a clamp would move the potential without crediting the trader the difference, which is the defect that made the earlier finite-reserve design path-dependent. The abort is unreachable at sane rates, since the trading fee alone is floored well above any plausible rebate. *Descoped:* a true net-credit mint, which needs a signed quote shape, a `min_proceeds`-style floor in place of `max_cost`, and a payout leg on the mint path.
- **The skew escrow is senior to the trading fee on a close.** The close path clamps the fee, builder fee and penalty at `min(redeem_amount, gross_proceeds - skew_charge)` rather than at `redeem_amount` alone. The first term is the pre-existing rule that the fee is charged against the payout and never against the trader's own returning escrow; the second makes the charge the senior claim. *Rationale:* the fee clamp exists because an expiry-ramped fee can exceed a deep out-of-the-money redeem, and a close must never cost more than it releases; stacking the charge on top of that clamp meant any skew charge on such a close aborted, stranding a position the clamp was written to keep closable. The escrow backs future rebates rather than being revenue, and `move.md` orders trader backing ahead of protocol revenue, so the charge is the senior claim. *Consequence:* the seniority cap binds only when the payout alone cannot cover both, so a close whose inventory rebate absorbs the charge pays exactly the fee it would have paid without skew. Clamping the fee against `redeem_amount - skew_charge` instead would divert that fee to the closer, since the escrow is funded from the payout under either ordering — the fee reduction would not reach it. `ESkewChargeExceedsCloseProceeds` survives for the residual case where the charge exceeds the whole payout; `predeploy/response-policies.md` RP-29 owns that response.
