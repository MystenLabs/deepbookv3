...
- **Skew charges sit in their own escrow, released at settlement.** `required_cash = payout_liability + inventory_impact_reserve + skew_reserve`, and `free_cash` nets out both. Cumulative collections equal the current potential by construction, so a rebate can never exceed what the same book already paid in — no pre-funding is needed and the reserve cannot go negative.
