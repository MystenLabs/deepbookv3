...
- **A contribution is refused when it would leave pool cash above the executable price ceiling (`EContributionExceedsPriceCeiling`).** This is the gate that makes the entrypoint safe to expose permissionlessly. RP-2 refunds every supply and withdraw head at a mark outside `[0.01, 100]` USDC/PLP, and `total_supply` grows only through a supply fill, so an above-ceiling pool can never price its way back down — the state is terminal. RP-2's risk profile rests on that state being unforceable ("attacker must win oracle-priced bets"); a contribution moves pool value up by an arbitrary amount with no bet to win, and against the 10 PLP genesis-lock share base about 1,000 USDC would have ended the pool's LP layer permanently, for anyone. The guard applies the band's upper test — `ceil(cash/band) <= supply`, the same comparison `lp_book::is_executable_mark` makes — to the pool cash the contribution is about to create: idle plus the net cash deployed into active expiries (`pool_accounting::deployed_expiry_cash`). It restates that test rather than sharing a symbol with `lp_book`, deliberately: extracting it would have put a no-behavior-change refactor in front of reviewers of a behavior change. The two definitions are instead held together behaviourally by `a_contribution_to_the_price_ceiling_is_accepted_and_the_pool_still_fills`, which contributes to the exact ceiling this guard admits and then requires a flush to fill there — a band that moved on either side fails it, which was verified by mutating `is_executable_mark` and watching that test go red. Both read `constants::executable_price_band_factor`, so the band width itself is already single-homed. *Why pool cash and not pool NAV:* NAV needs a flush to compute, and pool cash is precisely the protocol-controlled share of it. Idle alone is not: the mark counts cash the pool moved into markets, and `rebalance_expiry_cash` is permissionless, so an idle-only test could be emptied into a market and passed again (found in the external review of #1315; the first version shipped with it); the guard's duty is to stop this entrypoint from *manufacturing* the degenerate ratio, which is the maintainable direction RP-2 already names, while market-driven NAV moves stay owned by RP-1/RP-2. *Why a one-sided test:* a contribution can only move the price up, so the band's floor is unreachable from here. *Why an abort is on-ladder:* this is a single-user, voluntary, user-recoverable action, not a shared or mandatory path — the contributor simply contributes less, or supplies real liquidity first and contributes after. *Consequence:* the admissible contribution scales with the share base (100x supply, minus current pool cash), which constrains seeding a nearly empty pool and is slack for a funded one. *Rejected:* leaving it to RP-2 at the fill site (RP-2 responds to a degenerate mark that already exists; it cannot un-brick one this entrypoint created); bounding a single contribution as a fraction of pool value (repeated contributions walk past any per-call fraction); gating the entrypoint behind `AdminCap` (it would make the path operator-only, which is the opposite of its purpose).
