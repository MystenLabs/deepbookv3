...
## Inventory skew: the probability-weighted, windowless statistic (DBU-732)

- **The skew measure is the probability-weighted standard deviation of the payout profile, with weights frozen at the market's first mint.** `D(W) = sqrt(sum q*W^2 - (sum q*W)^2)` under the frozen measure; the charge on a trade is the difference of the potential `rate * D(W)` across the trade, so it is a state function: cycles telescope exactly and a round trip refunds to the unit. *Rejected — an unweighted spread over a fixed price window:* the window is a parameter that has to be sized, frozen in the right place, kept above one tick of resolution, and re-derived whenever it changes because the charge scales as `1/sqrt(N)` with window width; exposure parked just past its edge scores zero; and gating mint on the window's availability created a stranding class (a market whose reference tick never lands is unmintable). Weighting by probability deletes the window and every defect it owned. *Rejected — an entropic (weighted-LMSR) form:* the only form that is monotone at any rate with bounded worst-case loss, but its fold is multiplicative where the payout index is additive, round trips are not exactly invertible, and on measurement it never beat the weighted spread at any tolerance.
- **The measure freezes from the first mint's own validated pricer, atomically with that mint.** Before it lands, quoting from the live surface is quoting from the exact snapshot the mint installs, so the two paths cannot disagree; the book is empty at the freeze by construction, so every payout-index boundary ever created carries a weight from the one frozen surface. No admin step, no keeper, no market-launch ordering constraint. A zero-rate market — the rate is snapshotted per market at creation — never freezes and never evaluates a weight.
- **The accumulators are carried raw — payout times 1e9-scaled mass, division deferred to the read.** A fold that divided would floor differently on open and close and ratchet residue upward forever (measured on the divided form: fifty open-close cycles left an empty book scoring 273,861). Raw products make the fold exact by construction; `inventory_weighted_tests` pins the ragged round trip.
- **The payout index carries the weighted moments the read needs.** Each boundary node stores its frozen weight, subtree summaries carry `sum(weight * delta)` per side, and `range_weighted_payout_sum` resolves a range's `sum q*W` in `O(log n)` by the same two reads `range_max_payout` performs — the weighted analog of the tick-weighted first moment, with the frozen survival function replacing the tick coordinate, which is also what makes unbounded legs finite.

## Capital-occupancy charge removed in favor of the skew charge (DBU-732)

- **The inventory-impact (capital-occupancy) mechanism is removed; the probability-weighted inventory-skew charge is the one inventory mechanism.** The occupancy potential `phi(L)`, its snapshotted scale, its isolated reserve, its config rate and template setter, its quote and event fields, and the payout tree's `complement_max_payout` read all go. *Rationale:* the two mechanisms priced different objects — occupancy priced gross cash held, skew prices the payout profile's shape — but running both means two escrows, two rates to calibrate jointly, and a combined trader cost whose composition is harder to reason about than either part; the product decision is one inventory charge, and skew is the one with the measured risk signal (occupancy's measured steering was basis-point scale and largely a size proxy). *Consequence:* nothing prices capacity occupancy now — a flat complete-set book pays only ordinary fees while it holds backing — and the per-cadence `max_expiry_allocation` funding cap in pool accounting is the surviving control on capacity. *Reopen when:* capacity squatting is observed in practice; the occupancy potential returns as designed, and RP-30's duty inventory lists what comes back with it.
- **Off-chain consumers:** `OrderMinted` loses `inventory_impact_charge`, `LiveOrderRedeemed` loses `inventory_impact_rebate`, `MarketCreated` and `StrikeExposureTemplateConfigUpdated` lose `inventory_impact_max_rate`, and the public reads `inventory_impact_reserve`/`inventory_impact_max_rate`/`inventory_impact_scale`/`inventory_impact_charge(quote)` are deleted. Pre-deploy ABI change, no migration.

## Inventory measure: freeze the shape, re-anchor the centre (DBU-732)

- **The probability measure the inventory charge integrates against has its *shape* frozen at the market's first priced mint and its *centre* re-read from every later trade's own forward.** *Rationale:* a measure frozen at both goes stale as soon as spot moves — after a few standard deviations every strike the market can still trade sits in a tail the frozen measure calls near-impossible, so the charge decays toward zero exactly when the book is accumulating one-sided risk (measured: tracking of remaining-life risk falls from 0.94 to 0.40 once spot has travelled 3σ). Only the shape needs the volatility surface; the centre is the forward, which every trade already carries in a validated pricer. Re-anchoring therefore needs **no keeper** and recovers essentially all of the lost tracking (0.94 at 3σ, against 0.96 for a fully live measure). *Rejected — a keeper that refreshes the whole measure on an interval:* it buys marginally better tracking for a recurring operational dependency, a re-cut drift in the charge, and a market-launch ordering constraint. *Rejected — leaving both frozen:* simplest and cheapest, but it is the failure this entry exists to fix.
- **Re-anchoring is an integer cell shift, not a recomputation.** The measure is mirrored on a lattice uniform in log price, so translating the centre translates the whole mass profile along the lattice axis. The stored cumulative masses are read at a shifted index; no surface is evaluated after the lattice is built. Mass that shifts past either end folds into that end's sentinel, so the measure still sums to one under any shift.
- **The book is mirrored as difference arrays, so a trade touches two entries.** An order raises the profile at one cell and drops it at another; the profile is recovered by a running difference during the same scan that computes the statistic. That is what keeps the per-trade cost independent of how many cells an order spans.
- **The charge is charge-only: no rebate.** *Rationale:* a rebate is only coherent when the measure between the two legs is the same. Re-anchoring moves the measure between trades, so a refund computed under a later centre would not be the amount collected. Charging only is the honest form, and it deletes the escrow, its rebate guards, the seniority ordering on the close path, and the abort that ordering needed (RP-29, retired). *Consequence:* flattening the book is free rather than paid, so the mechanism steers by making concentration expensive rather than by paying for balance. *Reopen when:* evidence shows flow responds to rebates enough to justify re-introducing the escrow.
- **Sizing.** 256 cells across ±10 at-the-money volatilities. Measured against the exact continuous statistic, that lattice carries about 1% median error and 6% at the 99th percentile, against a charge that is itself a low single-digit percentage of the ordinary trading fee; doubling the cell count halves the error and doubles the per-trade byte cost. The span is measured in the market's own volatility so one constant serves every cadence and price level.
