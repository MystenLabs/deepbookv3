...
| --- | --- |
| `AdminCap` (on `ProtocolConfig`) | All template values (future markets only), all live configs (`PricingConfig`, `EwmaConfig`, `StakeConfig`), `protocol_reserve_profit_share`, `plp_supply_fee_rate` / `plp_withdraw_fee_rate`, `utilization_max_multiplier` / `utilization_threshold`, the `trade_liquidation_budget`, the LP-queue liveness knob `lp_request_limit_flush_attempts` (`set_lp_request_limit_flush_attempts`; ships at `1`, fill-or-kill — RP-12), the LP pool-value ceiling `max_lp_pool_value` (`set_max_lp_pool_value`; ships uncapped — RP-23), global `trading_paused`, the emergency `frozen` switch (`set_frozen`), the version watermark (`bump_version_watermark`) |
