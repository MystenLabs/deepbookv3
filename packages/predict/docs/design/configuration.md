...
- **`StakeConfig`** — the DEEP staking benefit curve thresholds (`lower_benefit_power`, `upper_benefit_power`) plus an `enabled` master switch. The benefit ratio rises linearly from 0 to half over `0..lower`, half to full over `lower..upper`, and caps at full above `upper`. That ratio scales the fixed maximum fee discount and applies directly as the loss-rebate share. Benefits are disabled by default, so the ratio is zero at every stake and both benefits are inert until an admin enables them; the thresholds keep their values across a toggle. Read live so that enabling the programme, or a benefit-curve change, applies to all stakers at once.
...
| --- | --- |
| `AdminCap` (on `ProtocolConfig`) | All template values (future markets only), all live configs (`PricingConfig`, `EwmaConfig`, `StakeConfig`), `protocol_reserve_profit_share`, `plp_supply_fee_rate` / `plp_withdraw_fee_rate`, the `trade_liquidation_budget`, the LP-queue liveness knob `lp_request_limit_flush_attempts` (`set_lp_request_limit_flush_attempts`; ships at `1`, fill-or-kill — RP-12), the LP pool-value ceiling `max_lp_pool_value` (`set_max_lp_pool_value`; ships uncapped — RP-23), the DEEP-stake benefit switch (`set_stake_benefits_enabled`; ships disabled), global `trading_paused`, the emergency `frozen` switch (`set_frozen`), the version watermark (`bump_version_watermark`) |
