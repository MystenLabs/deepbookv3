...
| `MarketLifecycleCap` (Registry allowlist) | Create expiry markets; also the sole authority to start the pool flush (`start_pool_valuation`); a fresh start discards any in-flight flush, so there is no separate abort entrypoint. No oracle-write or config authority |
