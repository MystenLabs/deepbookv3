...
- `plp_request_entry_fee_rate` — non-refundable fee charged when an LP supply or withdraw request is queued, in FLOAT_SCALING. Ships at 5 bps inside a hard `0..50 bps` envelope (`0` disables it). Same rate on both legs; accrues to the pool (DUSDC into idle on supply; PLP burned on withdraw). Valuation-locked. Independent of any fill-time PLP fee.
...
| --- | --- |
| `AdminCap` (on `ProtocolConfig`) | All template values (future markets only), all live configs (`PricingConfig`, `EwmaConfig`, `StakeConfig`), `protocol_reserve_profit_share`, the LP request entry fee `plp_request_entry_fee_rate` (`set_plp_request_entry_fee_rate`; ships at 5 bps), the `trade_liquidation_budget`, the LP-queue liveness knob `lp_request_limit_flush_attempts` (`set_lp_request_limit_flush_attempts`; ships at `1`, fill-or-kill — RP-12), the LP pool-value ceiling `max_lp_pool_value` (`set_max_lp_pool_value`; ships uncapped — RP-23), global `trading_paused`, the emergency `frozen` switch (`set_frozen`), the version watermark (`bump_version_watermark`) |
