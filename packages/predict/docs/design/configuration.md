...
- `plp_fee_rate` — the fee charged on executed PLP supply and withdraw fills, in 1e9 scaling, inside a hard `0..5%` envelope. Charged on the DUSDC leg in both directions (deducted from the DUSDC taken in before shares are priced on supply, withheld from the payout on withdraw) and retained by the pool, so it accrues to remaining PLP holders rather than the protocol reserve. Read once per flush into the frozen mark, so every fill in one flush is charged the same rate; request-time `min_plp_out` / `min_dusdc_out` limits are measured after the fee, and a partial fill's rescaled limit is too. Locked during valuation like the other economic setters.
...
| --- | --- |
| `AdminCap` (on `ProtocolConfig`) | All template values (future markets only), all live configs (`PricingConfig`, `EwmaConfig`, `StakeConfig`), `protocol_reserve_profit_share`, `plp_fee_rate`, the `trade_liquidation_budget`, the LP-queue liveness knob `lp_request_limit_flush_attempts` (`set_lp_request_limit_flush_attempts`; ships at `1`, fill-or-kill — RP-12), the LP pool-value ceiling `max_lp_pool_value` (`set_max_lp_pool_value`; ships uncapped — RP-23), global `trading_paused`, the emergency `frozen` switch (`set_frozen`), the version watermark (`bump_version_watermark`) |
