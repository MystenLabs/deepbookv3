...
- **USDC settlement asset has 6 decimals**; contract quantities are 6-decimal quote units, so `1_000_000` is one contract.
...
The shipped `StrikeExposureConfig` template initializes `base_fee` to `0.1`, `min_fee` to `0.022`, and `backing_buffer_lambda` to `0.31`. At `p = 0.5`, the Bernoulli term turns the `0.1` base coefficient into a `0.05` pre-ramp fee rate; near either probability edge, the `0.022` floor applies.
...
- `trading_paused` — when true, blocks *new risk creation*. Exits, settlement cleanup, and valuation are intentionally not blocked by the trading pause. `assert_trading_allowed` is exactly the not-paused check; the valuation flag gates a separate, non-trading set of flows (next bullet).
...
- `valuation_in_progress` — true for the whole multi-transaction span of a full-pool valuation, alongside the monotonic flush ordinal `flush_seq`. `begin_valuation`/`end_valuation` engage and release it (finish and abort both release). While engaged, `sponsor_fee_incentives`, LP request cancels, and most config setters abort; trading flows, `rebalance_expiry_cash` (refused only inside the still-open snapshot stage; post-seal it cannot reach the mark, whose every figure was frozen at the seal), and market creation (a new market is outside the flush's frozen expected set and joins the next snapshot) are deliberately **not** gated — they read the flag (with `flush_seq`) only to discard a stale stamp lazily, since a pending market's snapshot state is captured rather than recorded per trade. Most admin setters first assert no valuation is in progress so that policy cannot shift mid-valuation; the exceptions are the gates and controls a stalled flush must not be able to trap (`set_trading_paused`, `set_frozen`, `set_no_trade_window_ms`, `set_referral_fee_rate`, `set_fee_incentive_subsidy_rate`, and the settled-redeem keeper allowlist setters), none of which the flush reads.
- `max_valuation_window_ms` — how long a started full-pool valuation may stay in flight before `plp::finish_flush` refuses it (for everyone, including the operator). Defaults to 5 minutes, bounded 1 minute–4 hours. A stalled flush never pauses trading — it defers queued LP fills and the flush-set markets' settlement — so this window bounds LP-fill latency and the staleness of the frozen mark those fills execute at; the minimum keeps a working keeper's flush from being refused under it, the maximum bounds the wait even under a careless setting. Recovery past the window is a fresh `start_pool_valuation`, which discards the stalled flush and re-snapshots. See RP-29.
- `no_trade_window_ms` — a window before each market's expiry in which live quotes, mints, and live redeems abort (`ETradeWindowClosed`). Defaults to 2 seconds, bounded `0`–15 seconds, and `0` disables the block. It exists because a binary's probability moves further per unit of spot as expiry nears, so a fixed oracle staleness is worth progressively more the closer to expiry it is used — the edge scales as `sqrt(staleness / time_to_expiry)` and grows without bound, which no finite fee prices and no achievable oracle cadence removes. Settlement and settled redemption are not gated on it, so a holder blocked from closing rides to settlement rather than being stranded. Unlike the template values it is read live at trade time rather than snapshotted at market creation, so widening it takes effect on markets already trading.
- Settled-redeem keeper allowlist — the addresses allowed to call `expiry_market::redeem_settled_permissionless`, which redeems any account's settled order into that account without the owner's signature. It is stored as a dynamic field on `ProtocolConfig` because it was added after deploy, and it ships empty, so the keeper path is closed until admin adds an address. `add_settled_redeem_keeper` is version-gated like other grants; `remove_settled_redeem_keeper` bypasses the version gate so a compromised keeper can be dropped under the freeze. Each change emits `SettledRedeemKeeperUpdated`. The owner-auth `redeem_settled` path never consults it.
- `protocol_reserve_profit_share` — the merged protocol-and-insurance reserve share used when aggregate expiry profit is materialized, in 1e9 scaling. It defaults to 10%; the complement accrues to LPs.
- `referral_fee_rate` — the share of a referred mint's trader-paid trading fee and congestion surcharge routed to the referrer, in 1e9 scaling. It defaults to 10%, is bounded from 0% through 25%, and is read on every mint rather than snapshotted into Accounts or expiry markets.
- `fee_incentive_subsidy_rate` — the fraction of each mint's trading fee paid from the market's sponsor-funded fee-incentive balance, in 1e9 scaling. It defaults to 20%, is bounded from 0% through 50%, and is read on every mint rather than snapshotted, so a change reaches markets already trading; `0` stops incentives from being spent without moving them. The 50% ceiling is a policy cap: a trader always pays at least half of every trading fee, so a promotion cannot make wash volume free. Any ceiling must also stay at or below 100%, which keeps the subsidy from exceeding the fee and keeps `mint_exact_cost`'s budget search exact (RP-36). It became admin-tunable after Mainnet deploy, so it is stored in a dynamic field on `ProtocolConfig` (`FeeIncentiveSubsidyRateKey`) rather than a struct field; a config that has never set it reads the default, which is the fixed rate earlier package versions charged. Those earlier versions never read the field, so the setting binds every mint only once the version watermark has retired them. Each change emits `FeeIncentiveSubsidyRateUpdated`.
...
- `max_lp_pool_value` — a ceiling on LP-attributable pool value that queued supplies may raise the pool to, applied to the supply pass at the frozen flush mark. Defaults to 500,000 USDC. A supply larger than the remaining headroom fills up to the cap and keeps its unfilled balance queued at the head; one with no room left waits. It bounds pool *value*, not cumulative deposits, so trading profit alone can carry a capped pool above its ceiling — see RP-23.
- `plp_supply_fee_rate` / `plp_withdraw_fee_rate` — the fees charged on executed PLP supply and withdraw fills, in 1e9 scaling, each inside a hard `0..5%` envelope. **They ship asymmetric: supply `0`, withdraw 20 bps.** An exit concentrates the pool's outstanding risk on the holders who stay; a deposit dilutes it, so entry is not taxed and the supply knob exists only to keep that reversible. Charged on the USDC leg (deducted from the USDC taken in before shares are priced on supply, withheld from the payout on withdraw) and retained by the pool, so the charge accrues to holders rather than the protocol reserve. Both are read once per flush into the flush's frozen `FeeRates`, which rides alongside the mark rather than inside it, so every fill in one flush is charged the same pair; request-time `min_plp_out` / `min_usdc_out` limits are measured after the fee, and a partial fill's rescaled limit is too. Locked during valuation like the other economic setters.
...
1. The setter asserts no valuation is in progress.
...
Defaults are applied only in the module that constructs the config; runtime logic treats config fields as plain numbers and never reads the `default_*` seeds. The exception is a value made tunable after deploy and stored in a dynamic field (`fee_incentive_subsidy_rate`): its `ProtocolConfig` getter returns the default until the field is first set, so no migration transaction is needed. Bounds (`min_*`/`max_*`) may also be read directly by runtime logic when they intentionally serve as a hard floor or ceiling, but there are no config fields or getters for the bounds themselves.
...
| --- | --- |
| `AdminCap` (on `ProtocolConfig`) | All template values (future markets only), all live configs (`PricingConfig`, `EwmaConfig`), `protocol_reserve_profit_share`, `referral_fee_rate`, the live `fee_incentive_subsidy_rate` (`set_fee_incentive_subsidy_rate`; ships at 20%), `plp_supply_fee_rate` / `plp_withdraw_fee_rate`, the LP-queue liveness knob `lp_request_limit_flush_attempts` (`set_lp_request_limit_flush_attempts`; ships at `1`, fill-or-kill — RP-12), the LP pool-value ceiling `max_lp_pool_value` (`set_max_lp_pool_value`; ships at 500,000 USDC — RP-23), the flush finish deadline `max_valuation_window_ms` (`set_max_valuation_window_ms`; ships at 5 minutes — RP-29), the pre-expiry `no_trade_window_ms` (`set_no_trade_window_ms`; ships at 2 seconds, `0` disables), global `trading_paused`, the emergency `frozen` switch (`set_frozen`), the version watermark (`bump_version_watermark`), the settled-redeem keeper allowlist (`add_settled_redeem_keeper` / `remove_settled_redeem_keeper`; ships empty) |
| `AdminCap` (on an `ExpiryMarket`) | Per-expiry `mint_paused` (set and unset) |
| `AdminCap` (on `Registry`) | Register a Propbook underlying, set cadence deployment configs, PauseCap mint/revoke, market-lifecycle-cap mint/revoke, pool-valuation-cap mint/revoke |
| `AdminCap` (on `PoolVault`) | Genesis-bootstrap the pool (`lock_capital`); withdraw from the sponsor-funded fee-incentive reserve (`withdraw_fee_incentives`), which is outside PLP NAV |
...
| `MarketLifecycleCap` (Registry allowlist) | Create expiry markets. No flush, oracle-write, or config authority |
| `PoolValuationCap` (Registry allowlist) | The sole authority to start the pool flush (`start_pool_valuation`, on a registry-issued proof); a fresh start discards any in-flight flush, so there is no separate abort entrypoint. No market-creation, oracle-write, or config authority |
| Settled-redeem keeper (`ProtocolConfig` allowlist) | Redeem any account's settled order into that account (`redeem_settled_permissionless`). No other authority: the payout goes to the order's account, and the keeper keeps only the storage rebate from deleting the position |
| Permissionless | Cash rebalance and settled-market sweep flows (blocked while a flush is in flight, not by the trading pause); LP supply/withdraw requests (never gated on a flush) and their cancellation (gated while a flush is in flight); valuing and finishing an in-flight flush (`value_expiry`, `finish_flush` — permissionless once the snapshot seals; finish gated by `max_valuation_window_ms`) |
| Upgrade only | Everything in the `constants` module: scaling, lot size, minimum net premium, the dynamic admission-curve shape constant, the minimum per-expiry allocation cap, the 30-bit tick domain, and every `min_*`/`max_*` bound in `config_constants` |

All admin setters route through their owning module: global protocol policy through `protocol_config`, per-object policy through the object's own module, and only registry-owned concerns (pause caps, lifecycle caps, pool-valuation caps, uniqueness, underlying admission, and cadence deployment policy) through `registry`. The privileged pool flush is started on `plp`. The embedded config struct setters themselves are package-internal; the public, capability-gated entrypoints are the only external surface for changing policy.
