...
- `valuation_in_progress` — true for the whole multi-transaction span of a full-pool valuation, alongside the monotonic flush ordinal `flush_seq`. `begin_valuation`/`end_valuation` engage and release it (finish and abort both release). While engaged, `sponsor_fee_incentives`, LP request cancels, and most config setters abort; trading flows, `rebalance_expiry_cash` (an in-window live rebalance is recorded on the market's stamp and reversed out of the flush's mark), and market creation (a new market is outside the flush's frozen expected set and joins the next snapshot) are deliberately **not** gated — they read the flag (with `flush_seq`) to decide whether a market's valuation stamp is current and record their book deltas for the flush to roll back. Most admin setters first assert no valuation is in progress so that policy cannot shift mid-valuation; `set_max_valuation_log_ops` is the deliberate exception (below).
- `max_valuation_window_ms` — how long a started full-pool valuation may stay in flight before `plp::abort_valuation` becomes permissionless. Defaults to 10 minutes, bounded 1 minute–4 hours. A stalled flush never pauses trading — it defers queued LP fills and the flush-set markets' settlement — so this window bounds LP-fill latency and the staleness of the frozen mark those fills execute at; the minimum keeps a working keeper's flush from being discarded under it, the maximum bounds the wait even under a careless setting, and `abort_valuation_privileged` is immediate for the operator. See RP-29.
- `max_valuation_log_ops` — how many range operations one stamped market's valuation delta log may record while it awaits its `value_expiry`; a market whose log is full refuses further trades until it is valued or the flush is discarded. Defaults to 256, bounded 64–1,024. Raising it buys more mid-flush trading headroom per market at the cost of compute in that market's valuation transaction (one extra pricer evaluation per logged boundary; the log adds no object loads). Its setter is deliberately not valuation-locked, so the cap can be raised mid-flush to lift a market's trade freeze without discarding the flush.
...
1. The setter asserts no valuation is in progress (`set_max_valuation_log_ops` is the deliberate exception, so the delta-log cap can be raised mid-flush).
...
| --- | --- |
| `AdminCap` (on `ProtocolConfig`) | All template values (future markets only), all live configs (`PricingConfig`, `EwmaConfig`), `protocol_reserve_profit_share`, `referral_fee_rate`, `plp_supply_fee_rate` / `plp_withdraw_fee_rate`, the LP-queue liveness knob `lp_request_limit_flush_attempts` (`set_lp_request_limit_flush_attempts`; ships at `1`, fill-or-kill — RP-12), the LP pool-value ceiling `max_lp_pool_value` (`set_max_lp_pool_value`; ships uncapped — RP-23), the stalled-flush deadline `max_valuation_window_ms` (`set_max_valuation_window_ms`; ships at 10 minutes — RP-29), the per-market delta-log cap `max_valuation_log_ops` (`set_max_valuation_log_ops`; ships at 256, tunable mid-flush), global `trading_paused`, the emergency `frozen` switch (`set_frozen`), the version watermark (`bump_version_watermark`) |
...
| `MarketLifecycleCap` (Registry allowlist) | Create expiry markets; also the sole authority to start the privileged pool flush (`start_pool_valuation`) and to discard one immediately (`abort_valuation_privileged`). No oracle-write or config authority |
| Permissionless | Cash rebalance and settled-market sweep flows (blocked while a flush is in flight, not by the trading pause); LP supply/withdraw requests (never gated on a flush) and their cancellation (gated while a flush is in flight); discarding a flush stalled past `max_valuation_window_ms` (`abort_valuation`) |
