...
- **`PricingConfig`** — three freshness thresholds: Pyth spot freshness, Block Scholes price freshness, and Block Scholes SVI freshness. `Pricing` reads these when resolving live probabilities for mint, redeem, liquidation, and the NAV flush. Pyth-stale falls back to the Block Scholes forward, stale BS spot/forward is a hard abort, and stale SVI is a separate hard abort under a looser window. Because freshness is a protocol-safety concern rather than a contract term, every flow uses the current thresholds the instant it runs; an admin tightening freshness takes effect immediately and protocol-wide.
...
| --- | --- |
| `AdminCap` (on `ProtocolConfig`) | All template values (future markets only), all live configs (`PricingConfig`, `EwmaConfig`, `StakeConfig`), `protocol_reserve_profit_share`, the `trade_liquidation_budget`, global `trading_paused`, the emergency `frozen` switch (`set_frozen`), the version watermark (`bump_version_watermark`) |
