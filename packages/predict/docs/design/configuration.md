...
Quote calibration adds a fourth live config, `QuoteCalibrationConfig`, holding three admin-tunable values plus the corrections a keeper has published. `enabled` ships `false`, so publishing the package applies no correction and turning it on is a deliberate admin transaction; it is also the only fast way back, since there is no per-underlying discard. `staleness_ms` (six hours by default, one minute to one day) is how old a published correction may be before quotes fall back to the uncorrected formula. `max_deviation` (ten percentage points by default, zero to twenty-five) is how far a correction may move a probability, applied when a quote is priced rather than when a correction is published — so lowering it binds on the next quote, and a range quote, being a difference of two corrected probabilities, can move by up to twice it. A `QuoteCalibrationCap` is a revocable capability the admin mints into `Registry.allowed_quote_calibration_caps`; revoking it stops further publications but does not retire the standing one.
...
| --- | --- |
| `AdminCap` (on `ProtocolConfig`) | All template values (future markets only), all live configs (`PricingConfig`, `EwmaConfig`, `QuoteCalibrationConfig`), `protocol_reserve_profit_share`, `plp_supply_fee_rate` / `plp_withdraw_fee_rate`, the LP-queue liveness knob `lp_request_limit_flush_attempts` (`set_lp_request_limit_flush_attempts`; ships at `1`, fill-or-kill — RP-12), the LP pool-value ceiling `max_lp_pool_value` (`set_max_lp_pool_value`; ships uncapped — RP-23), the quote-calibration switch, staleness window and deviation cap, global `trading_paused`, the emergency `frozen` switch (`set_frozen`), the version watermark (`bump_version_watermark`) |
...
| `QuoteCalibrationCap` (Registry allowlist) | Publish one registered underlying's quote calibration correction, which every quoted probability for that underlying is then mapped through. No config, oracle-write, market-creation or custody authority; bounded by `max_deviation` and inert while the mechanism is switched off — see [risks](../risks.md) |
