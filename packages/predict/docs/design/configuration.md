...
- `valuation_in_progress` — a lock held for the whole duration of a full-pool valuation, across every transaction it spans. `begin_valuation`/`end_valuation` open and close it; while held, config mutations and new-risk flows abort. Most admin setters first assert the valuation lock is *not* in progress so that policy cannot shift mid-valuation.
...
- `max_valuation_window_ms` — how long a started full-pool valuation may stay in flight before `plp::abort_valuation` becomes permissionless. The valuation lock now spans transactions, so this is the maximum protocol pause a stalled keeper can cause: mint, redeem, liquidate, settlement and LP requests all wait on it. Ships at one hour, bounded [5 min, 4 h], and the setter refuses to move it under a flush in flight. Size it against the flush interval rather than in the abstract — at a ten-minute cadence the one-hour default is six missed cycles, while a live operator never waits it out because `abort_valuation_privileged` is immediate. See RP-27.
...
| --- | --- |
| `AdminCap` (on `ProtocolConfig`) | All template values (future markets only), all live configs (`PricingConfig`, `EwmaConfig`), `protocol_reserve_profit_share`, `plp_supply_fee_rate` / `plp_withdraw_fee_rate`, the `trade_liquidation_budget`, the LP-queue liveness knob `lp_request_limit_flush_attempts` (`set_lp_request_limit_flush_attempts`; ships at `1`, fill-or-kill — RP-12), the LP pool-value ceiling `max_lp_pool_value` (`set_max_lp_pool_value`; ships uncapped — RP-23), the abandoned-flush window `max_valuation_window_ms` (`set_max_valuation_window_ms`; ships at one hour — RP-27), the DEEP-stake benefit policy snapshotted by future markets (`set_template_max_benefit_ratio`, ships at `0`; `set_template_benefit_powers`), global `trading_paused`, the emergency `frozen` switch (`set_frozen`), the version watermark (`bump_version_watermark`) |
