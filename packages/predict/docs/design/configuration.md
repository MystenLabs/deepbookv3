...
- `max_lp_pool_value` — a ceiling on LP-attributable pool value that queued supplies may raise the pool to, applied to the supply pass at the frozen flush mark. Defaults to `u64::MAX`, so the pool is uncapped until an operator sets a figure. A supply larger than the remaining headroom fills up to the cap and has the rest returned; one with no usable room is refunded. It bounds pool *value*, not cumulative deposits, so trading profit alone can carry a capped pool above its ceiling — see RP-23.
...
| --- | --- |
| `AdminCap` (on `ProtocolConfig`) | All template values (future markets only), all live configs (`PricingConfig`, `EwmaConfig`, `StakeConfig`), `protocol_reserve_profit_share`, the `trade_liquidation_budget`, the LP-queue liveness knob `lp_request_limit_flush_attempts` (`set_lp_request_limit_flush_attempts`; ships at `1`, fill-or-kill — RP-12), the LP pool-value ceiling `max_lp_pool_value` (`set_max_lp_pool_value`; ships uncapped — RP-23), global `trading_paused`, the emergency `frozen` switch (`set_frozen`), the version watermark (`bump_version_watermark`) |
