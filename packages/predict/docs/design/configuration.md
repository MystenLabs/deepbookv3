...
- `plp_supply_fee_rate` / `plp_withdraw_fee_rate` — the fees charged on executed PLP supply and withdraw fills, in 1e9 scaling, each inside a hard `0..5%` envelope. **They ship asymmetric: supply `0`, withdraw 20 bps.** An exit concentrates the pool's outstanding risk on the holders who stay; a deposit dilutes it, so entry is not taxed and the supply knob exists only to keep that reversible. Charged on the DUSDC leg (deducted from the DUSDC taken in before shares are priced on supply, withheld from the payout on withdraw) and retained by the pool, so the charge accrues to holders rather than the protocol reserve. Both are scaled by the utilization multiplier below, clamped to the same 5% ceiling, and frozen once per flush into the mark, so every fill in one flush is charged the same pair; request-time `min_plp_out` / `min_dusdc_out` limits are measured after the fee, and a partial fill's rescaled limit is too. Locked during valuation like the other economic setters.
- `utilization_max_multiplier` / `utilization_threshold` — the peak fee multiplier at full pool utilization and the utilization at which the ramp begins, both in 1e9 scaling. Utilization is pool-wide `total_payout_liability / pool_nav` (rebalance-invariant; not idle-share or per-expiry). The multiplier is a linear ramp from 1.0 at the threshold to the max at `u = 1.0`, mirroring `expiry_fee_multiplier`. **Ships disabled:** max multiplier defaults to 1.0 so the ramp term is arithmetically zero. Envelope: max multiplier `[1.0, 3.0]`, threshold `[0, 1.0)` (strictly below full scale — a zero denominator). Locked during valuation. Each flush also writes the computed `u` and a timestamp onto `ProtocolConfig` (`cached_pool_utilization` / `cached_pool_utilization_ms`, with `utilization_cache_written`) for the trade path; that cache is only refreshable inside the capped flush.
- `utilization_cache_fresh_ms` / `utilization_cache_decay_ms` — how long the trade path trusts a cached utilization in full, and the subsequent window over which the surcharge decays linearly to zero (multiplier → 1.0). Both default to one hour; zero decay is a hard cliff at the fresh boundary. Envelope each `[0, 7 days]`. Locked during valuation. A never-written cache is multiplier 1.0.
...
| --- | --- |
| `AdminCap` (on `ProtocolConfig`) | All template values (future markets only), all live configs (`PricingConfig`, `EwmaConfig`, `StakeConfig`), `protocol_reserve_profit_share`, `plp_supply_fee_rate` / `plp_withdraw_fee_rate`, `utilization_max_multiplier` / `utilization_threshold`, `utilization_cache_fresh_ms` / `utilization_cache_decay_ms`, the `trade_liquidation_budget`, the LP-queue liveness knob `lp_request_limit_flush_attempts` (`set_lp_request_limit_flush_attempts`; ships at `1`, fill-or-kill — RP-12), the LP pool-value ceiling `max_lp_pool_value` (`set_max_lp_pool_value`; ships uncapped — RP-23), global `trading_paused`, the emergency `frozen` switch (`set_frozen`), the version watermark (`bump_version_watermark`) |
