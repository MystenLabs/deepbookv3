...
| --- | --- |
| `AdminCap` (on `ProtocolConfig`) | All template values (future markets only), all live configs (`PricingConfig`, `EwmaConfig`), `protocol_reserve_profit_share`, `referral_fee_rate`, `plp_supply_fee_rate` / `plp_withdraw_fee_rate`, the LP-queue liveness knob `lp_request_limit_flush_attempts` (`set_lp_request_limit_flush_attempts`; ships at `1`, fill-or-kill — RP-12), the LP pool-value ceiling `max_lp_pool_value` (`set_max_lp_pool_value`; ships at 500,000 USDC — RP-23), the flush finish deadline `max_valuation_window_ms` (`set_max_valuation_window_ms`; ships at 5 minutes — RP-29), the pre-expiry `no_trade_window_ms` (`set_no_trade_window_ms`; ships at 2 seconds, `0` disables), global `trading_paused`, the emergency `frozen` switch (`set_frozen`), the version watermark (`bump_version_watermark`), the settled-redeem keeper allowlist (`add_settled_redeem_keeper` / `remove_settled_redeem_keeper`; ships empty) |
