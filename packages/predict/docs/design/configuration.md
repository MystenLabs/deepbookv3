...
- **`StakeConfig`** — the DEEP staking benefit curve thresholds (`lower_benefit_power`, `upper_benefit_power`), read live, plus `template_benefits_enabled`, which is **not** live. The benefit ratio rises linearly from 0 to half over `0..lower`, half to full over `lower..upper`, and caps at full above `upper`. That ratio scales the fixed maximum fee discount and applies directly as the loss-rebate share. The thresholds are read live so a curve retune applies to all stakers at once. Whether the programme applies at all is a contract term instead: each `ExpiryMarket` snapshots `template_benefits_enabled` at creation and both benefits read that snapshot, so a toggle never reprices a market already trading. Benefits ship disabled, so the ratio is zero at every stake until a market is created under an enabled seed. This split is deliberate — the rationale is in [decisions.md](./decisions.md) § Fees, staking, and rebates.
...
| --- | --- |
| `AdminCap` (on `ProtocolConfig`) | All template values (future markets only), all live configs (`PricingConfig`, `EwmaConfig`, `StakeConfig`), `protocol_reserve_profit_share`, `plp_supply_fee_rate` / `plp_withdraw_fee_rate`, the `trade_liquidation_budget`, the LP-queue liveness knob `lp_request_limit_flush_attempts` (`set_lp_request_limit_flush_attempts`; ships at `1`, fill-or-kill — RP-12), the LP pool-value ceiling `max_lp_pool_value` (`set_max_lp_pool_value`; ships uncapped — RP-23), the DEEP-stake benefit switch snapshotted by future markets (`set_template_stake_benefits_enabled`; ships disabled), global `trading_paused`, the emergency `frozen` switch (`set_frozen`), the version watermark (`bump_version_watermark`) |
