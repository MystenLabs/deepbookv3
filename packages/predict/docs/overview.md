...
- **Cash always backs payouts.** Each expiry's `ExpiryCash` enforces, on every cash movement, that its balance is at least its payout liability. Surplus above that line is the only cash the pool may sweep. An expiry can always pay its winners.
...
- [Fees and rebates](./concepts/fees-and-rebates.md) — the variance-based trading fee, expiry ramp, builder fee, congestion surcharge, and the inventory charge.
