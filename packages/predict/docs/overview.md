...
Predict is an on-chain protocol for option-like prediction contracts on the Sui blockchain. Trading is organized into independent per-expiry markets: each market settles once, at one timestamp, against one price feed, and every position is a contract that pays out based on where the feed's price lands at expiry relative to a chosen strike range. Leverage is built into each contract as a deterministic, rising floor rather than as separate debt, and a single LP-backed pool is the counterparty to every market.

This page gives the whole mental model fast and routes onward. For the trust assumptions and known limitations behind every claim here, read [risks](./risks.md).

## What Predict is

A Predict position is a claim on whether the settlement price lands inside a strike range `(lower, higher]`. A plain (1x) position pays its full `quantity` if settlement is inside the range and `0` otherwise. Its live value before settlement is the range's model probability times its quantity.

Leverage transforms that same contract: a leveraged order subtracts a deterministic, time-varying **floor** from the range payoff, clamped at zero. The holder pays only a fraction of the contract's entry value upfront; the floor is the implied financing — the slice of value the holder did *not* pay for — that the contract must cover before the holder owns anything above it. The floor rises deterministically over time toward a terminal value as expiry approaches. A 1x order is the special case where the floor is zero, recovering the plain range payoff exactly.

This is *limited-recourse* financing. A leveraged order's floor can only ever consume that one order's own value or payout, capped at it. There is no margin call against the holder's other assets and no shared debt pool. An order that falls to or below its floor is simply worth zero to its holder and is liquidated; it never produces a negative balance the protocol must chase.
...
- **Settled redeem** pays a winning position `quantity − terminal_floor` (zero outside the range), is permissionless, and requires a full close.
- **Liquidation** removes a leveraged order whose live value has decayed to or below its floor-derived threshold. It is a permissionless, bounded-budget knockout that touches no manager and leaves a tombstone the holder later clears for zero payout.
...
- [Leverage and the floor](./concepts/leverage-and-floor.md) — the floor model, the floor index and floor shares, mint admission, and settlement payout.
