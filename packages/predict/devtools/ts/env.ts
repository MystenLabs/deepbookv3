...
export const USDC_PACKAGE_ID = requireEnv("USDC_PACKAGE_ID");
export const USDC_CURRENCY_ID = requireEnv("USDC_CURRENCY_ID");
...
export function getSignerForAddress(address: string): Ed25519Keypair {
  ...
  for (const priv of keystore) {
    ...
    if (pair.getPublicKey().toSuiAddress() === address) {
      return pair;
    }
  }

  throw new Error(`Keypair not found for ${address}`);
}

export function getSigner(): Ed25519Keypair {
  return getSignerForAddress(ACTIVE_ADDRESS);
}
