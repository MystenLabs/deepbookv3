...
`upgrade_sessions.ts` defaults to a non-broadcasting build and serialized-transaction preflight. Before signing, it binds the exact CLI, RPC, gas budgets, and compiled latest-package dependency set and verifies the live original-to-latest dependency map. `--execute` derives and journals the exact upgrade digest before signing, upgrades the pinned v1 package with the pinned `UpgradeCap`, then audits package version 2, module membership, v1 type origins, dependency linkage, capability owner/policy/version, and both Account-registry authorizations. `--execute --smoke` additionally funds the deployer's existing Account with at most 2 SUI of temporary principal, authorizes a deterministic but unrecorded ephemeral signer for five minutes, places and cancels a post-only non-crossing DEEP/SUI limit bid, executes one exact minimum-size bounded market bid against live liquidity, validates the receipt against exact Account balance deltas, revokes the session, withdraws smoke-attributable Account funds back to their recorded baselines, and returns temporary session gas. Each successful transaction and its semantic checkpoint are journaled atomically so a known digest is reconciled rather than duplicated after interruption. Only a complete audit and cleaned-up smoke may update `packages/sessions/Published.toml` and the schema-5 manifest.
