...
-   `upgrade_sessions.test.ts` pins the v2 target bindings, source-dirt boundary, UpgradeCap resume boundary, every irreversible journal transition, remaining-balance calculation, completion gate, receipt parsers, and schema-5 manifest update.
...
`upgrade_sessions.ts` defaults to a non-broadcasting build and serialized-transaction preflight. Before signing, it binds the exact protocol-compatible CLI, RPC, gas budgets, remaining execution reserve, and compiled latest-package dependency set and verifies the live original-to-latest dependency map. `--execute` derives and journals the exact upgrade digest before signing, upgrades the pinned v1 package with the pinned `UpgradeCap`, then audits package version 2, module membership, v1 type origins, dependency linkage, capability owner/policy/version, and both Account-registry authorizations. `--execute --smoke` additionally funds the deployer's existing Account with at most 2 SUI, sized for the price-limit principal plus the protocol's maximum fee and a buffer, authorizes a deterministic but unrecorded ephemeral signer for five minutes with a bounded retry reserve, places and cancels a post-only non-crossing DEEP/SUI limit bid, executes one exact minimum-size bounded market bid against live liquidity, validates the applied exact/partial result against exact Account balance deltas, sweeps any settled core amounts from a limit order that filled between checkpoints, revokes the session, withdraws smoke-attributable Account funds back to their recorded baselines, and returns temporary session gas. Expired, externally filled, failed, and partial-IOC attempts are cleaned and may restart up to the journaled three-attempt bound; each fresh attempt rechecks its remaining deployer reserve. Each successful transaction and its semantic checkpoint are journaled atomically so a known digest is reconciled rather than duplicated after interruption. Only a complete exact-fill audit with zero pool custody and recovered temporary funds may update `packages/sessions/Published.toml` and the schema-5 manifest.
