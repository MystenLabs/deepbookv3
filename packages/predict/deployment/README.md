...
-   `deploy.ts` is the resumable deployment state machine. It publishes `fixed_math`, Account, Propbook, Predict, the DeepBook core Account wrapper, and Sessions; authorizes their apps; wires BTC oracle state; applies cadence policy; bootstraps the pool; fills the initial market windows; transfers the lifecycle capability; and audits the result from Testnet.
...
The first command builds every package, verifies the package plan and dependency identities, checks the chain, signer, external objects, capability owners, gas, and DUSDC funding, and submits no transaction. The second command broadcasts.
...
The existing DeepBook registry's consensus-owned `DeepbookAdminCap` has a different owner. After the fresh core wrapper is published, the workflow stops unless that owner has authorized the new `DeepbookCoreAccountApp` type on the existing DeepBook registry. Perform that one external authorization, then rerun the same execute command; verified package checkpoints are reused.

After authorization, the workflow creates and binds the BTC Pyth and Block Scholes objects, registers BTC, writes all six cadence records, bootstraps the pool with the configured DUSDC, and creates markets from longest to shortest enabled cadence. It transfers `MarketLifecycleCap` last with `sui::transfer::public_party_transfer` to a `sui::party::single_owner` party and verifies its `ConsensusAddressOwner` custody.
...
The schema-6 manifest contains the six fresh package IDs, shared objects, coin types, oracle bindings, lifecycle capability, earliest publication checkpoint, numeric units, and the initial protocol and cadence snapshot. Mutable `ProtocolConfig`, Predict `Registry`, `SessionsConfig`, and DeepBook `Registry` values are anchored to the exact object versions and digests read by the final audit.
