...
export interface MintReceipt {
	...
	/** Separate inventory-impact charge escrowed by the market. */
...
export interface RedeemReceipt {
	...
	/** NET quote credited: gross + inventory rebate − ordinary fees. */
	proceeds: number;
...
export function decodeMints(cfg: PredictConfig, result: DecodableTransactionResult): MintReceipt[] {
	return decodeAll(result, cfg.packages.predict, "order_events", "OrderMinted", OrderMintedBcs).map(
		(e) => ({
			...
			inventoryImpactCharge: fromRaw(BigInt(e.inventory_impact_charge), 6),
...
function liveProceedsRaw(e: {
	...
	builder_fee: string;
	penalty_fee: string;
	inventory_impact_rebate: string;
}): bigint {
	return (
		BigInt(e.redeem_amount) +
		BigInt(e.inventory_impact_rebate) -
...
export function decodeRedeems(
	...
	const live = decodeAll(
		...
		(e): RedeemReceipt => ({
			...
			inventoryImpactRebate: fromRaw(BigInt(e.inventory_impact_rebate), 6),
			builderCodeId: optId(e.builder_code_id),
			raw: {
				...
				builderFee: BigInt(e.builder_fee),
				penaltyFee: BigInt(e.penalty_fee),
				inventoryImpactRebate: BigInt(e.inventory_impact_rebate),
			},
		}),
	);
	return live;
