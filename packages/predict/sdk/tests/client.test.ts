...
const MINTED_EVENT = {
	...
	bcs: bcs
		...
		.serialize({
			...
			inventory_skew_charge: 40_000n, // $0.04 escrowed, not earned
...
const REDEEMED_EVENT = {
	...
	bcs: bcs
		...
		.serialize({
			...
			builder_fee: 0n,
			penalty_fee: 0n,
			inventory_skew_rebate: 10_000n, // $0.01 paid back out of the escrow
			builder_code_id: null,
		})
		.toBytes(),
};
...
describe("tx.mint (market resolution + unit conversion)", () => {
	...
	test("read.quoteMint dry-runs the mint and computes the all-in cost", async () => {
		...
		// cost = premium + (trading − subsidy) + builder + penalty + skew charge
		expect(q.inventorySkewCharge).toBe(0.04);
	...
	test("read.quoteRedeem dry-runs the close and returns NET proceeds", async () => {
		...
		// The rebate is credited on top of the gross, so it raises net proceeds.
		expect(q.inventorySkewRebate).toBe(0.01);
