...
fun test_cancel_orders_ok() {
    ...
    let order_info1 = pool_proxy::place_limit_order<USDC, USDT>(
        ...
        constants::self_matching_allowed(),
        1_000_000,
        1000, // Increased quantity to meet minimum size
        false, // is_bid (sell USDC for USDT)
        false,
        2000000, // expire_timestamp
        &clock,
...
fun test_submit_proposal_ok() {
    ...
    // Deposit and stake DEEP tokens first
    mm.deposit<USDC, USDT, DEEP>(
        &registry,
        mint_coin<DEEP>(5000 * 1_000_000_000, scenario.ctx()),
        scenario.ctx(),
    );
    ...
    pool_proxy::stake<USDC, USDT>(
        ...
        &mut mm,
        &mut pool,
        1000 * 1_000_000_000, // stake amount
        scenario.ctx(),
    );
