...
fun test_margin_trading_with_oracle() {
    ...
    let request = mm.borrow_quote<USDT, USDC>(
        &registry,
        &mut usdc_pool,
    ...
    mm.prove_and_destroy_request<USDT, USDC, USDC>(
        &registry,
        &mut usdc_pool,
        &pool,
        &usdt_price,
...
fun test_withdrawal_ok_when_risk_ratio_above_limit() {
    ...
    let borrow_request = mm.borrow_quote<USDT, USDC>(
        &registry,
        &mut usdc_pool,
    ...
    let (withdrawn_coin, withdraw_request) = mm.withdraw<USDT, USDC, USDT>(
        &registry,
        withdraw_amount,
        scenario.ctx(),
    );
    ...
    mm.prove_and_destroy_request<USDT, USDC, USDC>(
        &registry,
        &mut usdc_pool, // Use the pool where we have a loan
        &pool,
        &usdt_price,
        &usdc_price,
        &clock,
...
fun test_withdrawal_fails_when_risk_ratio_goes_below_limit() {
    ...
    let borrow_request = mm.borrow_quote<USDT, USDC>(
        &registry,
        &mut usdc_pool,
        4000 * test_constants::usdc_multiplier(),
        &clock,
        scenario.ctx(),
    );
    ...
    let (withdraw_coin, withdraw_request) = mm.withdraw<USDT, USDC, USDT>(
        &registry,
        7000 * test_constants::usdt_multiplier(),
        scenario.ctx(),
    );
...
fun test_borrow_fails_from_both_pools() {
    ...
    let request1 = mm.borrow_quote<USDT, USDC>(
        &registry,
        &mut usdc_pool,
        1000 * test_constants::usdc_multiplier(),
        &clock,
    ...
    let _request2 = mm.borrow_base<USDT, USDC>(
        &registry,
        &mut usdt_pool,
...
fun test_borrow_fails_with_zero_amount() {
    ...
    let _request = mm.borrow_quote<USDT, USDC>(
        &registry,
        &mut usdc_pool,
        0,
        &clock,
...
fun test_repay_fails_wrong_pool() {
    ...
    mm.repay_base<USDT, USDC>(
        &registry,
        &mut usdt_pool,
        option::some(1000 * test_constants::usdt_multiplier()),
        &clock,
        scenario.ctx(),
    );
...
fun test_repay_exact_amount_no_rounding_errors() {
    ...
    test_amounts.do!(|borrow_amount| {
        ...
        let exact_amount = usdc_pool.to_borrow_amount(borrowed_shares);
        ...
        if (remaining_shares > 0) {
            let remaining_amount = usdc_pool.to_borrow_amount(remaining_shares);
