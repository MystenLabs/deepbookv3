...
public(package) fun default_protocol_fees(ctx: &mut TxContext, clock: &Clock): ProtocolFees {
    ...
    manager.referrals.add(default_id, ReferralTracker {
        shares: 0,
        share_ms: 0,
...
public(package) fun mint_referral(self: &mut ProtocolFees, clock: &Clock, ctx: &mut TxContext): ID {
    ...
    self.referrals.add(id.to_address(), ReferralTracker {
        shares: 0,
        share_ms: 0,
        last_update_timestamp: clock.timestamp_ms(),
    });
...
public(package) fun increase_fees_per_share(self: &mut ProtocolFees, total_shares: u64, fees_accrued: u64) {
...
public(package) fun increase_shares(self: &mut ProtocolFees, referral: &Referral, shares: u64, clock: &Clock) {
...
public(package) fun decrease_shares(self: &mut ProtocolFees, referral: &Referral, shares: u64, clock: &Clock) {
    let referral_tracker = self.referrals.borrow_mut(referral.id.to_address());
    referral_tracker.update_share_ms(clock);
    referral_tracker.shares = referral_tracker.shares - shares;
}

public(package) fun calculate_claim(self: &mut ProtocolFees, referral: &mut Referral, clock: &Clock): u64 {
    let referral_tracker = self.referrals.borrow_mut(referral.id.to_address());
    referral_tracker.update_share_ms(clock);
