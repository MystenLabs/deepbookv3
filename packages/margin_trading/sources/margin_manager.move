...
public fun calculate_debts<BaseAsset, QuoteAsset, DebtAsset>(
    ...
    let base_debt = if (debt_is_base) {
        ...
        margin_pool.to_borrow_amount_round_up(debt_shares)
    } else {
        0
    };
...
fun repay<BaseAsset, QuoteAsset, RepayAsset>(
    ...
    let repay_amount = if (repay_amount.is_some()) {
        ...
        if (repay_is_base) {
            margin_pool.to_borrow_amount_round_up(self.base_borrowed_shares)
        } else {
            margin_pool.to_borrow_amount_round_up(self.quote_borrowed_shares)
        }
    };
