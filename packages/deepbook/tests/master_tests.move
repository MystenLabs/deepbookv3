...
fun test_withdraw_settled_amounts_permissionless_no_balance_e() {
    ...
    test.next_tx(BOB);
    {
        ...
        // This should fail with ENoBalanceToSettle
        pool::withdraw_settled_amounts_permissionless<SUI, USDC>(
            &mut pool,
            &mut alice_manager,
        );
...
fun test_master(error_code: u64) {
    ...
    // Alice stakes 100 DEEP into pool 1 during epoch 0 to be effective in epoch
    // 1
    stake<SUI, USDC>(
        ALICE,
        pool1_id,
    ...
    // Governance changed maker fees to 0.02%, taker fees to 0.06%, same deep
    // staking required
    ...
    // Bob places market ask order with large size in pool 1, only quantity 3
    // should be filled with Alice's bid order
    // Bob will not get discounted fees as even though he's staked, there's
    // volume traded yet
    ...
    // Bob proposes new fees as well after Alice unstakes, but quorum is based
    // on old voting power
    ...
    // then alice will place a bid for quantity 1
    // Discount will be 50% for Alice because sheplaces a maker order first
    // that's taken
    // Bob will not get discount because he takes Alice's order
    // Fees paid for each should be 0.02% maker for both, 0.03% taker for Alice,
    // 0.06% taker for Bob
    ...
    let avg_taker_fee = math::mul(
        taker_fee + math::mul(constants::half(), taker_fee),
        constants::half(),
    );
    ...
    // quantity being traded is halved, each person will make 0.5 quantity and
    // take 0.5 quantity
...
fun test_master_input_tokens(error_code: u64) {
    ...
    // Alice places bid order in pool 1, cancels, then places the same order
    let order_info_1 = pool_tests::place_limit_order<SUI, USDC>(
        ALICE,
        pool1_id,
        alice_balance_manager_id,
    ...
    let usdc_penalty_fee = math::mul(
        constants::fee_penalty_multiplier(),
        usdc_base_fee,
    );
    alice_balance.usdc = alice_balance.usdc - usdc_asset - usdc_penalty_fee;

    // Alice places ask order in pool 2, cancels, then places the same order
    ...
    let spam_asset = quantity;
    let spam_base_fee = math::mul(maker_fee, spam_asset);
    let spam_penalty_fee = math::mul(
        constants::fee_penalty_multiplier(),
        spam_base_fee,
    );
    alice_balance.spam = alice_balance.spam - quantity - spam_penalty_fee;
    ...
    alice_balance.usdc = alice_balance.usdc + math::mul(price, quantity);
    let usdc_base_fee = math::mul(old_maker_fee, usdc_asset);
    let usdc_penalty_fee = math::mul(
        constants::fee_penalty_multiplier(),
        usdc_base_fee,
    );
    alice_balance.usdc = alice_balance.usdc + usdc_penalty_fee;
    check_balance(
        alice_balance_manager_id,
        &alice_balance,
        &mut test,
    ...
    // that's taken
    // Bob will not get discount because he takes Alice's order
    // Fees paid for each should be 0.02% maker for both, 0.06% taker for Alice,
    // 0.06% taker for Bob
    // Alice should have 46 more SUI at the end of the loop
    // Bob should have 92 more USDC at the end of the loop
    while (i > 0) {
...
fun test_master_deep_price(error_code: u64) {
    let mut test = begin(OWNER);
    let registry_id = pool_tests::setup_test(OWNER, &mut test);
    ...
    let pool1_id = pool_tests::setup_reference_pool<SUI, DEEP>(
        OWNER,
        registry_id,
        owner_balance_manager_id,
        100 * constants::float_scaling(),
    ...
    // Alice should pay 0.05% * (quantity 1 * price 10 * conversion 150) = 0.75
    // DEEP
    ...
    // Both alice and bob will pay 0.05% * (quantity 1 * price 10 * conversion
    // 150) = 0.75 DEEP
    // for maker order and 0.10% * (quantity 1 * price 10 * conversion 150) =
    // 0.75 DEEP for taker order
...
fun test_flash_loan(error_code: u64) {
    let mut test = begin(OWNER);
    let registry_id = pool_tests::setup_test(OWNER, &mut test);
    pool_tests::set_time(0, &mut test);
    ...
    let reference_pool_id = pool_tests::setup_reference_pool<SUI, DEEP>(
        OWNER,
        registry_id,
        owner_balance_manager_id,
        100 * constants::float_scaling(),
    ...
    let pool_id = pool_tests::setup_pool_with_default_fees<SUI, USDT>(
        OWNER,
        registry_id,
        false,
        false,
    ...
    // Owner places a bid order of at price 100 for 10 SUI into pool 1, which is
    // a SUI/DEEP pool
    ...
    test.next_tx(ALICE);
    {
        let mut loan_pool = test.take_shared_by_id<Pool<SUI, DEEP>>(
            reference_pool_id,
        );
        ...
        // Alice needs to swap SUI back to DEEP in a deep pool, in this scenario
        // also the loan pool
        // to pay back the flash loan. She places a market order of 1 SUI for
        // 100 DEEP.
        ...
        // Alice withdraws the 1 DEEP she borrowed from balance_manager and
        // returns the loan
        let quote_return = alice_balance_manager.withdraw<DEEP>(
            quote_needed,
            test.ctx(),
        );

        if (error_code == EIncorrectLoanPool) {
            let wrong_quote_return = alice_balance_manager.withdraw<USDT>(
                quote_needed,
                test.ctx(),
            );
    ...
    test.next_tx(ALICE);
    {
        ...
        let base_return = alice_balance_manager.withdraw<SUI>(
            base_needed,
            test.ctx(),
        );
    ...
    // Alice borrows the base asset, and tries to return the quote asset in
    // place of the base asset
    // This will fail
...
fun test_master_both_conversion_available(deep_is_base: bool) {
    let mut test = begin(OWNER);
    let registry_id = pool_tests::setup_test(OWNER, &mut test);
    pool_tests::set_time(0, &mut test);
    ...
    // Since both price points are available, SPAM (quote) conversion should be
    // used
...
fun test_trader_permission_and_modify_returned(error_code: u64) {
    ...
    let bob_trade_cap_id = authorize_trader(
        ALICE,
        alice_balance_manager_id,
        BOB,
        &mut test,
    ...
    // Bob places an order with quantity 10 in SUI/USDC pool at a price of 2
    // using Alice's balance manager
    ...
    // Owner places an ask order at the same price matches with 5 of Alice's
    // order
    pool_tests::place_limit_order<SUI, USDC>(
        OWNER,
        pool1_id,
    ...
    // Alice cancels the order herself, should get correct refund of remaining
    // quantity
    pool_tests::cancel_order<SUI, USDC>(
        ALICE,
        pool1_id,
...
fun test_get_level_2_range() {
    ...
    // We call get level 2 range for the reference pool, should return correct
    // vectors
    let mut test = begin(OWNER);
    let registry_id = pool_tests::setup_test(OWNER, &mut test);
    pool_tests::set_time(0, &mut test);
    ...
    // OWNER places another bid order in the reference pool at price 20 and
    // quantity 2
    ...
    // OWNER places another order in the reference pool at price 30 and quantity
    // 5
    ...
    // OWNER places another ask order in the reference pool at price 180 and
    // quantity 2
...
fun claim_rebates<BaseAsset, QuoteAsset>(
    ...
    test.next_tx(sender);
    {
        ...
        let trade_proof = my_manager.generate_proof_as_trader(
            &trade_cap,
            test.ctx(),
        );
...
fun check_locked_balance<BaseAsset, QuoteAsset>(
    ...
    let (base, quote, deep) = locked_balance<BaseAsset, QuoteAsset>(
        sender,
        pool_id,
        balance_manager_id,
        test,
...
fun get_level2_ticks_from_mid<BaseAsset, QuoteAsset>(
    ...
    test.next_tx(sender);
    {
        ...
        let (
            ...
            ask_prices,
            ask_quantities,
        ) = pool.get_level2_ticks_from_mid<BaseAsset, QuoteAsset>(
            ticks,
            &clock,
        );
        return_shared(pool);
...
fun locked_balance<BaseAsset, QuoteAsset>(
    ...
    test.next_tx(sender);
    {
        ...
        let (base, quote, deep) = pool::locked_balance<BaseAsset, QuoteAsset>(
            &pool,
            &my_manager,
        );
        return_shared(pool);
