...
module deepbook::pool_metadata_tests {
    ...
    fun new_proposal() {
        ...
        metadata.adjust_voting_power(300, 0);
        metadata.refresh(1); // quorum = 150
        metadata.add_proposal(200, 500, 10000);

        // Alice votes with 100 stake, not enough to push proposal 0 over quorum
        let winning_proposal = metadata.vote(option::some(0), option::none(), 100);
        ...
        metadata.add_proposal(300, 600, 20000);

        // Alice moves 100 stake from proposal 0 to 1, but not enough to push proposal 1 over quorum
        let winning_proposal = metadata.vote(option::some(1), option::some(0), 100);
        let (maker_fee, taker_fee, stake_required) = winning_proposal.borrow().proposal_params();
        assert!(maker_fee == 200, 0);
        assert!(taker_fee == 500, 0);
        assert!(stake_required == 10000, 0);
        ...
        let winning_proposal = metadata.vote(option::some(1), option::none(), 200);
        let (maker_fee, taker_fee, stake_required) = winning_proposal.borrow().proposal_params();
    ...
    fun new_proposal_stable() {
        ...
        metadata.add_proposal(20, 50, 10000);
        metadata.add_proposal(50, 100, 20000);
    ...
    fun voting_power() {
        ...
        next_tx(&mut scenario, owner);
        {
            ...
            // Bob votes
            metadata.vote(option::some(0), option::none(), 200);
            ...
            metadata.adjust_voting_power(0, 100);
            ...
            metadata.adjust_voting_power(0, 3000);
    ...
    fun proposal_does_not_exist_e() {
        ...
        next_tx(&mut scenario, owner);
        {
            let mut metadata = pool_metadata::empty(0);
            metadata.add_proposal(200, 500, 10000);
            metadata.vote(option::some(1), option::none(), 100);
            abort 0
        }
    }
    ...
    fun new_proposal_taker_too_high_e() {
        ...
        {
            let mut metadata = pool_metadata::empty(0);
            metadata.add_proposal(200, 1001, 10000);
            abort 0
        }
    }
    ...
    fun new_proposal_taker_too_low_e() {
        ...
        {
            let mut metadata = pool_metadata::empty(0);
            metadata.add_proposal(200, 499, 10000);
            abort 0
        }
    }
    ...
    fun new_proposal_taker_too_high_stable_e() {
        ...
        next_tx(&mut scenario, owner);
        {
            let mut metadata = pool_metadata::empty(0);
            metadata.set_as_stable(true);
            metadata.add_proposal(50, 200, 10000);
            abort 0
        }
    }
    ...
    fun new_proposal_maker_too_high_e() {
        ...
        {
            let mut metadata = pool_metadata::empty(0);
            metadata.add_proposal(501, 500, 10000);
            abort 0
        }
    }
    ...
    fun new_proposal_maker_too_low_e() {
        ...
        {
            let mut metadata = pool_metadata::empty(0);
            metadata.add_proposal(199, 500, 10000);
            abort 0
        }
    }
