...
module deepbook::account_tests {
    ...
    fun process_maker_fill_ok() {
        ...
        let mut account = account::empty(test.ctx());
        account.add_order(1);
        let fill = fill::new(1, id_from_address(@0xB), false, false, 100, 500, false, 0, constants::deep_multiplier(), true, false);
        ...
        account.add_order(2);
        let fill = fill::new(2, id_from_address(@0xC), false, true, 100, 500, true, 0, constants::deep_multiplier(), true, false);
        account.process_maker_fill(&fill);
        let (settled, owed) = account.settle();
        ...
        account.add_order(3);
        let fill = fill::new(3, id_from_address(@0xC), true, false, 100, 500, true, 0, constants::deep_multiplier(), true, false);
        account.process_maker_fill(&fill);
        let (settled, owed) = account.settle();
        assert_eq(settled, balances::new(100, 0, 0));
        assert_eq(owed, balances::new(0, 0, 0));
        ...
        account.add_order(4);
        let fill = fill::new(4, id_from_address(@0xC), false, true, 100, 500, true, 0, constants::deep_multiplier(), true, false);
        account.process_maker_fill(&fill);
        let (settled, owed) = account.settle();
        assert_eq(settled, balances::new(0, 500, 0));
        assert_eq(owed, balances::new(0, 0, 0));
