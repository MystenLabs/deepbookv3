use  rate::handlers::{is_deepbook_tx, struct_tag, try_extract_move_call_package};
use  rate::models::deepbook::vault::FlashLoanBorrowed;
use  sync_trait::async_trait;
...
use tracing::debug;
    ...
    event_type: StructTag,
     ...
     FlashLoanHandler {
          new(package_id_override: Option<AccountAddress>) -> Self {
        Self {
    ...
    Processor     FlashLoanHandler {
    const NAME: &'static str = "FlashLoan";
         Value = Flashloan;

       process(&self, checkpoint: &Arc<CheckpointData>) -> anyhow::Result<Vec<Self::Value>> {
        checkpoint
            .transactions
            .iter()
...
#[async_trait]
                FlashLoanHandler {
     sync    commit(values: &[Self::Value], conn: &mut Connection<'_>) -> anyhow::Result<usize> {
