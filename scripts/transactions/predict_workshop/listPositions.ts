...
const fmtUsd = (raw: number | string | bigint | null | undefined, scale = DUSDC_SCALE): string => {
...
(async () => {
	...
	for (const b of summary.balances ?? []) {
		console.log(`Balance:                $${fmtUsd(b.balance)} (${b.quote_asset.split('::').pop()})`);
	}
	...
	const positions: any[] = await get(`/managers/${managerId}/positions/summary`);
	...
	for (const p of open) {
		...
		const entry = (Number(BigInt(p.average_entry_price)) / Number(PRICE_SCALE)).toFixed(4);
		const mark = p.mark_price ? (Number(BigInt(p.mark_price)) / Number(PRICE_SCALE)).toFixed(4) : '—';
	...
	const mints: RangeEvent[] = await get(`/ranges/minted?manager_id=${managerId}`).catch(() => []);
	const redeems: RangeEvent[] = await get(`/ranges/redeemed?manager_id=${managerId}`).catch(() => []);
