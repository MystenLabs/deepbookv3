...
```
`preflight.py` guards the primer (the single point of failure — its module map + D-id citations reach every subagent): a missing module path is fatal, a D-id that resolves only to the local decision journal warns (promote it into a committed ledger). The workflows also self-warn if `groundTruth` looks empty/short, but run the lint yourself so drift is caught before the launch questionnaire.
...
Purpose: surface the easy cleanups (mechanical rule violations, hygiene, surface-area/assertion/architecture nits) for the operator to fix or disposition BEFORE a deep run, so the expensive lenses spend their `maxFindings` slots on genuinely new ground instead of re-finding known easy stuff. Everything is reported RAW (`verifyCap 0`, no codex panels) — the operator is the verifier for this tier. ~18 finder agents + 1 promote total, a few hundred k tokens. Still gated: present this plan + cost and get an explicit "yes" first.
```
Workflow({ scriptPath: '.claude/skills/predict-audit/rule-sweep.workflow.js',
           args: { groundTruth: '<build/test summary>', depth: 'mini' } })          # all 14 mechanical families
Workflow({ scriptPath: '.claude/skills/predict-audit/orchestrator.workflow.js',
