...
const RULE_FAMILIES = [
  ...
  { key: 'config-api-shape', scope: 'packages/predict/sources',
    rule: 'Public admin entrypoints live on the module owning the mutated state: `protocol_config` for global config, object modules for per-object admin state, `registry` for version/pause-cap/lifecycle-cap/pool-valuation-cap/uniqueness/multi-object creation. Embedded config-struct setters/constructors/bounds/template wiring stay `public(package)`. Global-template setters (affect future but not existing objects) include `template` in the name.',
