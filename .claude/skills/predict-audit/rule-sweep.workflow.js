...
const RULE_FAMILIES = [
  ...
  { key: 'module-layout', scope: ALL,
    rule: 'Within a module: `public fun` first, then `public(package) fun`, then private `fun`; within each visibility group, read-only/query functions before mutating ones; private helpers placed after their first caller when that does not break the visibility grouping; `init` early (after struct definitions). Every Predict-cluster source file starts with the Mysten copyright/SPDX header and has a module-level `///` doc immediately before `module`. Error constants are `EPascalCase`, regular constants `ALL_CAPS`, capabilities suffixed `Cap`, events named in past tense.',
