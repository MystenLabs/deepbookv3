...
- After a settled-market sweep, the expiry may retain only payout and unresolved rebate backing. Return free cash and unused fee incentives to `PoolVault`; live rebalancing must not run for expired or settled markets.
...
- Terminology: docs use options/structured-product vocabulary (canonical glossary: `packages/predict/docs/glossary.md`). Mint-economics identifiers are `entry_value`, `net_premium`, `min_net_premium` (and the `OrderMinted.net_premium` event field); the holder pays the contract's full entry value, so net premium equals entry value. The EWMA congestion surcharge keeps core's `penalty` vocabulary in code.
...
- The payout tree stores one atom per boundary (`quantity`); insertion, removal, reinsertion, and settlement recompute all read it through `strike_payout_tree::payout_terms`. Every atom must round-trip bit-identically through the packed order ID, and flow policy must validate before evaluation.
