...
- **NAV-mark directional invariant — never undercount the SUPPLY mark.** The NAV mark used to price PLP **supply** must be an *upper bound* on true recoverable value (`supply_NAV >= TRUE_NAV`): a supplier priced `>= TRUE` mints `<=` fair shares, so it can never over-mint and dilute incumbents. The **withdraw** mark must satisfy `withdraw_NAV <= supply_NAV` and MAY sit *below* TRUE — under-paying an exiting LP only protects remaining holders and never threatens solvency (cash backing is enforced separately and conservatively), so the reduction below the supply mark is an intentional, tunable withdraw fee. Concretely: both legs share the hard `OPTIMISTIC - Q` mark (`>= TRUE`, `Q = max(0, D_max - unscanned_range)`); withdraw additionally subtracts a *soft* estimate `L_U` of unscanned ITM liability to land `~= TRUE` (may dip below). Approximation/statistics are allowed ONLY in the withdraw-side reduction (over-estimating is safe; under-estimating is a bounded residual never worse than the `Q` mark), NEVER in the supply mark, which stays a hard upper bound. Never let `withdraw_NAV > supply_NAV` (free round-trip) or push `supply_NAV < TRUE`. Liveness clamps must *maximize* NAV when they fire.
