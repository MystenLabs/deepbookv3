...
- After renaming a `public(package)` or `public` function, grep the entire package for the old name in **doc comments** (`///`) too — the compiler resolves call sites and import paths but never checks identifiers inside `///` blocks, so references like `Read by \`pricing_config::compute_up_quote\` to gauge ...` will silently rot. Every rename should end with a `grep -rn '<old_name>' --include='*.move'` sweep across both `sources/` and `tests/`. Same applies after behavior changes that invalidate documented invariants (e.g., removing a clamp): re-check doc comments on the public-API function and on any module-level docs that paraphrased the old behavior.

- When implementing financial / pricing math that **intentionally deviates from a well-known standard form**, add a code comment at the deviation point that names the standard, cites the deviation, and explains the reasoning. The bar isn't "is this code complex" but "would a future maintainer who knows the textbook formula assume this is a typo and 'fix' it?" Example: a binary-option inventory shift that uses `tte_factor = √(τ_ref/τ)` (amplifies near expiry) inverts the Avellaneda-Stoikov vanilla convention `(T−t)` (decreases near expiry). The math is correct for binaries — binary delta scales as `1/√τ`, so risk-stationary inventory accounting requires the inverted scaling — but a maintainer reading just the line of code would suspect a sign error. Naming the standard ("differs from A-S; binary delta gamma blows up near ATM as `1/√τ`") inoculates against future "corrections." This is stricter than the general "complex logic? leave a comment" rule: it specifically targets *standard-deviating* math, where the danger isn't complexity but apparent-but-false familiarity.
