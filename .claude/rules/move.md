...
- Authorization-state placement is constraint-driven, not stylistic: the allowlist must live in a module at or below every gating call site in the dependency order, because struct fields are module-private and Move forbids import cycles. The market-lifecycle allowlist lives on `Registry` because its lone gating call site is `registry::create_expiry_market` — the allowlist sits on the consumer module itself. (It briefly lived on `PoolVault` (`plp`) when `plp::compact_storage` was a second consumer in a lower module and both call sites had to reach one allowlist; the prune deleted that path, leaving `registry` the sole consumer.) Within that constraint there are two shapes: a protocol-wide allowlist on a singleton for cross-cutting ops (`PauseCap` and `MarketLifecycleCap`, both on `Registry`), and a per-instance set on the guarded object for instance-scoped authority (`MarketOracleWriterCap` per oracle, manager caps per `PredictManager`). Pick by intended blast radius.
...
- **NAV-mark directional invariant — never undercount the SUPPLY mark.** The NAV mark used to price PLP **supply** must be an *upper bound* on true recoverable value (`supply_NAV >= TRUE_NAV`): a supplier priced `>= TRUE` mints `<=` fair shares, so it can never over-mint and dilute incumbents. PLP owns the active-expiry mark from one limited-recourse bucket decomposition: `pool_nav` returns free cash plus raw range/floor totals; PLP computes `supply_liability = max(0, verified_range - verified_floor) + max(0, unscanned_range - unscanned_floor)` and `band = min(unscanned_floor, unscanned_range)`. The **withdraw** mark must satisfy `withdraw_NAV <= supply_NAV` and MAY sit *below* TRUE — under-paying an exiting LP only protects remaining holders and never threatens solvency (cash backing is enforced separately and conservatively), so the reduction below the supply mark is an intentional, tunable withdraw fee. Approximation/statistics are allowed ONLY in the withdraw-side reduction (over-estimating is safe; under-estimating is a bounded residual), NEVER in the supply mark, which stays a hard upper bound. Never let `withdraw_NAV > supply_NAV` (free round-trip) or push `supply_NAV < TRUE`. Liveness clamps must *maximize* NAV when they fire.
