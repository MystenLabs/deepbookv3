...
- To remove human-EOA authority from an object that uses an owner-address sentinel (e.g. `BalanceManager.owner`), set the sentinel to the wrapping object's own ID-as-address: `let owner = id.to_inner().to_address();` where `id: UID` comes from `derived_object::claim` or `object::new`. No `ctx.sender()` can ever equal that address, so `assert_owner(ctx)` becomes permanently unreachable and any cap pattern minted at construction time is the only remaining authority. Pair with a constructor like `balance_manager::new_with_custom_owner_caps_v2` that mints initial caps in the same call (post-construction cap minting goes through the now-dead owner check). Useful for contracts (vaults, custodial products) that don't want a deployer-key trust anchor.

- When a compound constructor returns more capabilities than the caller needs (e.g. `new_with_custom_owner_caps_v2` hands back a `TradeCap` even if you only use deposit/withdraw), and the unused cap type lacks `drop`, the cleanest workaround is `Option<UnusedCap>` as a dead field on the wrapping struct. Avoid `transfer::public_transfer(..., @0x0)` — that creates a stranded owned object in storage. Better long-term fix is asking the upstream module to expose a `destroy_*` for the cap; the `Option<T>` field is a stopgap, not a pattern to celebrate.

- For functions that dispatch on state where only some branches need a proof/witness, take it as `Option<Proof>` by value rather than splitting into two public functions. If `Proof: drop`, branches that don't consume it just let the `Option` fall out of scope — no explicit `destroy_none`/`destroy_some` needed. The live branch does `assert!(proof.is_some(), E...)` then `let p = proof.destroy_some(); ...&p...`. Keeps a single entrypoint for callers who would otherwise have to dispatch on state themselves to pick which function to call. Note: `Option<&Proof>` doesn't work since references aren't `store`; the proof must be passed by value.

- A container object declared `has key` (no `store`) can only exit the constructing module through whatever transfer/share/freeze function that module chooses to expose — there's no `transfer::public_*` path for key-only types. In practice that often means the module exposes one `share()` helper and callers are forced to share. To let callers *wrap* the object inside another struct as a field (e.g. a vault that contains a manager), declare it `has key, store`; that enables `transfer::public_transfer/freeze/share` from any module and lets it appear as a field in any other struct. BalanceManager uses `key, store` for this reason. Adding `store` is generally low-risk when all mutations are gated by explicit owner/cap checks — `store` only widens the lifecycle surface, not the authority surface.

- When you need multiple "kinds" of derived objects per address (e.g. sender-owned vs self-owned manager), don't reach for a second key struct. Reuse the existing `(address, u64)` key shape and reserve named slots in `constants.move`: `public macro fun sender_owned_manager_slot(): u64 { 0 }`, `self_owned_manager_slot(): u64 { 1 }`. Each constructor calls `derived_object::claim(reg, Key(sender, constants::*_slot!()))`. Cheaper than a parallel key type, and the slot reservation is documented in one place. Future "multi-manager-per-sender" extensions can use higher indices.
