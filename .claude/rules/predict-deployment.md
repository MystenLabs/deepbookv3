...
- Mainnet collateral resolves through Circle's upstream source and publication records, not the local test currency. Do not publish `packages/usdc` on Mainnet or claim Circle's address in that package's publication history. The mechanism is owned by [design decisions](../../packages/predict/docs/design/decisions.md).
