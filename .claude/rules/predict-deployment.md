...
- One dependency fails that verification by design: on mainnet, `usdc` is Circle's package and `packages/usdc` is only the type declaration that lets one source resolve on both networks, so its bytecode is not Circle's and never will be. Verify the linked package and original IDs against the target chain instead of the source, and keep the skip scoped to that publish command. The mechanism is owned by [design decisions](../../packages/predict/docs/design/decisions.md).
