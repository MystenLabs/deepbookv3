...
The declared source changes are Pyth’s generated `meta.move` version (`2`, receiver chain `21`) and the Wormhole dependency: the exact `sui/mainnet` commit `b71be5cbb9537c4aac8e23e74371affa3825efcd`. No upstream tests are vendored because their signed fixtures target version 1; consumer tests live in `packages/propbook` and `packages/predict`.
